...
def _braced_block_after(source: str, marker: str) -> str:
    """Return the balanced source block opened by *marker*, independent of indentation."""
    marker_start = source.index(marker)
    open_brace = source.index("{", marker_start)
    depth = 0
...
def test_desktop_update_offer_remains_actionable_from_settings():
    ...
    assert "checkForUpdate," in hook.rsplit("return {", 1)[1]
...
def test_empty_native_updater_channel_is_quietly_disabled():
    """An empty Tauri endpoint list makes UpdaterBuilder fail with EmptyEndpoints.

    The native policy must surface that configuration as disabled so the renderer never
    turns the intentionally absent channel into a startup/hourly update-check error.
    """
    config = (REPO / "studio/src-tauri/tauri.conf.json").read_text(encoding = "utf-8")
    hook = TAURI_UPDATE_HOOK.read_text(encoding = "utf-8")
    settings = DESKTOP_UPDATE_CONTROL.read_text(encoding = "utf-8")
    policy = DESKTOP_UPDATE_POLICY.read_text(encoding = "utf-8")

    assert '"endpoints": []' in config
    assert "Disabled" in policy
    assert "updater_endpoints_configured(&app)" in policy
    assert 'if (policy.mode === "disabled") {' in hook
    disabled_branch = _braced_block_after(hook, 'if (policy.mode === "disabled") {')
    assert "setCheckError(null);" in disabled_branch
    assert "checkDesktopUpdate()" not in disabled_branch
    assert settings.count('update.updatePolicyMode === "disabled"') == 2
...
def test_desktop_manages_the_remote_password_through_the_account_dialog():
    ...
    assert "await storeAuthTokensSecurely(accessToken, refreshToken)" in submitted
...
def test_desktop_titlebar_separates_navigation_from_sidebar_brand():
    ...
    assert 'aria-label="Helix Harness home"' in header
    assert "HELIX HARNESS" in header
    assert header.index("<DesktopTitlebarNavigation") < header.index('aria-label="Helix Harness home"')
