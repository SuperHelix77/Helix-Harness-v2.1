...
def test_macos_v2_build_flavor_changes_only_bundle_identity() -> None:
    base = json.loads(read(TAURI / "tauri.conf.json"))
    flavor = json.loads(read(TAURI / "tauri.helix-v2.conf.json"))

    assert base["productName"] == "Helix Harness"
    assert base["identifier"] == "ai.helix.harness"
    assert flavor == {
        "productName": "Helix Harness v2",
        "identifier": "ai.helix.harness.v2",
    }

    build = read(REPO / "scripts/build_helix_macos_v2.sh")
    assert "--bundles app" in build
    assert "--config src-tauri/tauri.helix-v2.conf.json" in build


def test_macos_v2_1_build_flavor_has_distinct_public_identity() -> None:
    flavor = json.loads(read(TAURI / "tauri.helix-v2-1.conf.json"))
    assert flavor["productName"] == "Helix Harness v2.1"
    assert flavor["identifier"] == "ai.helix.harness.v21"
    assert flavor["app"]["windows"][0]["title"] == "Helix Harness v2.1"
    assert flavor["app"]["windows"][0]["resizable"] is True
    assert flavor["app"]["windows"][0]["transparent"] is True
    assert flavor["plugins"]["deep-link"]["desktop"]["schemes"] == [
        "helixharness-v21"
    ]
    assert flavor["bundle"]["homepage"] == (
        "https://github.com/SuperHelix77/Helix-Harness-v2.1"
    )

    build = read(REPO / "scripts/build_helix_macos_v2_1.sh")
    assert "--bundles app" in build
    assert "--config src-tauri/tauri.helix-v2-1.conf.json" in build
