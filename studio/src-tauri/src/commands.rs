...
pub async fn start_managed_server(
    ...
    // A newer desktop bundle may be installed over an already-functional managed
    // Studio runtime without running the managed installer/update path.  The Python
    // backend shipped with the app is part of the desktop contract, so reconcile it
    // before spawning the managed server.  The applier hashes all overlay-owned files
    // first and is a fast no-op when the runtime already matches; stale runtimes use
    // the existing atomic stage/swap/validate/rollback path.
    #[cfg(target_os = "macos")]
    {
        let managed_bin = process::find_unsloth_binary()
            .ok_or_else(|| "Managed Unsloth binary is unavailable.".to_string())?;
        let overlay_app = app.clone();
        let overlay_bin = managed_bin.clone();
        let overlay_result = tauri::async_runtime::spawn_blocking(move || {
            update::apply_bundled_helix_backend_overlay(&overlay_app, &overlay_bin)
        })
        .await
        .map_err(|error| format!("Helix backend overlay task failed: {error}"))?;
        overlay_result?;
    }
