...
pub(crate) use version::{
    backend_version_stale_reason, expected_backend_version,
    helix_backend_identity_stale_reason, DESKTOP_BACKEND_MANAGEABILITY_VERSION,
    DESKTOP_PROTOCOL_VERSION,
};
#[cfg(test)]
pub(crate) use version::expected_helix_backend_identity;
...
mod tests {
    ...
    fn fake_cli(test_name: &str, script: &str) -> FakeCli {
        ...
        // Managed preflight now verifies the Helix backend contract through the
        // managed runtime's sibling Python before it probes desktop capabilities.
        // These tests isolate CLI capability classification/cache ordering, so
        // provide a successful contract-probe runtime rather than accidentally
        // failing earlier because the synthetic CLI directory has no Python.
        let python = dir.join("python");
        fs::write(&python, "#!/bin/sh\nexit 0\n").unwrap();
        let mut python_perms = fs::metadata(&python).unwrap().permissions();
        python_perms.set_mode(0o755);
        fs::set_permissions(&python, python_perms).unwrap();
        FakeCli { bin, dir }
    }
...
"#,
    ...
    fn helix_identity_json() -> String {
        let (contract, tree) =
            version::expected_helix_backend_identity().expect("bundled Helix manifest");
        format!(
            r#""helix_backend_contract":"{contract}","helix_backend_tree_sha256":"{tree}","helix_backend_verified":true,"#
        )
    }

    fn desktop_ready_health_with_owner(root_id: &str, include_owner: bool) -> String {
        let owner = desktop_owner_json(include_owner);
        let helix = helix_identity_json();
        ...
        format!(
            r#"{{"status":"healthy","service":"Unsloth UI Backend","version":"2026.8.4","desktop_protocol_version":1,"desktop_manageability_version":2,"supports_desktop_auth":true,"supports_desktop_backend_ownership":true,{leases}{helix}"studio_root_id":"{root_id}"{owner}}}"#
        )
    }
    ...
    async fn legacy_manageability_same_root_backend_is_still_ready() {
        ...
        let probe = probe_test_backend(
            format!(
                r#"{{"status":"healthy","service":"Unsloth UI Backend","version":"2026.8.4","desktop_protocol_version":1,"desktop_manageability_version":1,"supports_desktop_auth":true,"supports_desktop_backend_ownership":true,"native_path_leases_supported":true,{helix}"studio_root_id":"{EXPECTED_ROOT_ID}"{}}}"#,
                desktop_owner_json(true)
            ),
            "401 Unauthorized",
        )
    ...
    async fn terminal_started_backend_without_native_path_leases_stays_adoptable() {
        ...
        let probe = probe_test_backend(
            format!(
                r#"{{"status":"healthy","service":"Unsloth UI Backend","version":"2026.8.4","desktop_protocol_version":1,"desktop_manageability_version":2,"supports_desktop_auth":true,"supports_desktop_backend_ownership":true,"native_path_leases_supported":false,{helix}"studio_root_id":"{EXPECTED_ROOT_ID}"}}"#,
            ),
            "401 Unauthorized",
        )
        .await;
    ...
    async fn backend_predating_the_lease_capability_field_stays_adoptable() {
        ...
        let probe = probe_test_backend(
            format!(
                r#"{{"status":"healthy","service":"Unsloth UI Backend","version":"2026.8.4","desktop_protocol_version":1,"desktop_manageability_version":2,"supports_desktop_auth":true,"supports_desktop_backend_ownership":true,{helix}"studio_root_id":"{EXPECTED_ROOT_ID}"}}"#,
            ),
            "401 Unauthorized",
        )
        .await;
    ...
    async fn a_stale_version_is_still_reported_as_a_version_problem() {
        ...
        let probe = probe_test_backend(
            format!(
                r#"{{"status":"healthy","service":"Unsloth UI Backend","version":"2026.5.1","desktop_protocol_version":1,"desktop_manageability_version":2,"supports_desktop_auth":true,"supports_desktop_backend_ownership":true,"native_path_leases_supported":false,{helix}"studio_root_id":"{EXPECTED_ROOT_ID}"}}"#,
            ),
            "401 Unauthorized",
        )
        .await;
    ...
    async fn stale_same_root_without_desktop_owner_is_external_conflict() {
        ...
        let probe = probe_test_backend(
            format!(
                r#"{{"status":"healthy","service":"Unsloth UI Backend","version":"2026.5.1","desktop_protocol_version":1,"desktop_manageability_version":2,"supports_desktop_auth":true,"supports_desktop_backend_ownership":true,{helix}"studio_root_id":"{EXPECTED_ROOT_ID}"}}"#,
            ),
            "401 Unauthorized",
        )
        .await;
