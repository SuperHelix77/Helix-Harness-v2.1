...
fn confirm_quit_during_install(app: &tauri::AppHandle) -> bool {
    ...
    app.dialog()
        .message(
            "Helix Harness is still installing. Quitting now stops it part-way and \
...
fn confirm_quit_during_update(app: &tauri::AppHandle) -> bool {
    ...
    app.dialog()
        .message(
            "Helix Harness is still updating. Quitting now stops it part-way and \
             leaves the installation incomplete, so it will need to be repaired before \
             it can start.",
        )
        .kind(MessageDialogKind::Warning)
...
fn setup_quit_menu(app: &tauri::App) -> Result<(), Box<dyn std::error::Error>> {
    ...
    let quit = MenuItemBuilder::with_id(APP_QUIT_MENU_ID, "Quit Helix Harness")
...
fn setup_tray(app: &tauri::App) -> Result<(), Box<dyn std::error::Error>> {
    let open = MenuItemBuilder::with_id("open", "Open Helix Harness").build(app)?;
...
fn main() {
    ...
    setup_logging();
    info!("Helix Harness desktop app starting");
    ...
    tauri::Builder::default()
        ...
        .invoke_handler(tauri::generate_handler![
            ...
            desktop_auth::desktop_refresh_auth,
            desktop_auth::desktop_store_refresh_auth,
            desktop_auth::desktop_clear_refresh_auth,
