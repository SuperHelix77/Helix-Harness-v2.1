...
use super::version::{
    backend_version_stale_reason, helix_backend_identity_stale_reason,
    DESKTOP_BACKEND_MANAGEABILITY_VERSION, DESKTOP_PROTOCOL_VERSION,
};
...
pub(super) async fn backend_health(client: &reqwest::Client, port: u16) -> Option<BackendHealth> {
    ...
    let service = json
        .get("service")
        .and_then(|v| v.as_str())
        .map(|s| crate::desktop_service::is_studio_backend_service(Some(s)))
        .unwrap_or(false);
    info!(
    ...
    let helix_backend_contract = json
        .get("helix_backend_contract")
        .and_then(|v| v.as_str())
        .map(ToOwned::to_owned);
    let helix_backend_tree_sha256 = json
        .get("helix_backend_tree_sha256")
        .and_then(|v| v.as_str())
        .map(ToOwned::to_owned);
    let helix_backend_verified = json
        .get("helix_backend_verified")
        .and_then(|v| v.as_bool());
    ...
    Some(BackendHealth {
        ...
        studio_root_id,
        helix_backend_contract,
        helix_backend_tree_sha256,
        helix_backend_verified,
        desktop_owner,
        version,
        stale_reason,
    })
...
fn backend_capability_stale_reason(health: &BackendHealth) -> Option<String> {
    ...
    if let Some(reason) = helix_backend_identity_stale_reason(
...
mod tests {
    ...
    fn health(studio_root_id: Option<&str>) -> BackendHealth {
        let (contract, tree) =
            super::super::version::expected_helix_backend_identity().expect("bundled Helix manifest");
        BackendHealth {
            ...
            helix_backend_contract: Some(contract.to_string()),
            helix_backend_tree_sha256: Some(tree.to_string()),
