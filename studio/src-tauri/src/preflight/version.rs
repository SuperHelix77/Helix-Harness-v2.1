...
static EXPECTED_HELIX_IDENTITY: OnceLock<Option<(String, String)>> = OnceLock::new();

pub(crate) fn expected_helix_backend_identity() -> Option<(&'static str, &'static str)> {
    EXPECTED_HELIX_IDENTITY
        .get_or_init(|| {
            let raw = include_str!("../../artifacts/helix-backend/HELIX_BACKEND_MANIFEST.json");
            let value: serde_json::Value = serde_json::from_str(raw).ok()?;
            let contract = value.get("contract")?.as_str()?.to_string();
            let tree = value.get("tree_sha256")?.as_str()?.to_string();
            if contract.is_empty()
                || tree.len() != 64
                || !tree.bytes().all(|b| b.is_ascii_digit() || matches!(b, b'a'..=b'f'))
            {
                return None;
            }
            Some((contract, tree))
        ...
        .as_ref()
        .map(|(contract, tree)| (contract.as_str(), tree.as_str()))
}

pub(crate) fn helix_backend_identity_stale_reason(
    contract: Option<&str>,
    tree_sha256: Option<&str>,
    verified: Option<bool>,
) -> Option<String> {
    let Some((expected_contract, expected_tree)) = expected_helix_backend_identity() else {
        return Some("helix_backend_bundle_identity_unavailable".to_string());
    };
    if verified != Some(true) {
        return Some("helix_backend_identity_unverified".to_string());
    }
    if contract != Some(expected_contract) {
        return Some("helix_backend_contract_mismatch".to_string());
    }
    if tree_sha256 != Some(expected_tree) {
        return Some("helix_backend_tree_mismatch".to_string());
    }
    None
}
