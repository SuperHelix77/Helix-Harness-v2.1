...
struct HealthResponse {
    ...
    helix_backend_contract: Option<String>,
    helix_backend_tree_sha256: Option<String>,
    helix_backend_verified: Option<bool>,
}
...
fn liveness_verifies_metadata(
    ...
    let alive = matches!(liveness.status.as_deref(), Some("alive") | Some("healthy"))
        && crate::desktop_service::is_studio_backend_service(liveness.service.as_deref());
...
fn lifecycle_control_block_reason(liveness: &DesktopLiveness) -> Option<String> {
    ...
    if let Some(reason) = crate::preflight::helix_backend_identity_stale_reason(
        liveness.helix_backend_contract.as_deref(),
        liveness.helix_backend_tree_sha256.as_deref(),
        liveness.helix_backend_verified,
    ) {
        return Some(reason);
    }
    None
...
fn ready_for_use_status(health: Option<&HealthResponse>) -> OwnedBackendReadiness {
    ...
    let Some(health) = health else {
        return OwnedBackendReadiness::Stale {
            reason: "helix_backend_identity_unverified".to_string(),
        };
    };
    if let Some(reason) = crate::preflight::helix_backend_identity_stale_reason(
        health.helix_backend_contract.as_deref(),
        health.helix_backend_tree_sha256.as_deref(),
        health.helix_backend_verified,
    ) {
        return OwnedBackendReadiness::Stale { reason };
    }
    match health.native_path_leases_supported {
...
mod tests {
    ...
    fn exact_health(extra_fields: &str) -> &'static str {
        let (contract, tree) =
            crate::preflight::expected_helix_backend_identity().expect("bundled Helix identity");
        Box::leak(
            format!(
                r#"{{"version":"2026.8.4","helix_backend_contract":"{contract}","helix_backend_tree_sha256":"{tree}","helix_backend_verified":true{extra_fields}}}"#
            )
            .into_boxed_str(),
        )
    }
    ...
    async fn authenticated_health_uses_login_bearer_and_accepts_ready_version() {
        let (port, seen, server) = http_sequence_server(vec![
            ...
            ("200 OK", exact_health(r#","native_path_leases_supported":true"#)),
        ])
        .await;
    ...
    async fn authenticated_health_marks_missing_or_disabled_native_leases_stale() {
        for health in [
            exact_health(""),
            exact_health(r#","native_path_leases_supported":false"#),
        ] {
    ...
    fn owned_liveness(manageability: u16) -> DesktopLiveness {
        ...
        DesktopLiveness {
            ...
            helix_backend_contract: Some(helix_contract.to_string()),
            helix_backend_tree_sha256: Some(helix_tree.to_string()),
            helix_backend_verified: Some(true),
    ...
    async fn owned_backend_that_stalls_after_the_first_request() -> u16 {
        ...
        let (helix_contract, helix_tree) =
            crate::preflight::expected_helix_backend_identity().expect("bundled Helix identity");
        let body = format!(
            r#"{{"status":"alive","service":"Unsloth UI Backend","desktop_protocol_version":{},"desktop_manageability_version":{},"supports_desktop_auth":true,"supports_desktop_backend_ownership":true,"studio_root_id":"{}","helix_backend_contract":"{}","helix_backend_tree_sha256":"{}","helix_backend_verified":true,"desktop_owner":{{"kind":"{}","token_sha256":"{}"}},"inference_active":true}}"#,
