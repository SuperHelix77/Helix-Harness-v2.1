...
def minimum_reply_tokens(context_length: int) -> int:
    """Least reply room a prompt must leave before it is worth starting generation.

    This is deliberately independent of ``max_tokens``. A large output allowance is a cap,
    not evidence that a prompt occupying 76% of the window is unsafe to serve. Callers that
    need a hard context-safety boundary should use this floor instead of ``prompt_budget``.
    """
    return max(1, context_length // _RESCUE_REPLY_FLOOR_DIVISOR)


def hard_safety_prompt_target(context_length: int) -> int:
    """Largest prompt that still leaves the minimum useful reply room.

    ``prompt_budget`` reserves the requested output allowance (up to a quarter of the
    window), which is useful for rolling-window sizing but is intentionally a softer policy
    boundary. This target answers the separate hard-safety question: can the model produce
    at least a minimally useful reply before the context wall?
    """
    if context_length <= 1:
        return context_length
    return max(1, context_length - minimum_reply_tokens(context_length))
...
def _reply_floor(context_length: int) -> int:
    ...
    return minimum_reply_tokens(context_length)
