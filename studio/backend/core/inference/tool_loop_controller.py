...
# A context-window truncation notice is the runtime's answer, not the tool's. If
# several calls with different arguments all collapse to the same notice, those
# are one dead end and should count as the same result. Ordinary tool output keeps
# the argument key so repeated acknowledgements from distinct mutations (e.g.
# three successful writes all returning "OK") remain legitimate progress.
_WINDOW_RESULT_NOTICE_MARKERS = (
    "chars for the model;",
    "no context room left",
    "No room in the window for this result",
    "an earlier timed-out call to tool",
)
MAX_IDENTICAL_TOOL_RESULTS = 3


def is_window_result_notice(result: str) -> bool:
    return any(marker in result for marker in _WINDOW_RESULT_NOTICE_MARKERS)
...
_SOURCE_MAP_TOOLS = frozenset({"search_knowledge_base", "search_conversation", "search_memory"})
...
class ToolLoopController:
    ...
    def __init__(
        ...
        *,
        ...
        self._last_result_key: tuple[str, str, str | None] | None = None
        self._identical_result_runs = 0
    ...
    def export_state(self) -> dict[str, Any]:
        """Versioned JSON state used only by a durable same-loop checkpoint."""

        return {
            "version": 1,
            "completed_one_shot_tools": sorted(self._completed_one_shot_tools),
            "successful_keys": sorted(self._successful_keys),
            "successful_equivalent_keys": dict(self._successful_equivalent_keys),
            "successful_equivalent_family_tool": dict(
                self._successful_equivalent_family_tool
            ),
            "failure_counts": dict(self._failure_counts),
            "failure_novel_at": dict(self._failure_novel_at),
            "workspace_ran": sorted(self._workspace_ran),
            "workspace_novel": self._workspace_novel,
            "workspace_novel_at": dict(self._workspace_novel_at),
            "duplicate_noop_counts": dict(self._duplicate_noop_counts),
            "history": [
                {
                    "key": item.key,
            ...
            "verification_pending": self._verification_pending,
            "verification_runs": self._verification_runs,
            "last_result_key": list(self._last_result_key) if self._last_result_key else None,
            "identical_result_runs": self._identical_result_runs,
        }

    def restore_state(self, state: Mapping[str, Any]) -> None:
        """Restore a state emitted by :meth:`export_state`, failing closed."""

        if int(state.get("version") or 0) != 1:
            raise ValueError("unsupported tool-loop controller checkpoint")
        self._completed_one_shot_tools = set(state.get("completed_one_shot_tools") or [])
        self._successful_keys = set(state.get("successful_keys") or [])
        self._successful_equivalent_keys = dict(
            state.get("successful_equivalent_keys") or {}
        )
        self._successful_equivalent_family_tool = dict(
            state.get("successful_equivalent_family_tool") or {}
        )
        self._failure_counts = {
            str(k): int(v) for k, v in dict(state.get("failure_counts") or {}).items()
        }
        self._failure_novel_at = {
            str(k): int(v) for k, v in dict(state.get("failure_novel_at") or {}).items()
        }
        self._workspace_ran = set(state.get("workspace_ran") or [])
        self._workspace_novel = int(state.get("workspace_novel") or 0)
        self._workspace_novel_at = {
            str(k): int(v) for k, v in dict(state.get("workspace_novel_at") or {}).items()
        }
        self._duplicate_noop_counts = {
            str(k): int(v)
            for k, v in dict(state.get("duplicate_noop_counts") or {}).items()
        }
        history = state.get("history") or []
        if not isinstance(history, list):
            raise ValueError("invalid tool-loop controller history")
        self._history = [
            _ToolCallRecord(
                key=str(item["key"]),
        ...
        self._executed_count = int(state.get("executed_count") or 0)
        self._successful_count = int(state.get("successful_count") or 0)
        self._failed_count = int(state.get("failed_count") or 0)
        self._suppressed_exact_duplicates = int(
            state.get("suppressed_exact_duplicates") or 0
        )
        self._suppressed_equivalent_duplicates = int(
            state.get("suppressed_equivalent_duplicates") or 0
        )
        self._suppressed_repeated_failures = int(
            state.get("suppressed_repeated_failures") or 0
        )
        self._force_final_answer = bool(state.get("force_final_answer"))
        self._verification_pending = bool(state.get("verification_pending"))
        self._verification_runs = int(state.get("verification_runs") or 0)
        last = state.get("last_result_key")
        self._last_result_key = tuple(last) if isinstance(last, list) and len(last) == 3 else None
        self._identical_result_runs = int(state.get("identical_result_runs") or 0)
    ...
    def record_result(self, decision: ToolCallDecision, result: Any) -> ToolCallCompletion:
        ...
        result_digest = hashlib.sha1(result_text.encode("utf-8", "replace")).hexdigest()
        result_key = (
            decision.tool_name,
            result_digest,
            None if is_window_result_notice(result_text) else decision.key,
        )
        if result_key == self._last_result_key:
            self._identical_result_runs += 1
        else:
            self._last_result_key = result_key
            self._identical_result_runs = 1
        if self._identical_result_runs >= MAX_IDENTICAL_TOOL_RESULTS:
            result_text = repeated_result_message(
                decision.tool_name,
                self._identical_result_runs,
                result_text,
            )
            self._identical_result_runs = 0
