...
# Proactive Mem0 handoff: compact when the live prompt occupies this share of n_ctx,
# even if it still fits prompt_budget. Hard overflow remains a second trigger.
HANDOFF_RATIO = float(os.environ.get("UNSLOTH_CONTEXT_HANDOFF_RATIO", "0.80"))
...
_SEARCHABLE = (
    ...
    "search_conversation tool or search_memory."
)
_NOT_SEARCHABLE = (
...
def occupancy_triggers_handoff(prompt_tokens: int, context_length: int) -> bool:
    """True when the live prompt already occupies the Mem0-handoff share of n_ctx."""
    if context_length <= 0 or prompt_tokens <= 0:
        return False
    return (prompt_tokens / context_length) >= HANDOFF_RATIO
...
def fit_checkpoint_context(
    ...
    *,
    ...
    # Two different thresholds answer two different questions and must not be conflated:
    #
    # * HANDOFF_RATIO is the adaptive epoch boundary. It deliberately fires at 80% of the
    #   real context window, independent of the caller's output allowance.
    # * hard_prompt_target leaves only the minimum useful reply room. Crossing it means the
    #   request is unsafe to serve without compaction even if adaptive handoff were disabled.
    #
    # ``prompt_budget`` is still useful for sizing the carried-forward block: its output
    # reserve controls how much optional historical instruction text we can afford. It no
    # longer decides WHEN a checkpoint begins.
    carried_prompt_budget = prompt_budget(context_length, max_tokens)
    hard_prompt_target = hard_safety_prompt_target(context_length)
    initial_tokens = count_tokens(list(messages))
    initial_soft_pressure = occupancy_triggers_handoff(initial_tokens, context_length)
    initial_hard_pressure = initial_tokens > hard_prompt_target
    if (
        sticky_dropped <= 0
        and not initial_soft_pressure
        and not initial_hard_pressure
    ):
        return messages, None

    budget = min(MAX_TOKENS, max(0, int(carried_prompt_budget * MAX_FRACTION)))
    ...
    # the branch still being under adaptive or hard pressure: a saved boundary describes the branch AND the window it
    # was measured against. Grow the context mid-thread and the branch falls below both thresholds, yet the boundary
    # still rides on a live assistant turn. Replaying it there would preserve a compaction the current window no longer
    # needs.
    fitted = list(messages)
    dropped = 0
    is_new_epoch = False
    if sticky_dropped > 0 and (initial_soft_pressure or initial_hard_pressure):
    ...
    # Phase two: start a new epoch on explicit adaptive pressure (80% of n_ctx) or the
    # distinct hard-safety boundary. keep_ratio 0.0 takes every evictable group in one pass;
    # the primitive itself protects system, developer, final and newest user groups.
    #
    # Do not use ``carried_prompt_budget`` here. With an omitted/large max_tokens it is 75%
    # of n_ctx, which made the nominal 80% policy silently fire around 75%.
    new_epoch_pressure = (
        occupancy_triggers_handoff(current_tokens, context_length)
        or current_tokens > hard_prompt_target
    )
    reset_blocked = False
    if new_epoch_pressure:
        ...
        # Resolve the archive/search gate only when there is actually an old turn to
        # evict. If no reset can reduce the prompt, probing storage buys nothing.
        if reset_dropped and _resolved(can_reset):
            fitted = candidate
            ...
            # `_fit_context` treats a checkpoint refusal as a request to use the rolling
            # fitter. Preserve that escape hatch when the archive/embedder is degraded:
            # adaptive pressure still gets relieved without starting an unsearchable epoch.
            reset_blocked = True

    if dropped == 0 and current_tokens <= hard_prompt_target:
        if reset_blocked:
            return messages, {
                "fits": False,
                "dropped_messages": 0,
                ...
                "prompt_target": hard_prompt_target,
                "checkpoint_reset_blocked": True,
            }
        return messages, None
    if dropped == 0:
    ...
    if current_tokens > hard_prompt_target:
