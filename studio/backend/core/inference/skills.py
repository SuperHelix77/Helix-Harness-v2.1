...
from utils.paths import account_path, ensure_dir, studio_root, workspace_root
...
def _write_new_skill_manifest(
    ...
    *,
    ...
    root = base / root
    try:
        relative_root = root.relative_to(base)
    except ValueError as exc:
        raise SkillError("Agent Skills directory is missing or unsafe.") from exc
    if any(part in {".", ".."} for part in relative_root.parts):
        raise SkillError("Agent Skills directory is missing or unsafe.")
    ancestors_list: list[Path] = []
    current = base
    for part in relative_root.parts:
        current = current / part
        ancestors_list.append(current)
    ancestors = tuple(ancestors_list)
    if not ancestors:
        raise SkillError("Agent Skills directory is missing or unsafe.")
...
def _validate_skill_dir(
    skill_dir: Path,
    raw_capture: Optional[list[bytes]] = None,
) -> tuple[dict, Path]:
    ...
    raw = _read_limited(manifest, MAX_SKILL_MD_BYTES, contained_in = target)
    if raw_capture is not None:
        raw_capture.append(raw)
    metadata = _parse_skill_markdown(raw, skill_dir.name)
...
def _default_enabled(source: str, name: str = "") -> bool:
    # Owner chats need skill-creator in preflight so the model can inspect and create skills.
    if name == "skill-creator":
        return is_owner_context()
    # Other bundled skills stay off until the user enables them.
...
def _discover(
    home: Optional[Path],
    *,
    manifest_capture: Optional[dict[Path, bytes]] = None,
) -> list[tuple[dict, Optional[Path], Optional[os.stat_result]]]:
    ...
    for source, root in _skill_roots(home):
        ...
        for candidate in candidates:
            ...
            try:
                raw_capture: list[bytes] | None = [] if manifest_capture is not None else None
                metadata, skill_dir = _validate_skill_dir(candidate, raw_capture = raw_capture)
                identity = os.stat(skill_dir, follow_symlinks = False)
                if manifest_capture is not None and raw_capture:
                    manifest_capture[skill_dir] = raw_capture[0]
            except OSError:
                found.append(
            ...
            record = {
                **base,
                **metadata,
                "enabled": overrides.get(name, _default_enabled(source, name)),
                "valid": True,
            }
...
def _selected_skill(
    ...
    *,
    ...
    normalized = _normalize_skill_name(name)
    for record, skill_dir, identity in _discover(home, manifest_capture = manifest_capture):
        if record["valid"] and not record["shadowed"] and record["name"] == normalized:
            return record, skill_dir, identity
    if thread_id:
        for record in list_temp_skills(thread_id):
            if record.get("name") == normalized:
                return record, Path(record["path"]).parent, None
...
def set_skill_enabled(
    ...
    *,
    ...
    with _LOCK:
        ...
        overrides = _load_overrides()
        if enabled == _default_enabled(record["source"], record["name"]):
    ...
    *,
    ...
    if temporary:
        if not thread_id or not str(thread_id).strip():
            raise SkillError("Temporary skills need a thread id.")
        return _write_temp_skill(
            normalized,
            description.strip(),
            instructions.strip(),
...
def learn_skill(
    ...
    *,
    ...
    """Create an ephemeral skill for the current chat thread.

    This is deliberately narrower than :func:`create_skill`: callers cannot
    promote the write to a durable Agent Skill by changing an argument. The
    learned procedure stays in the thread-local temporary skill root until a
    separate promotion step explicitly makes it durable.
    """
    if not thread_id or not str(thread_id).strip():
        raise SkillError("learn_skill needs a thread id so the skill can stay thread-scoped.")
    return create_skill(
        name,
        description,
        instructions,
...
def match_skills(query: str, skills: Optional[list[dict]] = None, *, limit: int = 3) -> list[dict]:
    """Pick enabled skills whose name/description overlap the user request."""
    tokens = set(re.findall(r"[a-z0-9]{3,}", str(query or "").lower()))
    compact_query = re.sub(r"[^a-z0-9]+", "", str(query or "").lower())
    scored: list[tuple[int, dict]] = []
    for skill in skills or []:
        if not skill.get("enabled") or not skill.get("valid"):
            continue
        name = str(skill.get("name") or "")
        hay = f"{name} {skill.get('description') or ''}".lower()
        hay_tokens = set(re.findall(r"[a-z0-9]{3,}", hay))
        score = len(tokens & hay_tokens)
        if name and name.replace("-", "") in compact_query:
            score += 5
        if score:
            scored.append((score, skill))
    scored.sort(key = lambda item: (-item[0], str(item[1].get("name") or "")))
    return [skill for _, skill in scored[: max(1, min(int(limit), 6))]]


def skill_preflight_instruction(
    ...
    *,
    ...
    """System block: look at existing skills, use matches, create only when needed."""
    relevant = match_skills(user_text, skills) if user_text else []
    lines = [
        "<skill_preflight>",
        "DeepSeek-style skill preflight: inspect the catalog first, plan which skills this task needs, then act. If one matches, call read_skill for it and follow those instructions. Do not re-invent a skill you already have. If the latest user message mentions an enabled skill as @skill-name, call read_skill for that named skill before answering.",
    ]
    if relevant:
        names = ", ".join(str(item.get("name") or "") for item in relevant if item.get("name"))
        lines.append(f"Likely relevant now: {names}. Call read_skill on these first.")
    if catalog.strip():
        lines.append("Enabled skills:")
        lines.append(catalog.strip())
    if can_learn:
        lines.append(
            "If this task exposes a repeatable workflow and no existing skill covers it, call "
            "learn_skill with a concise name, description, and complete reusable instructions. "
            "learn_skill always creates a temporary skill scoped to this thread; after it succeeds, "
            "call read_skill for that name and follow it immediately."
        )
    elif can_create:
        lines.append(
            "If this task is a repeatable workflow and no existing skill covers it, read skill-creator, then create_skill with temporary=true for this thread. After creating it, call read_skill and follow it immediately. Promote it later only if it proves useful. Never skip this preflight to jump to QLoRA."
        )
    lines.append("</skill_preflight>")
    return "\n".join(lines)
...
def _safe_thread_id(thread_id: str) -> str:
    ...
    if raw and raw == cleaned and len(raw) <= 80:
        return raw
    digest = hashlib.sha256(raw.encode("utf-8")).hexdigest()[:12]
    prefix = cleaned[: max(1, 80 - len(digest) - 1)] or "thread"
    return f"{prefix}-{digest}"


def _temp_skills_root(thread_id: str) -> Path:
    path = account_path(f"learning/temp-skills/{_safe_thread_id(thread_id)}")
    ensure_dir(path)
    return path


def _write_temp_skill(name: str, description: str, instructions: str, *, thread_id: str) -> dict:
    root = _temp_skills_root(thread_id)
    destination = root / name
    if destination.exists():
        raise SkillError(f"Skill '{name}' already exists.")
    frontmatter = yaml.safe_dump(
        {"name": name, "description": description, "temporary": True, "thread_id": thread_id},
        allow_unicode = True,
        sort_keys = False,
    )
...
def list_temp_skills(thread_id: str) -> list[dict]:
    if not thread_id:
        return []
    root = _temp_skills_root(thread_id)
    records = []
    ...
    for child in children:
        ...
        try:
            ...
            metadata = _parse_skill_markdown(text.encode("utf-8"), child.name)
        except Exception:
            continue
        records.append(
            {
...
def discard_temp_skill(name: str, *, thread_id: str) -> dict:
    """Discard one temporary skill from exactly one chat thread.

    Temporary skills created by this module contain only ``SKILL.md``.  Delete
    that marker directly and remove the directory only when it is empty instead
    of recursively deleting an arbitrary tree.  This keeps a tampered symlink or
    unexpected resource from turning retrospective cleanup into a broader file
    deletion primitive.
    """
    normalized = _normalize_skill_name(name)
    if not thread_id or not str(thread_id).strip():
        raise SkillError("Temporary skill discard needs a thread id.")
    thread = str(thread_id)
    root = _temp_skills_root(thread)
    destination = root / normalized
    marker = destination / "SKILL.md"
    with _LOCK:
        if destination.is_symlink() or marker.is_symlink():
            raise SkillError(f"Temporary skill '{normalized}' is missing or unsafe.")
        if not destination.is_dir() or not marker.is_file():
            raise SkillNotFoundError(f"Skill '{normalized}' was not found.")
        try:
            marker.unlink()
        except FileNotFoundError as exc:
            raise SkillNotFoundError(f"Skill '{normalized}' was not found.") from exc
        except OSError as exc:
            raise SkillError(f"Could not discard temporary skill '{normalized}'.") from exc
        try:
            destination.rmdir()
        except OSError as exc:
            # A temporary skill created here has no resources.  If the directory
            # was tampered with, leave unexpected files in place; removing the
            # manifest is sufficient to make the skill undiscoverable.
            if exc.errno not in (errno.ENOTEMPTY, errno.EEXIST):
                raise SkillError(f"Could not finish discarding temporary skill '{normalized}'.") from exc
    return {
        "name": normalized,
        "temporary": True,
        "thread_id": thread,
...
def promote_temp_skill(name: str, *, thread_id: str, home: Optional[Path] = None) -> dict:
    normalized = _normalize_skill_name(name)
    temps = [item for item in list_temp_skills(thread_id) if item.get("name") == normalized]
    if not temps:
        raise SkillNotFoundError(f"Skill '{normalized}' was not found.")
    path = Path(temps[0]["path"])
    ...
    record = create_skill(
        normalized,
        str(temps[0].get("description") or normalized),
        body,
        home = home,
        temporary = False,
    )
...
def read_skill_resource(
    ...
    *,
    ...
    with _LOCK:
        if thread_id:
            record, skill_dir, identity = _selected_skill(
                name, home = home, thread_id = thread_id
            )
        else:
...
def _read_skill_document(
    ...
    *,
    ...
    """Read one bounded skill resource under one resolver lock.

    The route-level skill reader returns the complete document, so it must not
    perform independent page reads that can observe a replacement between pages.
    This helper keeps selection, containment checks, descriptor-relative opening,
    and the full bounded read in one critical section.
    """
    with _LOCK:
        manifest_capture: dict[Path, bytes] = {}
        if thread_id:
            record, skill_dir, identity = _selected_skill(
                name,
                home = home,
            ...
            record, skill_dir, identity = _selected_skill(
                name, home = home, manifest_capture = manifest_capture
            )
        if not record["enabled"]:
            raise SkillError(f"Skill '{record['name']}' is disabled.")
        path = _normalize_resource_path(resource)
        ...
        if path.as_posix() == "SKILL.md":
            raw = manifest_capture.get(skill_dir)
            if raw is None:
                raise SkillError("Skill manifest could not be read safely.")
        else:
            raw = _read_limited(
                candidate, MAX_SKILL_FILE_BYTES, contained_in = skill_dir, identity = identity
            )
