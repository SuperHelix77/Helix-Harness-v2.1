...
from typing import TYPE_CHECKING, Optional, Union, Generator, Tuple
...
class InferenceBackend:
    ...
    def generate_chat_completion_with_tools(
        ...
        adaptive_context_ratio: Optional[float] = None,
    ...
    def generate_stream(
        ...
        try:
            ...
            if adaptive_context_ratio is not None:
                from core.inference.adaptive_context import adaptive_segment_plan
                from core.inference.runtime_context import runtime_context_length

                _adaptive_plan = adaptive_segment_plan(
                    prompt_tokens = prompt_len,
                    context_length = runtime_context_length(model),
    ...
    def _record_generation_stats(
        ...
        *,
        ...
        truncated = (
            ...
            and (
                (bool(max_new_tokens) and completion_tokens >= int(max_new_tokens))
                or (
                    adaptive_plan is not None
                    and adaptive_plan.adaptive_limited
                    and adaptive_plan.effective_max_tokens <= 0
                )
            )
        )
        stats = {
        ...
        if adaptive_plan is not None:
            ...
            checkpoint = adaptive_checkpoint_metadata(
                adaptive_plan,
                finish_reason = "length" if truncated else "stop",
                completion_tokens = completion_tokens,
            )
            if checkpoint is not None:
                stats["adaptive_checkpoint"] = checkpoint
