...
def _build_generation_stats(
    ...
    if adaptive_checkpoint is not None:
        stats["adaptive_checkpoint"] = adaptive_checkpoint
    return stats
...
class MLXInferenceBackend:
    ...
    def generate_chat_response(
        ...
        if self._is_vlm:
            stream = self._generate_vlm(
                ...
                video = video,
                adaptive_context_ratio = adaptive_context_ratio,
            )
        else:
    ...
    def _generate_text(
        ...
        *,
        ...
        with (
            ...
            _adaptive_plan = None
            if adaptive_context_ratio is not None:
                ...
                _adaptive_plan = adaptive_segment_plan(
                    prompt_tokens = _adaptive_prompt_n,
                    context_length = self._served_context,
                    requested_max_tokens = _requested_max_new_tokens,
                    ratio = adaptive_context_ratio,
                )
                if _adaptive_plan is not None and _adaptive_plan.adaptive_limited:
                    if _adaptive_plan.effective_max_tokens <= 0:
                        self.last_generation_stats = _build_generation_stats(
                            max(_adaptive_prompt_n - cached_n, 0),
                            0.0,
                            0,
                            0.0,
                            cached_n,
                            finish_reason = "length",
                            adaptive_checkpoint = adaptive_checkpoint_metadata(
                                _adaptive_plan,
                                finish_reason = "length",
                                completion_tokens = 0,
                            ),
                    ...
                    max_new_tokens = min(
                        int(max_new_tokens),
                        _adaptive_plan.effective_max_tokens,
                    )
            logger.info(
            ...
            try:
                ...
                if final_response is not None:
                    _finish_reason = _mlx_finish_reason(
                        final_response,
                        _mlx_stop_token_ids(self._tokenizer, self._model),
                        getattr(final_response, "generation_tokens", 0),
                        max_new_tokens,
                    )
                    _adaptive_checkpoint = None
                    if _adaptive_plan is not None:
                        from core.inference.adaptive_context import adaptive_checkpoint_metadata

                        _adaptive_checkpoint = adaptive_checkpoint_metadata(
                            _adaptive_plan,
                            finish_reason = _finish_reason,
                            completion_tokens = getattr(final_response, "generation_tokens", 0),
                        )
                    self.last_generation_stats = _build_generation_stats(
                        ...
                        adaptive_checkpoint = _adaptive_checkpoint,
                    )
    ...
    def _generate_vlm(
        ...
        *,
        ...
        _adaptive_plan = None
        if (
            adaptive_context_ratio is not None
            and image is None
            and video is None
        ):
            from core.inference.adaptive_context import (
            ...
            _adaptive_prompt_n = self._count_prompt_tokens(prompt)
            _adaptive_plan = adaptive_segment_plan(
                prompt_tokens = _adaptive_prompt_n,
                context_length = self._served_context,
                requested_max_tokens = _requested_max_new_tokens,
                    ...
                    for response in vlm_stream(
                        ...
                        **vlm_kwargs,
                        ...
                        if vlm_token_decoder is not None and token_id is not None:
                            # Only a special id is re-decoded; ordinary ids keep mlx-vlm's text. mlx-vlm finalizes its
                            # streaming detokenizer on the terminal stop result, however, so response.text on that one
                            # can contain buffered text from PRECEDING ids. Qwen3.8 does exactly this for
                            # ``</think>\n\n<tool_call>...``: replacing that non-empty flush with the decoded EOS token
                            # erases the whole call before the tool parser sees it.
                            _buffered_text = token_text
                            _decoded = vlm_token_decoder.decode_stream_token(token_id, token_text)
                            if int(token_id) in vlm_stop_ids:
                                # The stop id itself is framing and normally stays hidden. Preserve any detokenizer
                                # flush attached to this result; it does not contain the stop id because mlx-vlm checks
                                # stopping_criteria before adding that id to the detokenizer. A stop id that is itself a
                                # required protocol closer is appended after the flush so strict parsing still sees it.
                                token_text = _buffered_text
                                if _decoded and closes_an_open_envelope(
                                    sampled + _buffered_text + _decoded, _decoded
                                ):
                                    token_text += _decoded
                            else:
                                token_text = _decoded
                        ...
                        if _adaptive_plan is not None:
                            ...
                            _adaptive_checkpoint = adaptive_checkpoint_metadata(
                                ...
                                completion_tokens = getattr(
                                    final_response, "generation_tokens", 0
                                ),
                            )
                        self.last_generation_stats = _build_generation_stats(
                            prefilled_n,
