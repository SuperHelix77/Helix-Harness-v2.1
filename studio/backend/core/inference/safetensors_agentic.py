...
if TYPE_CHECKING:
    from core.helix_engine.optimization_trace import OptimizationTraceRecorder
...
def _user_visible_answer_text(text: str, *, reasoning_prefilled: bool = False) -> str:
    """Return only text that can reach the user-visible answer channel.

    Unlike :func:`_reprompt_intent_text`, this never falls back to private
    reasoning.  Trace semantics therefore cannot count a reasoning-only model
    segment as a completed final answer.
    """
    if reasoning_prefilled:
        close = _THINK_CLOSE_RE.search(text)
        if close is None:
...
class _InvocationTraceSlot:
    """Own the at-most-one live model invocation in one tool-loop generator."""

    def __init__(self, recorder: "OptimizationTraceRecorder") -> None:
    ...
    def _invalidate(self, reason: str) -> None:
        ...
        try:
            ...
            logger.exception("optimization_trace.invalidation_failed")

    def begin(self) -> None:
        if not self._usable:
            return
        if self._active is not None:
            self._invalidate("trace_begin_failed")
            return
        try:
            self._active = self._recorder.begin_invocation()
        except Exception:
            logger.exception("optimization_trace.begin_failed")
            self._invalidate("trace_begin_failed")

    def _finish(self, method: str, *args):
        if self._active is None:
            return None
        handle = self._active
        # Clear first so a failed finalizer is never retried from generator
        # cleanup and can never turn a successful run into an exception.
        self._active = None
        try:
            return getattr(handle, method)(*args)
        except Exception:
            logger.exception("optimization_trace.finalize_failed")
            self._invalidate("trace_finalize_failed")
            return None

    def complete_final_answer(self) -> None:
        self._finish("complete_final_answer")

    def complete_action_plan(self) -> None:
        self._finish("complete_action_plan")

    def complete_wasted(self, reason: str) -> None:
        self._finish("complete_wasted", reason)

    def complete_continuation(self) -> None:
        self._finish("complete_continuation")

    def fail(self) -> None:
        self._finish("fail", "model_generator_exception")

    def cancel(self) -> None:
        self._finish("cancel", "cooperative_cancellation")

    def discard_active(self) -> None:
        self._finish("discard", "generator_closed_before_classification")
...
def _run_safetensors_tool_loop_impl(
    *,
    ...
    _invocation_trace_slot: Optional[_InvocationTraceSlot] = None,
    ...
    * ``{"type": "tool_end", "tool_name", "tool_call_id", "result"}``
    ...
    from core.inference.durable_tool_journal import current_resume_checkpoint

    _durable_resume_checkpoint = current_resume_checkpoint(backend="safetensors")
    ...
    ) or bool(continue_final_message and trailing_assistant_text(conversation)) or bool(
        _durable_resume_checkpoint
    )
    ...
    _start_iteration = 0
    if _durable_resume_checkpoint is not None:
        from core.inference.durable_tool_journal import resume_checkpoint_calls

        stored_conversation = _durable_resume_checkpoint.get("conversation")
        if not isinstance(stored_conversation, list):
            raise RuntimeError("durable safetensors checkpoint has no conversation")
        conversation = list(stored_conversation)
        kb_search_count = int(_durable_resume_checkpoint.get("kb_search_count") or 0)
        final_attempt_done = bool(
            _durable_resume_checkpoint.get("final_attempt_done", False)
        )
        next_call_id = int(_durable_resume_checkpoint.get("next_call_id") or 0)
        reprompt_count = int(_durable_resume_checkpoint.get("reprompt_count") or 0)
        last_reprompt_text = str(
            _durable_resume_checkpoint.get("last_reprompt_text") or ""
        )
        tool_denied = bool(_durable_resume_checkpoint.get("tool_denied", False))
        _executed_tool_iters = int(
            _durable_resume_checkpoint.get("executed_tool_iters") or 0
        )
        resumed = yield from resume_checkpoint_calls(
            _durable_resume_checkpoint,
            backend="safetensors",
        ...
        if int(resumed.get("executed") or 0) > 0:
            _executed_tool_iters += 1
        tool_denied = tool_denied or bool(resumed.get("denied"))
        _start_iteration = int(_durable_resume_checkpoint.get("iteration") or 0) + 1
    ...
    for iteration in range(_start_iteration, max_tool_iterations + _extra_iters + 1):
        ...
        if _invocation_trace_slot is not None:
            _invocation_trace_slot.begin()
        try:
            gen = _call_single_turn(single_turn, conversation, active_tools, tool_protocol_active)
            _gen_iter = iter(gen)
        except Exception:
            if _invocation_trace_slot is not None:
                _invocation_trace_slot.fail()
            raise
        prev_cumulative = ""
        ...
        _turn_stats = (generation_stats_holder or {}).get("stats")
        _retained_continuation = bool(
            isinstance(_turn_stats, dict)
            and (
                _turn_stats.get("truncated") is True
                or isinstance(_turn_stats.get("adaptive_checkpoint"), dict)
            )
        )
        ...
        if detect_state == _state_streaming:
            ...
            if not safety_tc:
                # A capped segment is a resumable text boundary only after the
                # safety parser confirms that no complete tool call finished on
                # the same token. Complete calls continue into execution below.
                if _retained_continuation:
                    if _invocation_trace_slot is not None:
                        _invocation_trace_slot.complete_continuation()
                    yield {"type": "status", "text": ""}
                    return
                ...
                if (
                    ...
                    if _invocation_trace_slot is not None:
                        _invocation_trace_slot.complete_wasted("plan_without_action")
                    reprompt_count += 1
                ...
                if _invocation_trace_slot is not None:
                    if _user_visible_answer_text(
                        content_accum,
                        reasoning_prefilled=reasoning_prefilled,
                    ):
                        _invocation_trace_slot.complete_final_answer()
                    else:
                        _invocation_trace_slot.complete_wasted("empty_model_output")
            ...
            if not tool_calls:
                if _invocation_trace_slot is not None:
                    _invocation_trace_slot.complete_wasted("malformed_or_unparseable_output")
        ...
        if final_attempt_done:
            if _invocation_trace_slot is not None:
                _invocation_trace_slot.complete_wasted("tool_output_suppressed_on_final_attempt")
            if content_text:
        ...
        for _call_index, tc in enumerate(tool_calls or []):
            ...
            if _invocation_trace_slot is not None:
                # Parsing, controller policy, approval and execution are deterministic
                # suboperations. The accepted proposal is the model's semantic work.
                _invocation_trace_slot.complete_action_plan()
            ...
            from core.inference.durable_tool_journal import (
                freeze_pre_tool_checkpoint,
                proposal_envelope,
            )

            _checkpoint, _checkpoint_digest = freeze_pre_tool_checkpoint(
                {
                    "version": 1,
                    "backend": "safetensors",
                    "conversation": conversation,
                    ...
                    "remaining_calls": list(tool_calls or [])[_call_index + 1 :],
                    "controller": tool_controller.export_state(),
                    "iteration": iteration,
                    "executed_tool_iters": _executed_tool_iters,
                    "kb_search_count": kb_search_count,
                    "reprompt_count": reprompt_count,
                    "last_reprompt_text": last_reprompt_text,
                    "final_attempt_done": final_attempt_done,
            ...
            if needs_confirm:
                ...
                if _private is not None:
                    start_event["_durable_tool_approval"] = _private
            ...
            from core.inference.durable_tool_journal import settle_controller_completion

            completion = settle_controller_completion(result, decision, tool_controller)
            if provisional_match:
                provisional_resolved = True
        ...
        if _invocation_trace_slot is not None:
            # Reaching here with a live handle means every parsed proposal was
            # suppressed by the controller and no semantic action was accepted.
            _invocation_trace_slot.complete_wasted("controller_suppressed_output")
...
def run_safetensors_tool_loop(
    *,
    ...
    generation_stats_holder: Optional[dict] = None,
    helix_turn_id: Optional[str] = None,
    invocation_recorder: Optional["OptimizationTraceRecorder"] = None,
) -> Generator[dict, None, None]:
    """Run the safetensors tool loop with optional explicit invocation tracing.

    With no ``invocation_recorder`` this is a transparent delegation to the
    existing generator.  The wrapper's ``finally`` guarantees that closing the
    consumer early cannot leave a begun model invocation unrecorded.
    """
    trace_slot = (
        _InvocationTraceSlot(invocation_recorder)
        if invocation_recorder is not None
        else None
    )
    try:
        yield from _run_safetensors_tool_loop_impl(
