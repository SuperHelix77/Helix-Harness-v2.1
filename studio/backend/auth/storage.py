...
# ObservationPack intentionally has its own installation key.  In particular,
# it must never be an alias for the credential key above: credentials and
# tool-output evidence have different rotation and disclosure boundaries.
_OBSERVATION_MASTER_KEY_DB_KEY = "observation_master_key_v1"
_observation_master_key_cache: Optional[bytes] = None
_observation_master_key_cache_path: Optional[str] = None


class ObservationMasterKeyError(RuntimeError):
    """The protected ObservationPack master-key record is malformed."""


def _observation_key_id(secret: bytes) -> str:
    """Return an opaque key id that fingerprints the actual key material.

    A key id is recorded with each binding.  Including the complete digest is
    intentional: recreating auth.db must not accidentally produce a key with
    the same externally recorded id.
    """
    return "observation-key-v1:sha256:" + hashlib.sha256(secret).hexdigest()


def _observation_secret_from_row(value: object) -> bytes:
    if not isinstance(value, str):
        raise ObservationMasterKeyError("Observation master key is not text encoded")
    try:
        secret = bytes.fromhex(value)
    except ValueError as exc:
        raise ObservationMasterKeyError("Observation master key is not hexadecimal") from exc
    if len(secret) != 32:
        raise ObservationMasterKeyError("Observation master key must be 32 bytes")
    return secret


def _load_observation_master_key(*, create: bool) -> Optional[bytes]:
    global _observation_master_key_cache, _observation_master_key_cache_path

    db_key = str(DB_PATH)
    # A test, key-rotation path, or account-reset path can replace the protected
    # row without changing DB_PATH.  Do not trust an in-process cache as key
    # authority: every access re-reads and validates the auth.db row.  The
    # cache remains only as diagnostic state for tests and path transitions.
    if _observation_master_key_cache_path != db_key:
        _observation_master_key_cache = None

    conn = get_connection()
    try:
        row = conn.execute(
            "SELECT value FROM app_secrets WHERE key = ?",
            (_OBSERVATION_MASTER_KEY_DB_KEY,),
        ).fetchone()
        if row is None and create:
            # INSERT OR IGNORE makes two first callers converge on one key; the
            # value is read back rather than trusting the losing candidate.
            conn.execute(
                "INSERT OR IGNORE INTO app_secrets (key, value) VALUES (?, ?)",
                (_OBSERVATION_MASTER_KEY_DB_KEY, secrets.token_hex(32)),
            )
            conn.commit()
            row = conn.execute(
                "SELECT value FROM app_secrets WHERE key = ?",
        ...
        if row is None:
            return None
        secret = _observation_secret_from_row(row["value"])
    finally:
        conn.close()

    _observation_master_key_cache = secret
    _observation_master_key_cache_path = db_key
    return secret


def get_or_create_observation_master_key() -> bytes:
    """Return the dedicated 256-bit ObservationPack key from protected auth.db."""
    secret = _load_observation_master_key(create = True)
    if secret is None:  # pragma: no cover - ``create=True`` guarantees a row
        raise ObservationMasterKeyError("Observation master key could not be created")
    return secret


def get_observation_master_key(*, refresh: bool = False) -> Optional[bytes]:
    """Read the existing ObservationPack key without silently recreating it.

    Recall uses this form so a missing/recreated auth.db is reported as
    ``missing_key`` instead of decrypting old ciphertext under a new key.
    """
    if refresh:
        global _observation_master_key_cache
        _observation_master_key_cache = None
    return _load_observation_master_key(create = False)


def get_observation_master_key_id() -> str:
    return _observation_key_id(get_or_create_observation_master_key())


def observation_master_key_id_for(secret: bytes) -> str:
    """Return the persisted-id format for tests and authenticated callers."""
    if not isinstance(secret, bytes) or len(secret) != 32:
        raise ValueError("Observation master key must be 32 bytes")
    return _observation_key_id(secret)
