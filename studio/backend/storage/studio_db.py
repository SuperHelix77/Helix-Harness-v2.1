...
def _ensure_schema(conn: sqlite3.Connection) -> None:
    ...
    conn.execute(
        ...
        CREATE TABLE IF NOT EXISTS chat_generation_runs (
            ...
            started_at INTEGER,
            completed_at INTEGER,
            observation_pack_version INTEGER NOT NULL DEFAULT 0
                CHECK (observation_pack_version IN (0, 1)),
    ...
    if "observation_pack_version" not in chat_generation_run_cols:
        # Additive and backwards-compatible: old durable runs remain on the
        # ordinary bounded-result path.  The CHECK is a database backstop;
        # create_run validates the backend-owned pin before it reaches SQLite.
        conn.execute(
            """ALTER TABLE chat_generation_runs
               ADD COLUMN observation_pack_version INTEGER NOT NULL DEFAULT 0
               CHECK (observation_pack_version IN (0, 1))"""
        )
    invalid_observation_pins = conn.execute(
        """SELECT id FROM chat_generation_runs
           WHERE observation_pack_version NOT IN (0, 1)
              OR observation_pack_version IS NULL
           LIMIT 1"""
    ).fetchone()
    if invalid_observation_pins is not None:
        raise sqlite3.DatabaseError(
            "chat_generation_runs contains an invalid observation pack version"
        )
    ...
    conn.execute(
        "CREATE INDEX IF NOT EXISTS idx_chat_generation_tool_approvals_state "
        "ON chat_generation_tool_approvals(run_id, status, execution_state, expires_at)"
    )
    conn.execute(
        """
    ...
    conn.execute(
        """
        CREATE TABLE IF NOT EXISTS chat_generation_observation_bindings (
            ...
            card_call_id TEXT NOT NULL DEFAULT '',
            approval_id TEXT,
            authority_kind TEXT NOT NULL CHECK(authority_kind IN ('approved', 'ungated')),
            payload_json TEXT NOT NULL,
    ...
    conn.execute(
        """CREATE INDEX IF NOT EXISTS idx_chat_generation_observation_bindings_read
           ON chat_generation_observation_bindings(
               account_id, thread_id, plaintext_digest, state, created_at
           )"""
    )
    conn.execute(
        """CREATE INDEX IF NOT EXISTS idx_chat_generation_observation_bindings_execution
           ON chat_generation_observation_bindings(run_id, execution_id)"""
    )
    conn.execute(
        "CREATE INDEX IF NOT EXISTS idx_chat_generation_tool_executions_state "
        "ON chat_generation_tool_executions(run_id, execution_state, claimed_at)"
    )
    conn.execute(
...
def _assistant_placeholder_is_empty(content) -> bool:
    if content in (None, [], ""):
        return True
    if isinstance(content, list):
        if not content:
...
def delete_orphan_assistant_placeholder(thread_id: str, message_id: str) -> bool:
    """Delete an empty assistant row with no children and no active generation run."""
    from storage.chat_generation_runs_db import list_active as list_active_runs

    message = get_chat_message(thread_id, message_id)
    if message is None or message.get("role") != "assistant":
        return False
    if not _assistant_placeholder_is_empty(message.get("content")):
        return False
    if list_active_runs(thread_id):
        return False
    conn = get_connection()
    try:
        conn.execute("BEGIN IMMEDIATE")
        child = conn.execute(
            "SELECT 1 FROM chat_messages WHERE thread_id = ? AND parent_id = ? LIMIT 1",
            (thread_id, message_id),
        ).fetchone()
        ...
        deleted = conn.execute(
            "DELETE FROM chat_messages WHERE thread_id = ? AND id = ? AND role = 'assistant'",
            (thread_id, message_id),
        ).rowcount
        if deleted:
            _recompute_chat_thread_updated_at(conn, thread_id)
