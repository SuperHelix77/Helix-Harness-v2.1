...
def account_tool_stream(stream):
    ...
    def scoped(invoke, *args, **kwargs):
        # Preserve the wrapped callable's signature.  stream_tool_execution uses
        # inspect.signature() to decide whether it may inject its private
        # tool_cancel_event; functools.partial(run_as, ...) advertises **kwargs
        # from run_as and therefore falsely claims that every legacy/fake tool
        # accepts that keyword.
        @wraps(invoke)
        def scoped_invoke(*invoke_args, **invoke_kwargs):
            return run_as(account, invoke, *invoke_args, **invoke_kwargs)

        # A generator function does not execute when it is created.  Binding only
        # scoped_invoke therefore leaves stream-level account state (for example
        # the timed-out-tool registry key and the context copied into its worker)
        # dependent on whichever account happens to iterate the generator later.
        # Drive every step under the account captured above so a suspended stream
        # cannot migrate between account namespaces.
        inner = run_as(account, stream, scoped_invoke, *args, **kwargs)

        def scoped_iteration():
            try:
                while True:
                    try:
                        item = run_as(account, next, inner)
                ...
                if callable(close):
                    run_as(account, close)

        return scoped_iteration()
