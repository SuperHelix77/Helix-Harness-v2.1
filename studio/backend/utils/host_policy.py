...
def cors_origins_for_mode(*, api_only: bool, secure: bool) -> list[str]:
    ...
    API-only mode is always restricted to known local desktop/dev origins.
    RemoteAccessCORSMiddleware adds only the currently published tunnel origin
    at runtime; a tunnel must never admit arbitrary third-party origins.
    """
    if api_only:
