...
def _purge_account_runtime_state(account_id: str) -> None:
    """Best-effort cleanup for bounded, optional in-process account telemetry."""
    for module_name in (
        "core.helix_engine.capture",
        "core.inference.computer_browse",
        "core.helix_event_sequence",
    ):
        try:
            module = __import__(module_name, fromlist = ["purge_account"])
            purge = getattr(module, "purge_account", None)
            if purge is not None:
                purge(account_id)
        except Exception:  # noqa: BLE001 -- telemetry cleanup must not block retirement
            logger.warning("Could not purge optional runtime state for account %s", account_id, exc_info = True)
...
def retire_account_roots(account: AccountContext):
    ...
    _purge_account_runtime_state(account.account_id)
...
def set_account_active(account_id: str, payload: AccountActiveRequest):
    with _account_errors():
        ...
        if payload.is_active:
            ...
            _purge_account_runtime_state(account_id)
        return result
