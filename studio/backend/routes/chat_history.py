...
from storage.studio_db import (
    ...
    delete_orphan_assistant_placeholder,
...
class ChatGoalState(BaseModel):
    ...
    startedAt: Optional[Annotated[int, Field(ge = 1)]] = None
    elapsedMs: Optional[Annotated[int, Field(ge = 0)]] = None
...
def delete_orphan_assistant_message(
    thread_id: str,
    message_id: str,
    current_subject: str = Depends(get_current_subject),
):
    """Delete an empty assistant placeholder that never started generation."""
    if get_chat_thread(thread_id) is None:
        raise HTTPException(status_code = 404, detail = f"Thread {thread_id} not found")
    message = get_chat_message(thread_id, message_id)
    if message is None:
    ...
    if message.get("role") != "assistant":
        raise HTTPException(status_code = 400, detail = "Only empty assistant placeholders can be deleted")
    deleted = delete_orphan_assistant_placeholder(thread_id, message_id)
    if not deleted:
        raise HTTPException(
            status_code = 409,
            detail = "Message is not an empty assistant placeholder or the thread has an active run",
        )
