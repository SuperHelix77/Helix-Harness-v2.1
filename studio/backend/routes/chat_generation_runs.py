...
# Durable optimization evidence is for server-side consumers only. It remains in
# the event ledger so the cursor can pass it, but is never replayed to the frontend.
_PRIVATE_REPLAY_EVENT_TYPES = frozenset({"optimization.trace"})
    ...
    async def stream():
        ...
        while True:
            ...
            for event in events:
                ...
                if event["type"] in _PRIVATE_REPLAY_EVENT_TYPES:
                    # Advance past private durable evidence without exposing it
                    # or allowing a terminal private tail to keep reconnecting.
                    continue
                data = {
                    "seq": cursor,
                    "type": event["type"],
