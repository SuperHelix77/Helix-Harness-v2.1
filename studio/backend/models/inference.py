...
class ChatCompletionRequest(BaseModel):
    ...
    adaptive_context_ratio: Optional[float] = Field(
        None,
        gt = 0.0,
        lt = 1.0,
        description = (
            "[x-unsloth] Stop the current local generation segment when exact prompt plus "
            "generated-token occupancy reaches this share of the served context window. "
            "The public finish_reason remains 'length'; Studio receives separate adaptive "
    ...
    finalization_idempotency_key: Optional[str] = Field(
        None,
        max_length = 240,
        description = (
            "[x-unsloth] Stable logical-turn key used by Studio's durable post-answer "
            "finalizer. It is inert for ordinary OpenAI-compatible callers."
        ),
    )
