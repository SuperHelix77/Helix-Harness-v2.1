...
def _helix_runtime_identity() -> dict[str, Any]:
    """Verified identity of the backend overlay this process actually loaded."""
    try:
        from state.helix_build_identity import helix_backend_identity

        return helix_backend_identity()
...
from routes.helix_engine import router as helix_engine_router
...
async def lifespan(app: FastAPI):
    ...
    _recovered_durable_chat_runs: list[tuple[object, dict]] = []
    _deferred_durable_chat_plans: list[tuple[object, list[object]]] = []
    for _account in _reconcile_accounts:
        ...
        _preserve_chat_run_ids: set[str] = set()
        try:
            from core.inference.durable_agent_recovery import (
                plan_orphaned_runs,
                requeue_planned_runs,
            )
            from core.inference.chat_generation_runs import recovery_stale_before_ms

            _plans = _run_as(_account, plan_orphaned_runs)
            _stale_before = recovery_stale_before_ms()
            _recovered = (
                _run_as(
                    _account,
                    requeue_planned_runs,
                    _plans,
                    stale_before_ms=_stale_before,
                )
                if _stale_before is not None
                else []
            )
            _preserve_chat_run_ids = {
                str(_plan.run_id)
                for _plan in _plans
                if _plan.safe and _plan.request_payload is not None and _plan.run_id
            }
            _recovered_durable_chat_runs.extend((_account, _run) for _run in _recovered)
            _recovered_ids = {str(_run.get("id") or "") for _run in _recovered}
            _deferred = [
                _plan
                for _plan in _plans
                if _plan.safe
                and _plan.request_payload is not None
                and str(_plan.run_id) not in _recovered_ids
            ]
            if _deferred:
                _deferred_durable_chat_plans.append((_account, _deferred))
            if _recovered:
                _lifespan_log.warning(
                    "Recovered %s durable agent run(s) for safe restart replay.",
                    len(_recovered),
                )
        except Exception as exc:
            # Recovery is fail-closed: if the receipt planner itself cannot prove
            # safety, the ordinary orphan reconciliation below settles the run as
            # interrupted instead of risking a duplicate side effect.
            _lifespan_log.warning("durable agent recovery planning failed: %s", exc)

        try:
            from storage.chat_generation_runs_db import reconcile_runs
            reconciled_chat_run_ids = _run_as(
                _account,
                reconcile_runs,
                preserve_run_ids=_preserve_chat_run_ids,
            )
            reconciled_chat_runs = len(reconciled_chat_run_ids)
    ...
    # Disabling the periodic stale-run sweeper must not disable crash recovery.
    # Fresh safe rows still wait out a nonzero ownership lease before CAS replay.
    for _account, _plans in _deferred_durable_chat_plans:
        _run_as(
            _account,
            app.state.chat_generation_supervisor.schedule_recovery_plans,
            _plans,
        )
    # Start only the runs the receipt planner proved safe. run_as() executes
    # start() while the account ContextVar is bound, and asyncio.create_task then
    # captures that context for the producer and every tool worker it spawns.
    for _account, _run in _recovered_durable_chat_runs:
        try:
            _run_as(
                _account,
                app.state.chat_generation_supervisor.start_recovered,
                str(_run.get("id") or ""),
                thread_id=str(_run.get("threadId") or "") or None,
                model=str((_run.get("resumeRequestPayload") or _run.get("requestPayload") or {}).get("model") or "") or None,
                expected_worker_token=str(_run.get("_workerToken") or ""),
                recovery_action=str(_run.get("_recoveryAction") or "resume_model"),
                approval_id=str(_run.get("_approvalId") or "") or None,
                resume_checkpoint=_run.get("_resumeCheckpoint"),
            )
        except Exception as exc:
            _lifespan_log.warning(
                "durable agent recovery could not restart run %s: %s",
                _run.get("id"),
                exc,
            )
    # Post-answer finalizers are independent of generation process lifetime. A
    # crash after any keyed side effect leaves a running claim; requeue it under
    # the owning account context and replay the same idempotent receipts.
    for _account in _reconcile_accounts:
        try:
            recovered_finalizations = _run_as(
                _account,
                app.state.chat_generation_supervisor.resume_pending_finalizations,
            )
            if recovered_finalizations:
                _lifespan_log.warning(
                    "Resumed %s interrupted chat turn finalization(s) after restart.",
                    len(recovered_finalizations),
                )
        except Exception as exc:
            _lifespan_log.warning("chat finalization reconciliation failed: %s", exc)
...
app = FastAPI(
    ...
    description = "Helix Harness API — chat, tools, memory, and Helix Engine. Built on Unsloth Studio.",
    lifespan = lifespan,
...
class RemoteAccessCORSMiddleware(CORSMiddleware):
    """Allow the exact active remote origin, never arbitrary sites during a tunnel."""
    ...
    def is_allowed_origin(self, origin: str) -> bool:
        ...
        return canonical(origin) == canonical(published)
...
app.include_router(helix_engine_router, prefix = "/api/helix-engine", tags = ["helix-engine"])
...
async def liveness_check():
    ...
    helix_identity = _helix_runtime_identity()
    alive = {
        "status": "alive",
        "service": "Helix Harness",
        ...
        "helix_backend_contract": helix_identity.get("contract"),
        "helix_backend_tree_sha256": helix_identity.get("tree_sha256"),
        "helix_backend_verified": helix_identity.get("verified") is True,
