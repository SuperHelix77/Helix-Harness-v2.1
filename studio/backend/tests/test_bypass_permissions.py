...
def test_sandbox_nproc_default_is_platform_aware(monkeypatch):
    monkeypatch.delenv("UNSLOTH_STUDIO_SANDBOX_NPROC", raising=False)
    monkeypatch.setattr(tools.sys, "platform", "darwin")
    assert tools._sandbox_nproc_limit() == 1024
    monkeypatch.setattr(tools.sys, "platform", "linux")
    assert tools._sandbox_nproc_limit() == 256


def test_sandbox_nproc_explicit_override_wins(monkeypatch):
    monkeypatch.setenv("UNSLOTH_STUDIO_SANDBOX_NPROC", "777")
    monkeypatch.setattr(tools.sys, "platform", "darwin")
    assert tools._sandbox_nproc_limit() == 777
...
def captured_popen(monkeypatch):
    ...
    # These tests inspect the tool child's Popen contract. Durable child adoption
    # performs a later best-effort process-identity probe (ps on macOS), which is
    # intentionally outside that contract and would otherwise overwrite cap.
    monkeypatch.setattr(tools, "_adopt_tool_pid", lambda _pid: None)
    monkeypatch.setattr(tools, "_forget_tool_pid", lambda _proc: None)
    return cap
