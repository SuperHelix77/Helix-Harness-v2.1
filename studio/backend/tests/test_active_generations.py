...
def test_borrowed_registration_keeps_owner_epoch_across_fence_lift_and_rejects_stale_telemetry():
    account_id = "borrowed-account"
    event = threading.Event()
    owner = active_generations.ActiveGeneration(
        event,
    ...
    owner.__enter__()
    try:
        owner_binding = owner.lifecycle_binding
        assert owner_binding == (account_id, 0)

        active_generations.fence(account_id)
        active_generations.lift_fence(account_id)
        # The owner context intentionally hides the account's current epoch;
        # briefly unbind it to verify that fence + lift advanced the registry
        # epoch before rebinding the owner for the borrower check.
        owner.unbind()
        assert active_generations.lifecycle_epoch(account_id) == owner_binding[1] + 2
        owner.bind()

        borrower = active_generations.ActiveGeneration(
            event,
            account_id = account_id,
            run_id = "run-1",
            thread_id = "borrower-thread",
        )
        borrower.__enter__()
        try:
            assert borrower.lifecycle_binding == owner_binding
            assert active_generations.count(account_id) == 1
            with active_generations.telemetry_guard(
                account_id,
                epoch = owner_binding[1],
            ) as allowed:
                assert allowed is False
        finally:
            borrower.__exit__(None, None, None)

        # A nested tracker borrowed the owner entry; it must not release the
        # process-global registration when its own context ends.
        assert active_generations.count(account_id) == 1
        assert active_generations.snapshot(account_id)[0]["thread_id"] == "borrower-thread"
    finally:
        owner.__exit__(None, None, None)

    assert active_generations.count(account_id) == 0
