...
_HAS_PYTEST_ASYNCIO = _ilu.find_spec("pytest_asyncio") is not None
...
def pytest_pyfunc_call(pyfuncitem):
    """Run @pytest.mark.asyncio tests without making pytest-asyncio a backend dependency.

    The backend test environment intentionally does not require pytest-asyncio. A
    small number of supervisor tests still use its marker spelling, so on the
    managed runtime pytest would collect them and then fail before exercising any
    application code. If the real plugin is installed it remains authoritative;
    otherwise execute only explicitly marked coroutine tests with asyncio.run().
    """
