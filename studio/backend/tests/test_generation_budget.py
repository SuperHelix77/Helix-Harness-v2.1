...
def test_transformers_adaptive_cap_limits_standard_decode(monkeypatch):
    backend, model = _streaming_backend(monkeypatch, window = 100)
    ...
    # ceil(100 * 0.80) - 37 prompt tokens = 43 tokens of adaptive room.
    assert model.calls[0]["max_new_tokens"] == 43


def test_transformers_adaptive_cap_records_checkpoint_when_boundary_is_reached(monkeypatch):
    backend, model = _streaming_backend(monkeypatch, window = 100)
    ...
    monkeypatch.setattr(model, "generate", _generate)
    ...
    checkpoint = backend.last_generation_stats["adaptive_checkpoint"]
    assert backend.last_generation_stats["truncated"] is True
    assert checkpoint["completion_tokens"] == 43
    assert checkpoint["occupancy_tokens"] == 80
    assert checkpoint["segment_max_tokens"] == 43


def test_transformers_user_cap_below_adaptive_room_is_unchanged(monkeypatch):
    backend, model = _streaming_backend(monkeypatch, window = 100)
    ...
    assert model.calls[0]["max_new_tokens"] == 20
    assert "adaptive_checkpoint" not in (backend.last_generation_stats or {})


def test_transformers_adaptive_zero_room_checkpoints_before_generate(monkeypatch):
    backend, model = _streaming_backend(monkeypatch, window = 40)
    ...
    assert model.calls == []
    assert backend.last_generation_stats["truncated"] is True
    assert backend.last_generation_stats["adaptive_checkpoint"]["segment_max_tokens"] == 0
