...
def test_stalled_search_circuit_fails_fast_then_recovers(monkeypatch):
    from core.inference import tools
    ...
    monkeypatch.setattr(tools, "_search_knowledge_base", stalled_search)
    tools._RAG_SEARCH_STALLED.clear()
    try:
        first = tools._search_knowledge_base_with_budget(
            {"query": "q"}, {"kb_id": "a"}, timeout = 0.05
        )
        assert "timed out" in first.lower()
        assert started.is_set()
        assert tools._RAG_SEARCH_STALLED.is_set()

        began = time.monotonic()
        second = tools._search_knowledge_base_with_budget(
            {"query": "q2"}, {"kb_id": "a"}, timeout = None
        )
        assert time.monotonic() - began < 0.2
        assert "still unwinding" in second.lower()
        assert calls == 1

        release.set()
        deadline = time.monotonic() + 2
        while tools._RAG_SEARCH_STALLED.is_set() and time.monotonic() < deadline:
            time.sleep(0.01)
        assert not tools._RAG_SEARCH_STALLED.is_set()

        third = tools._search_knowledge_base_with_budget(
            {"query": "q3"}, {"kb_id": "a"}, timeout = 1
        )
        assert third == "late"
        assert calls == 2
    finally:
