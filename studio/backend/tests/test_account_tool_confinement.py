...
from core.inference import tool_capabilities, tool_confinement, tools
...
def _confinement_for(account: AccountContext):
    """Build the ordinary offline terminal confinement through the capability broker."""

    payload = "true"

    def build():
        grant = tool_capabilities.issue_local_execution_grant("terminal", payload)
        return tools._account_confinement(
            capability_grant=grant,
            tool_name="terminal",
            payload=payload,
        )

    return run_as(account, build)
...
def test_owner_sandbox_uses_host_confinement_when_available():
    if sys.platform != "darwin" and not (sys.platform == "linux" and LANDLOCK):
        with pytest.raises(tool_confinement.ToolConfinementUnavailable):
            _confinement_for(OWNER)
        return
    confinement = _confinement_for(OWNER)
    assert confinement is not None
    if sys.platform == "darwin":
        assert confinement.mechanism == "sandbox-exec"
    else:
        assert confinement.mechanism.startswith("landlock-abi")
    kwargs = {"preexec_fn": tools._sandbox_preexec}
    argv = ["bash", "-c", "true"]
    wrapped = tools._apply_confinement(confinement, kwargs, argv)
    assert wrapped[-3:] == argv


@pytest.mark.parametrize(("platform", "abi"), [("win32", None), ("linux", 9)])
def test_owner_capability_confinement_fails_closed_without_a_mechanism(
    monkeypatch, platform, abi
):
...
def test_owner_macos_profile_runs_normal_process_and_denies_ambient_network():
    confinement = _confinement_for(OWNER)
    assert confinement is not None
    profile = confinement.wrapper[2]
    assert "(allow default)" in profile
    assert "(deny network*)" in profile
    import subprocess

    result = subprocess.run(
        confinement.wrap(["/bin/bash", "-c", "printf HELIX_OWNER_SANDBOX_OK"]),
    ...
    assert result.stdout == "HELIX_OWNER_SANDBOX_OK"
...
def test_owner_opt_out_runs_unconfined_when_no_mechanism(monkeypatch):
    ...
    confinement = _confinement_for(ALICE)
...
def test_macos_profile_keeps_the_interpreter_readable_under_a_hidden_root(tmp_path, monkeypatch):
    ...
    run_as(ALICE, tools._get_workdir, "chat")
    profile = _confinement_for(ALICE).wrap(["bash"])[2]
...
@pytest.mark.parametrize("abi", [6, 9])
def test_landlock_below_abi_10_is_refused(monkeypatch, abi):
    monkeypatch.setattr(tool_confinement, "landlock_abi", lambda: abi)
    assert tool_confinement._linux_confinement(tools._SANDBOX_SITE_DIR) is None


def test_landlock_network_rights_are_handled_only_for_offline_grants(tmp_path, monkeypatch):
    rule_root = tmp_path / "rule"
    rule_root.mkdir()
    monkeypatch.setattr(tool_confinement, "landlock_abi", lambda: 10)
    monkeypatch.setattr(
        tool_confinement,
        "_landlock_rules",
        lambda _abi, _site: [(str(rule_root), tool_confinement._FS_READ_DIR)],
    )

    offline = tool_confinement._linux_confinement("unused", allow_network=False)
    online = tool_confinement._linux_confinement("unused", allow_network=True)

    assert offline.preexec.keywords["handled_access_net"] == (
        tool_confinement._NET_BIND_TCP
        | tool_confinement._NET_CONNECT_TCP
        | tool_confinement._NET_BIND_UDP
        | tool_confinement._NET_CONNECT_SEND_UDP
    )
    assert offline.preexec.keywords["network_rules"] == ()
    assert online.preexec.keywords["handled_access_net"] == (
        tool_confinement._NET_BIND_TCP | tool_confinement._NET_BIND_UDP
    )
    assert online.preexec.keywords["network_rules"] == (
        (tool_confinement._NET_BIND_UDP, 0),
    )
    assert offline.preexec.args[2] == (
        tool_confinement._SCOPE_ABSTRACT_UNIX_SOCKET | tool_confinement._SCOPE_SIGNAL
    )


def test_landlock_unix_socket_right_is_limited_to_writable_roots(tmp_path, monkeypatch):
    system = tmp_path / "system"
    interpreter = tmp_path / "interpreter"
    account = tmp_path / "account"
    sandbox_site = tmp_path / "sandbox-site"
    writable = tmp_path / "writable"
    for path in (system, interpreter, account, sandbox_site, writable):
        path.mkdir()
    monkeypatch.setattr(tool_confinement, "_SYSTEM_READ_ROOTS", (str(system),))
    monkeypatch.setattr(tool_confinement, "_interpreter_roots", lambda: [str(interpreter)])
    monkeypatch.setattr(tool_confinement, "_readable_account_roots", lambda: [str(account)])
    monkeypatch.setattr(tool_confinement, "_writable_roots", lambda: [str(writable)])
    monkeypatch.setattr(tool_confinement, "_protected_roots", lambda: [])
    monkeypatch.setattr(tool_confinement, "_private_system_paths", lambda: [])
    monkeypatch.setattr(tool_confinement, "_device_nodes", lambda: [])
    rules = tool_confinement._landlock_rules(10, str(sandbox_site))
    writable_roots = {str(writable)}

    assert tool_confinement._handled_mask(8) & tool_confinement._FS_RESOLVE_UNIX == 0
    assert tool_confinement._handled_mask(9) & tool_confinement._FS_RESOLVE_UNIX
    assert writable_roots
    for path, access in rules:
        if path in writable_roots:
            assert access & tool_confinement._FS_RESOLVE_UNIX, path
        else:
            assert access & tool_confinement._FS_RESOLVE_UNIX == 0, path


@pytest.mark.parametrize("replacement", ["directory", "symlink"])
def test_landlock_rule_path_replacement_fails_closed(tmp_path, replacement):
    rule_root = tmp_path / "rule"
    rule_root.mkdir()
    prepared = tool_confinement._capture_landlock_rules(
        [(str(rule_root), tool_confinement._FS_READ_DIR)]
    )[0]
    original = tmp_path / "original-rule"
    rule_root.rename(original)
    if replacement == "directory":
        rule_root.mkdir()
    else:
        rule_root.symlink_to(original, target_is_directory=True)

    with pytest.raises(tool_confinement.ToolConfinementUnavailable, match="rule path"):
        tool_confinement._open_landlock_rule_fd(prepared)


@pytest.mark.skipif(not LANDLOCK, reason = "Landlock ABI 10 not available on this kernel")
def test_landlock_network_boundary_for_offline_and_egress_grants(tmp_path):
    import socket

    run_as(BOB, tools._get_workdir, "network-test")
    ...
    def run(code):
        def build():
            grant = tool_capabilities.issue_local_execution_grant("python", code)
            return grant, tools._account_confinement(
                capability_grant=grant,
                tool_name="python",
                payload=code,
            )

        grant, confinement = run_as(BOB, build)
        result = subprocess.run(
            ...
            text=True,
            check=False,
            preexec_fn=confinement.preexec,
        )
        return grant, result
    ...
    try:
        dynamic = (
            "name = 'socket'\n"
            "socket = __import__(name)\n"
            "def attempt(label, action):\n"
            "    try:\n"
            "        action(); print(label, 'ALLOWED')\n"
            "    except OSError as exc:\n"
            "        print(label, 'DENIED', exc.errno)\n"
            f"attempt('TCP_CONNECT', lambda: socket.create_connection(('127.0.0.1', {tcp_port}), timeout=1).close())\n"
            f"attempt('UDP_SEND', lambda: socket.socket(socket.AF_INET, socket.SOCK_DGRAM).sendto(b'offline', ('127.0.0.1', {udp_port})))\n"
            "attempt('TCP_BIND', lambda: socket.socket().bind(('127.0.0.1', 0)))\n"
            "attempt('UDP_BIND_ZERO', lambda: socket.socket(socket.AF_INET, socket.SOCK_DGRAM).bind(('127.0.0.1', 0)))\n"
        )
        grant, denied = run(dynamic)
        assert grant.capabilities == frozenset()
        assert denied.returncode == 0, denied.stdout
        for operation in ("TCP_CONNECT", "UDP_SEND", "TCP_BIND", "UDP_BIND_ZERO"):
            assert f"{operation} DENIED" in denied.stdout, denied.stdout
            assert f"{operation} ALLOWED" not in denied.stdout, denied.stdout
        ...
        explicit = (
            ...
            "        print(label, 'DENIED', exc.errno)\n"
            f"attempt('TCP_CONNECT', lambda: socket.create_connection(('127.0.0.1', {tcp_port}), timeout=1).close())\n"
            f"attempt('UDP_SEND', lambda: socket.socket(socket.AF_INET, socket.SOCK_DGRAM).sendto(b'online', ('127.0.0.1', {udp_port})))\n"
            f"attempt('TCP_BIND', lambda: socket.socket().bind(('127.0.0.1', {udp_port})))\n"
            f"attempt('UDP_BIND_NONZERO', lambda: socket.socket(socket.AF_INET, socket.SOCK_DGRAM).bind(('127.0.0.1', {tcp_port})))\n"
            "attempt('UDP_BIND_ZERO', lambda: socket.socket(socket.AF_INET, socket.SOCK_DGRAM).bind(('127.0.0.1', 0)))\n"
        )
        grant, allowed = run(explicit)
        assert grant.allows(tool_capabilities.ToolCapability.NETWORK_EGRESS)
        assert allowed.returncode == 0, allowed.stdout
        for operation in ("TCP_CONNECT", "UDP_SEND", "UDP_BIND_ZERO"):
            assert f"{operation} ALLOWED" in allowed.stdout, allowed.stdout
        for operation in ("TCP_BIND", "UDP_BIND_NONZERO"):
            assert f"{operation} DENIED" in allowed.stdout, allowed.stdout
            assert f"{operation} ALLOWED" not in allowed.stdout, allowed.stdout
        assert receiver.recvfrom(32)[0] == b"online"
    finally:
        receiver.close()
        listener.close()
...
@pytest.mark.skipif(not LANDLOCK, reason = "Landlock ABI 10 not available on this kernel")
def test_landlock_unix_socket_boundary(monkeypatch):
    import shutil
    import socket
    import tempfile

    short_root = Path(tempfile.mkdtemp(prefix="hx-unix-", dir="/tmp"))
    monkeypatch.setenv("UNSLOTH_STUDIO_SANDBOX_HOME", str(short_root / "sandboxes"))
    workdir = Path(run_as(BOB, tools._get_workdir, "unix-test"))
    outside_path = short_root / "outside.sock"
    own_path = workdir / "own.sock"
    abstract_address = f"\0helix-{os.getpid()}-{short_root.name}"
    servers = []
    try:
        for address in (str(outside_path), str(own_path), abstract_address):
        ...
        code = (
            "name = 'socket'\n"
            "socket = __import__(name)\n"
            "def attempt(label, address):\n"
            "    client = socket.socket(socket.AF_UNIX, socket.SOCK_STREAM)\n"
            "    try:\n"
            "        client.connect(address); print(label, 'ALLOWED')\n"
            "    except OSError as exc:\n"
            "        print(label, 'DENIED', exc.errno)\n"
            "    finally:\n"
            "        client.close()\n"
            f"attempt('OUTSIDE_PATH', {str(outside_path)!r})\n"
            f"attempt('OWN_PATH', {str(own_path)!r})\n"
            f"attempt('ABSTRACT', {abstract_address!r})\n"
        )
        ...
        assert "OUTSIDE_PATH DENIED" in result.stdout, result.stdout
        assert "OUTSIDE_PATH ALLOWED" not in result.stdout, result.stdout
        assert "OWN_PATH ALLOWED" in result.stdout, result.stdout
        assert "ABSTRACT DENIED" in result.stdout, result.stdout
        assert "ABSTRACT ALLOWED" not in result.stdout, result.stdout
    finally:
...
def test_macos_profile_hides_shared_sandbox_and_project_bases(tmp_path, monkeypatch):
    prefix = tmp_path / "studio" / "runtime"
    ...
    profile = _confinement_for(BOB).wrap(["bash"])[2]
    ...
    for base in (sandboxes, projects):
        deny = profile.rindex(f'(deny file-read* file-write* (subpath "{base.resolve()}"))')
        assert profile.rindex(f'(allow file-read* (subpath "{prefix.resolve()}"))') < deny
...
def test_macos_profile_hides_the_hf_cache(tmp_path, monkeypatch):
    ...
    profile = _confinement_for(BOB).wrap(["bash"])[2]
    deny = profile.rindex(f'(deny file-read* file-write* (subpath "{cache.resolve()}"))')
    assert profile.rindex(f'(allow file-read* (subpath "{prefix.resolve()}"))') < deny
...
def test_macos_profile_network_is_capability_gated():
    denied = tool_confinement.macos_profile(
        read_roots=[],
        hidden_roots=[],
        writable_roots=[],
    )
    allowed = tool_confinement.macos_profile(
        read_roots=[],
        hidden_roots=[],
        writable_roots=[],
        allow_network=True,
    )
    assert "(allow network-outbound)" not in denied
    assert "(allow network-outbound)" in allowed
    assert "(allow network*)" not in allowed
    assert "network-inbound" not in allowed


def test_tool_network_capability_detection_is_positive_not_ambient():
    assert tools._python_requests_network("print('offline')") is False
    assert tools._python_requests_network("import requests\nprint('network')") is True
    assert tools._python_requests_network("__import__(name)") is False
    assert tools._python_requests_network("__import__('httpx')") is True

    assert tools._terminal_requests_network("pwd && git status") is False
    assert tools._terminal_requests_network("curl https://example.com") is True
    assert tools._terminal_requests_network("git clone https://example.com/repo.git") is True
    assert tools._terminal_requests_network("python -m pip install example") is True
