...
def test_skill_preflight_leads_the_tool_nudge(monkeypatch):
    monkeypatch.setattr(
        "routes.inference._skill_tool_tip",
        lambda **kwargs: "<skill_preflight>read existing skills first</skill_preflight>",
    )
    tools = [
        *_CODE_TOOLS,
        {"type": "function", "function": {"name": "read_skill", "parameters": {"properties": {}}}},
    ]
    nudge = _build_tool_action_nudge(tools = tools, model_name = "test-8B")
    assert nudge.startswith("<skill_preflight>")
    assert nudge.find("<skill_preflight>") < nudge.find("Tools are available")
