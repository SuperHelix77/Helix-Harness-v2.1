...
@pytest.mark.parametrize(
    ...
    [
        ...
        (True, True, "tauri"),  # tunnel origin is admitted dynamically, not wildcard
...
def test_api_only_cors_tracks_published_public_url():
    ...
    request = Headers(
        {
            "origin": "https://public.trycloudflare.com",
    ...
    assert response.headers["access-control-allow-origin"] == "https://public.trycloudflare.com"
    rejected = middleware.preflight_response(
        Headers(
            {
