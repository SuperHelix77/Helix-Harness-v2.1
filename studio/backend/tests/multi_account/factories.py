...
HELIX_SESSION_ID = "matrix-helix-session"
LEARNING_PROPOSAL_ID = "matrix-learning-proposal"
...
@seeder("helix-session")
def seed_helix_session(account) -> dict[str, str]:
    from core.helix_engine.capture import clear_session, record_tool_execution

    run_as(account, clear_session, HELIX_SESSION_ID)
    run_as(
        account,
    ...
    return {"session_id": HELIX_SESSION_ID}


@seeder("learning-proposal")
def seed_learning_proposal(account) -> dict[str, str]:
    from routes import learning

    state = learning._empty_state()
    state["pending"] = [
        {
            "id": LEARNING_PROPOSAL_ID,
            "kind": "user",
            "title": SENTINEL,
            "content": "Prefer the account-private matrix behavior.",
            "reason": "isolation matrix",
            "name": None,
            "target": "codex",
    ...
    run_as(account, learning._write_state, state)
    return {"proposal_id": LEARNING_PROPOSAL_ID}
...
CORE_FACTORIES = {
    ...
    "routes.skills:GET:/{name}": Factory(
        "skill", fragment = SENTINEL, absent = SENTINEL
    ),
    "routes.helix_engine:GET:/session/{session_id}": Factory(
        "helix-session", fragment = SENTINEL, absent = SENTINEL
    ),
    "routes.learning:POST:/proposals/{proposal_id}/approve": Factory(
        "learning-proposal", fragment = SENTINEL, absent = SENTINEL
    ),
    "routes.learning:POST:/proposals/{proposal_id}/reject": Factory(
        "learning-proposal", absent = SENTINEL
    ),
}
