...
ORPHAN_THREAD_ID = "chat-orphan-thread"
ORPHAN_MESSAGE_ID = "chat-orphan-assistant"
...
@seeder("chat-orphan-assistant")
def seed_chat_orphan_assistant(account) -> dict[str, str]:
    from storage import studio_db
    from utils.account_context import run_as

    run_as(account, studio_db.upsert_chat_thread, _thread(ORPHAN_THREAD_ID))
    run_as(
        account,
        studio_db.upsert_chat_message,
        {
            "id": ORPHAN_MESSAGE_ID,
            "threadId": ORPHAN_THREAD_ID,
            "role": "assistant",
            "content": [],
            "createdAt": 1000,
        },
    )
    return {"thread_id": ORPHAN_THREAD_ID, "message_id": ORPHAN_MESSAGE_ID}
...
FACTORIES = {
    ...
    "routes.chat_history:DELETE:/threads/{thread_id}/messages/{message_id}": Factory(
        "chat-orphan-assistant", fragment = '"ok":true'
    ),
