...
from routes.inference import (
    ...
    _request_adaptive_context_ratio,
...
def test_adaptive_context_defaults_only_for_studio_threaded_chat():
    ...
    assert _request_adaptive_context_ratio(payload, studio_events = True) == 0.80
    assert _request_adaptive_context_ratio(payload, studio_events = False) is None

    payload.adaptive_context_ratio = 0.70
    assert _request_adaptive_context_ratio(payload, studio_events = False) == 0.70


def test_external_provider_returns_before_local_adaptive_cap_resolution():
    src = inspect.getsource(produce_openai_chat_completions)
    external_return = src.index("return await _proxy_to_external_provider")
    adaptive_resolution = src.index("_adaptive_context_ratio = _request_adaptive_context_ratio")

    assert external_return < adaptive_resolution
...
def test_adaptive_checkpoint_frames_are_studio_gated():
    src = inspect.getsource(produce_openai_chat_completions)
    tree = ast.parse(src)
    guarded: set[int] = set()
    ...
    calls = [
        ...
        and node.func.id == "_adaptive_checkpoint_sse"
    ]
    assert calls, "adaptive checkpoint SSE emission disappeared from the producer"
    assert all(call.lineno in guarded for call in calls)
