...
def test_external_loop_uses_shared_window_result_no_progress_guard(executed, monkeypatch):
    notice = "(output omitted: 999 chars, no context room left)"
    monkeypatch.setattr(loop_mod, "execute_tool", lambda *_a, **_k: notice)
    turns = [
        [
            _sse(
                {
                    "tool_calls": [
                        _call_delta(0, f"call_{index}", "web_search", f'{{"query":"q{index}"}}')
                    ]
                }
            ),
            _sse(finish = "tool_calls"),
    ...
    lines = _run(FakeTransport(turns, heals = False), tools = [WEB])

    ends = _events(lines, "tool_end")
    assert len(ends) == 3
    assert ends[-1]["result"].startswith(notice)
    assert "it will not change" in ends[-1]["result"]


def test_external_loop_hard_deadline_recovers_to_final_answer(executed, monkeypatch):
    release = threading.Event()

    def noncooperative_tool(*_args, **_kwargs):
        release.wait(timeout = 5)
        return "late"

    monkeypatch.setattr(loop_mod, "execute_tool", noncooperative_tool)
    transport = FakeTransport(
        [
            [
                _sse(
                    {
                        "tool_calls": [
                            _call_delta(0, "call_slow", "web_search", '{"query":"slow"}')
                        ]
                    }
                ),
                _sse(finish = "tool_calls"),
            ...
            [_sse({"content": "done after timeout"}), _sse(finish = "stop"), _DONE],
        ],
        heals = False,
    )
    ...
    try:
        lines = _run(transport, tools = [WEB], timeout = 0.05)
    finally:
        release.set()
    ...
    ends = _events(lines, "tool_end")
    assert len(ends) == 1
    assert "exceeded the 0.05s wall-clock limit" in ends[0]["result"]
    assert "done after timeout" in _visible_text(lines)
    assert elapsed < 0.75
