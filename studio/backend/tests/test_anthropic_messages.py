...
class TestAnthropicRequestedStudioTools:
    ...
    def test_recognizes_thread_scoped_learn_skill_server_tool_by_type(self):
        tools = [{"type": "learn_skill", "name": "learn_skill"}]
        assert _anthropic_requested_studio_tools(tools) == {"learn_skill"}
...
class TestAnthropicMessagesToolRouting:
    ...
    def test_permission_mode_gating_for_server_tools(self, monkeypatch):
        ...
        # learn_skill is bounded to the request's thread-local temporary skill
        # root and cannot promote a durable skill, execute code, or reach the
        # network. Auto/default therefore treat it like the other unprompted-safe
        # local server tools; the tool itself refuses to write without thread_id.
        for extra in ({"permission_mode": "auto"}, {}):
            backend = _mock_backend(monkeypatch)
            payload = _basic_payload(
                enable_tools = True,
