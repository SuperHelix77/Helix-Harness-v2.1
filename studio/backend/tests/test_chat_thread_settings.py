...
def test_goal_patch_accepts_frontend_timer_fields():
    """The chat UI persists startedAt/elapsedMs on every Goal save. extra=forbid
    used to reject them as settingsPatch.goal extras."""
    goal = {
        **GOAL,
        "startedAt": 1_700_000_000_000,
        "elapsedMs": 0,
    }
    payload = ChatThreadPatch.model_validate(
        {"settingsPatch": {"goal": goal, "chatMode": "goal"}}
    )
    dumped = payload.model_dump(exclude_unset = True)
    assert dumped["settingsPatch"]["goal"]["startedAt"] == 1_700_000_000_000
    assert dumped["settingsPatch"]["goal"]["elapsedMs"] == 0
    assert dumped["settingsPatch"]["chatMode"] == "goal"
