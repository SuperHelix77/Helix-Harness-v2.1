...
def test_search_memory_is_account_wide_but_search_conversation_memory_is_thread_scoped(
    ...
    monkeypatch.setattr(
        mem0_store,
        "_instance",
        lambda: (_ for _ in ()).throw(RuntimeError("vector store unavailable")),
    )
    monkeypatch.setattr(learning_routes, "_read_state", lambda: {"mem0Enabled": True})
    monkeypatch.setattr(conversation_archive, "enabled", lambda: False)
    tools_mod._RAG_SEARCH_STALLED.clear()

    stored = mem0_store.add_experience(
        None,
        "Cross-thread durable memory: use the marmot verifier.",
        thread_id = "thread-a",
        title = "Marmot verifier",
    )
    assert stored["stored"] is True

    account_memory = tools_mod.execute_tool(
        "search_memory",
        {"query": "marmot verifier"},
        thread_id = "thread-b",
        timeout = 1,
    )
    conversation_memory = tools_mod.execute_tool(
        "search_conversation",
        {"query": "marmot verifier"},
        thread_id = "thread-b",
        timeout = 1,
    ...
    assert "marmot verifier" in account_memory.lower()
    assert "marmot verifier" not in conversation_memory.lower()
...
def test_memory_tool_is_account_scoped_while_conversation_tool_is_thread_scoped(monkeypatch):
    import asyncio
    import types
    ...
    tools = asyncio.run(
        routes_mod._select_request_tools(payload, tools_on = True, mcp_allowed = False)
    )
    names = [tool["function"]["name"] for tool in tools]
    assert "search_memory" in names
    assert "search_conversation" not in names

    monkeypatch.setattr(routes_mod, "_thread_has_conversation_archive", lambda _tid: True)
    monkeypatch.setattr("core.memory.mem0_store.has_any_memory", lambda: False)
    tools = asyncio.run(
        routes_mod._select_request_tools(payload, tools_on = True, mcp_allowed = False)
    )
    names = [tool["function"]["name"] for tool in tools]
