...
class TestParser:
    ...
    def test_call_parses_to_name_and_argument_fragment(self, text, name, fragment):
        ...
        assert result[0]["function"]["name"] == name
        assert isinstance(result[0]["function"]["arguments"], str)
        assert fragment in result[0]["function"]["arguments"]
...
class TestParserMultiFormat:
    ...
    def test_family_parallel_calls_parse_in_order(self, text, names):
        ...
        assert [c["function"]["name"] for c in result] == names
...
def test_adaptive_cap_executes_completed_tool_call_before_checkpoint():
    """A capped completed tool call is an action round, not resumable final text."""
    stats_holder: dict = {}
    recorder = _trace_recorder()
    turn = 0

    def _gen(_messages, **_kwargs):
        nonlocal turn
        turn += 1
        stats_holder["stats"] = {
            "usage": {"prompt_tokens": 800, "completion_tokens": 0, "total_tokens": 800},
            "truncated": True,
            "adaptive_checkpoint": {"reason": "context_ratio", "ratio": 0.80},
        }
        if turn == 1:
            yield 'web_search[ARGS]{"query":"cats"}'
    ...
    assert executed == [("web_search", {"query": "cats"})]
    assert any(event.get("type") == "tool_start" for event in events)
    assert any(event.get("type") == "tool_end" for event in events)
    assert turn == 2, "checkpoint belongs to the post-tool text segment"
    assert [event.semantic_contribution for event in recorder.snapshot()] == [
        SemanticContribution.ACTION_PLAN,
        SemanticContribution.NONE,
    ]


def test_invocation_trace_plain_final_answer():
    recorder = _trace_recorder()
    loop, _execute = _make_loop(
        turns=[["A complete answer."]],
        invocation_recorder=recorder,
    )
    ...
    [invocation] = recorder.snapshot()
    assert invocation.outcome is InvocationOutcome.COMPLETED
    assert invocation.semantic_contribution is SemanticContribution.FINAL_ANSWER
    assert invocation.wasted is False


def test_invocation_trace_adaptive_segment_is_retained_not_wasted():
    recorder = _trace_recorder()
    stats = {"stats": {"adaptive_checkpoint": {"reason": "length"}}}
    loop, _execute = _make_loop(
        turns=[["Retained partial answer"]],
        invocation_recorder=recorder,
        generation_stats_holder=stats,
    )
    ...
    [invocation] = recorder.snapshot()
    assert invocation.outcome is InvocationOutcome.COMPLETED
    assert invocation.semantic_contribution is SemanticContribution.NONE
    assert invocation.wasted is False


def test_invocation_trace_plain_length_segment_is_retained_not_final():
    recorder = _trace_recorder()
    stats = {"stats": {"truncated": True}}
    loop, _execute = _make_loop(
        turns=[["Visible but incomplete answer"]],
        invocation_recorder=recorder,
        generation_stats_holder=stats,
    )
...
def test_invocation_trace_reasoning_only_segment_is_not_final_answer():
    recorder = _trace_recorder()
    loop, _execute = _make_loop(
        turns=[["<think>Private reasoning only.</think>"]],
        invocation_recorder=recorder,
    ...
    [invocation] = recorder.snapshot()
    assert invocation.semantic_contribution is SemanticContribution.NONE
    assert invocation.wasted is True
    assert invocation.wasted_reason == "empty_model_output"


def test_invocation_trace_accepted_tool_plan_then_final_answer():
    recorder = _trace_recorder()
    loop, execute = _make_loop(
        turns=[
            ['web_search[ARGS]{"query":"cats"}'],
            ["Cats are mammals."],
        ],
        exec_results=["search result"],
        invocation_recorder=recorder,
        nudge_tool_calls=False,
    )
    ...
    assert execute.calls == [("web_search", {"query": "cats"})]
    assert [event.semantic_contribution for event in recorder.snapshot()] == [
        SemanticContribution.ACTION_PLAN,
        SemanticContribution.FINAL_ANSWER,
    ]


def test_invocation_trace_reprompt_and_controller_noop_are_wasted():
    recorder = _trace_recorder()
    loop, execute = _make_loop(
        turns=[
    ...
    assert execute.calls == [("web_search", {"query": "cats"})]
    traced = recorder.snapshot()
    assert [event.semantic_contribution for event in traced] == [
        SemanticContribution.NONE,
        SemanticContribution.ACTION_PLAN,
        SemanticContribution.NONE,
        SemanticContribution.FINAL_ANSWER,
    ]
    assert traced[0].wasted_reason == "plan_without_action"
    assert traced[2].wasted_reason == "controller_suppressed_output"


def test_invocation_trace_generator_exception_is_failed():
    recorder = _trace_recorder()
    ...
    with pytest.raises(RuntimeError, match="model failed"):
        list(loop)
    [invocation] = recorder.snapshot()
    assert invocation.outcome is InvocationOutcome.FAILED
    assert invocation.wasted_reason == "model_generator_exception"


def test_invocation_trace_cooperative_cancellation_is_cancelled():
    recorder = _trace_recorder()
    cancel = threading.Event()

    def cancelled(_messages):
        cancel.set()
        yield "partial"

    list(
        run_safetensors_tool_loop(
            single_turn=cancelled,
            messages=[{"role": "user", "content": "hi"}],
            tools=[],
            execute_tool=lambda *_args, **_kwargs: "unused",
            cancel_event=cancel,
    ...
    [invocation] = recorder.snapshot()
    assert invocation.outcome is InvocationOutcome.CANCELLED
    assert invocation.semantic_contribution is SemanticContribution.NONE
    assert invocation.wasted is True


def test_invocation_trace_durable_replay_is_not_a_model_invocation(monkeypatch):
    from core.inference import durable_tool_journal
    ...
    monkeypatch.setattr(
        durable_tool_journal,
        "current_resume_checkpoint",
        lambda *, backend: checkpoint,
    )
    ...
    def replay(*_args, **_kwargs):
        ...
        if False:
            yield None
        return {"executed": 1, "denied": False}

    monkeypatch.setattr(durable_tool_journal, "resume_checkpoint_calls", replay)
    ...
    assert order == ["replay", "model"]
    [invocation] = recorder.snapshot()
    assert invocation.semantic_contribution is SemanticContribution.FINAL_ANSWER


def test_invocation_trace_does_not_change_stream_events_when_absent_or_present():
    ...
    assert collect() == collect(_trace_recorder())


def test_invocation_trace_cap_failure_does_not_change_stream_events(monkeypatch):
    from core.helix_engine import optimization_trace

    baseline, _execute = _make_loop(turns=[["Same answer."]])
    expected = list(baseline)
    recorder = _trace_recorder()
    monkeypatch.setattr(optimization_trace, "MAX_INVOCATION_EVENTS", 0)
    instrumented, _execute = _make_loop(
        turns=[["Same answer."]],
        invocation_recorder=recorder,
    )

    assert list(instrumented) == expected
    trace = optimization_trace.optimization_trace_envelope(recorder)
    assert trace.status.value == "unavailable"
    assert trace.error == "trace_begin_failed"


def test_invocation_trace_finalize_failure_does_not_change_stream_events():
    class BrokenHandle:
        def complete_final_answer(self):
    ...
    assert list(instrumented) == expected
    assert recorder.invalid_reason == "trace_finalize_failed"


def test_invocation_trace_generator_close_is_discarded():
    recorder = _trace_recorder()
    loop, _execute = _make_loop(
        turns=[["one", " two"]],
        invocation_recorder=recorder,
    ...
    assert next(loop)["type"] == "content"
    loop.close()

    [invocation] = recorder.snapshot()
    assert invocation.outcome is InvocationOutcome.DISCARDED
    assert invocation.semantic_contribution is SemanticContribution.NONE
...
_DS_SHORT_OPEN = "<｜tool▁calls｜>"
_DS_END = "<｜tool▁calls▁end｜>"
...
class TestParserDeepSeek:
    ...
    def test_v3_1_strict_rejects_unclosed_envelope(self):
        # Envelope truncated mid-stream: healed by default, rejected with Auto-Heal off.
        text = _ds_section(_ds_call("get_time", '{"city": "Tokyo"}', closed = False), closed = False)
    ...
    def test_deepseek_section_strips_out_of_visible_text(self, opener):
        text = "before " + _ds_section(_ds_call("foo", "{}"), opener = opener) + " after"
        assert strip_tool_markup(text, final = True) == "before  after"
    ...
    def test_deepseek_signal_wakes_streaming(self):
        ...
        assert has_tool_signal(_DS_OPEN + "...")
...
class TestParserKimi:
    ...
    def test_kimi_simple_call(self):
        result = parse_tool_calls_from_text(
            _kimi_section(_kimi_call("functions.special_function:0", '{"arg1": 1}'))
        )
        assert len(result) == 1
    ...
    def test_kimi_strip_markup(self):
        text = "before " + _kimi_section(_kimi_call("functions.x:0")) + " after"
        assert strip_tool_markup(text, final = True) == "before  after"
...
def test_tool_markup_is_not_streamed_as_content(turns, expected_calls, hidden, visible):
    loop, exec_fn = _make_loop(turns = turns, exec_results = ["RESULT"], max_tool_iterations = 3)
    events = _collect_events(loop)
    assert exec_fn.calls == expected_calls, exec_fn.calls
    contents = [e["text"] for e in events if e["type"] == "content"]
    assert not any(h in t for t in contents for h in hidden), contents
...
class TestRoutesPythonTagStrip:
    ...
    def test_python_tag_strip(self, text, expected):
        assert self._strip(text) == expected
