...
def test_cancelled_route_during_spawn_keeps_the_worker_result_authoritative():
    ...
    async def run_cancellation_race():
        ...
        if not await wait_for_event(spawn_entered):
            if handler.done():
                pytest.fail(f"training start handler completed before spawn: {await handler!r}")
            pytest.fail("training start handler never reached the spawn fixture")
        handler.cancel()
    ...
    with (
        ...
        patch.object(route, "_refuse_unauthorized_cached_dataset", new = lambda request, token: None),
