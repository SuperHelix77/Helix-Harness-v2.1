...
def test_durable_execution_receipt_precedes_effect_and_finish_follows_result(monkeypatch):
    order = []

    monkeypatch.setattr(
        "core.inference.durable_tool_journal.record_execution_started",
        lambda tool_name, tool_call_id: order.append(("started", tool_name, tool_call_id)) or "exec-1",
    )
    monkeypatch.setattr(
        "core.inference.durable_tool_journal.record_execution_finished",
        lambda execution_id, **kwargs: order.append(
            ("finished", execution_id, kwargs.get("result"), kwargs.get("error"))
        ),
    )
    ...
    _events, result = _run_stream(tool, tool_name="terminal", tool_call_id="call-1")
    assert result == "done"
    assert order == [
        ("started", "terminal", "call-1"),
        ("effect",),
        ("finished", "exec-1", "done", None),
    ]


def test_failed_durable_start_receipt_fences_tool_before_side_effect(monkeypatch):
    executed = False

    def reject_start(_tool_name, _tool_call_id):
    ...
    with pytest.raises(RuntimeError, match="journal unavailable"):
        _run_stream(tool, tool_name="terminal", tool_call_id="call-2")
...
def test_hard_tool_deadline_cancels_only_the_tool(monkeypatch):
    from core.inference import tool_stream_exec

    monkeypatch.setattr(tool_stream_exec, "TOOL_HARD_TIMEOUT_S", 0.05)
    request_cancel = threading.Event()
    tool_observed_cancel = threading.Event()

    def tool(_callback, *, tool_cancel_event = None):
        assert tool_cancel_event is not None
        assert tool_cancel_event.wait(timeout = 2)
        tool_observed_cancel.set()
    ...
    assert events == []
    assert "exceeded the 0.05s wall-clock limit" in result
    assert "final state is unknown" in result
    assert tool_observed_cancel.wait(timeout = 1)
    assert not request_cancel.is_set()
    assert elapsed < 0.5


def test_hard_tool_deadline_does_not_wait_for_noncooperative_worker(monkeypatch):
    from core.inference import tool_stream_exec
...
def test_timed_out_noncooperative_tool_cannot_multiply_orphan_workers(monkeypatch):
    from core.inference import tool_stream_exec
    ...
    try:
        ...
        assert "exceeded the 0.05s wall-clock limit" in first
        assert calls == 1

        began = time.monotonic()
        _events, second = _run_stream(
            tool,
            tool_name = "edit_file",
            timeout_s = None,
            heartbeat_interval_s = 1,
        ...
        assert "earlier timed-out call" in second
        assert calls == 1

        release.set()
        deadline = time.monotonic() + 1
        while tool_stream_exec._STALLED_TOOLS and time.monotonic() < deadline:
            time.sleep(0.01)
        assert not tool_stream_exec._STALLED_TOOLS

        _events, third = _run_stream(
            tool,
            tool_name = "edit_file",
            timeout_s = 1,
            poll_interval_s = 0.005,
        ...
        release.set()
        with tool_stream_exec._STALLED_TOOLS_LOCK:
            tool_stream_exec._STALLED_TOOLS.clear()
...
def test_process_output_capture_is_bounded_before_result_truncation(monkeypatch):
    import subprocess

    from core.inference import tools

    monkeypatch.setattr(tools, "_PROCESS_OUTPUT_CAPTURE_MAX_CHARS", 128)
    monkeypatch.setattr(tools, "_PROCESS_OUTPUT_READ_CHARS", 31)
    produced = 5_000
    proc = subprocess.Popen(
        [sys.executable, "-c", f"import sys; sys.stdout.write('x' * {produced})"],
    ...
    output, timed_out = tools._drain_process_output(proc, 5, live.append)

    assert timed_out is False
    assert sum(len(chunk) for chunk in live) == produced
    assert f"{produced} chars were produced" in output
    assert "stdout capture capped at 128 chars" in output
    assert output.endswith("x" * 128)
