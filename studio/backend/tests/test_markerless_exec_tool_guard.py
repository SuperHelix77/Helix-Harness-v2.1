...
The fix: an execution-class tool (``python``/``terminal``/``edit_file``/``computer``) or any open-vocabulary
...
EXEC_ENABLED = {"web_search", "python", "terminal", "edit_file", "computer"}
# ``None`` = name-agnostic parsing (no tool list); the guard must hold here too.
GATES = [None, EXEC_ENABLED]
EXEC_NAMES = ["python", "terminal", "edit_file", "computer"]
...
def test_execution_class_covers_every_local_code_tool():
    ...
    assert EXECUTION_CLASS_TOOL_NAMES == frozenset({"python", "terminal", "edit_file", "computer"})


@pytest.mark.parametrize("name", EXEC_NAMES)
@pytest.mark.parametrize("enabled", [None, {"python", "terminal", "edit_file", "computer"}, {"web_search"}])
