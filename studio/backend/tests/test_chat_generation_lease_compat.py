...
def test_missing_generation_table_fails_closed(monkeypatch, clock):
    # An unexpectedly absent authority table is corruption/incomplete setup, not a
    # pre-migration compatibility state. Exposing a connection would let callers
    # make ownership decisions without any durable row to fence them.
    runs_db._schema_ready = set()
    conn = runs_db._connect()
    conn.execute("DROP TABLE IF EXISTS chat_generation_runs")
    conn.commit()
    runs_db._schema_ready = set()
    with pytest.raises(sqlite3.OperationalError, match="schema is unavailable"):
        runs_db._connect()
    ...
    class _Racy:
        ...
        def execute(self, sql, *a, **k):
            if sql.startswith("ALTER TABLE chat_generation_runs ADD COLUMN") and not state["fired"]:
                state["fired"] = True
                # Faithfully simulate the competing process: the column exists
                # by the time SQLite reports our duplicate-column race.
                self._inner.execute(sql, *a, **k)
...
def _migration_blocked(monkeypatch):
    ...
    Ownership-sensitive callers must not receive a partially migrated connection.
    The bounded retries are set to zero delay here; production still backs off.
    """
    _drop_lease_columns()
    runs_db._schema_ready = set()
    real_get = runs_db.get_connection
...
def test_streaming_fails_closed_while_required_migration_is_blocked(clock, monkeypatch):
    token = _seed()
...
def test_get_progress_fails_closed_when_required_columns_are_missing(clock, monkeypatch):
    _seed()
    with _migration_blocked(monkeypatch):
        with pytest.raises(sqlite3.OperationalError, match="database is locked"):
...
def test_live_reaping_fails_closed_until_the_migration_lands(clock, monkeypatch):
    """A missing progress lease cannot safely prove either liveness or expiry."""
    _seed()
    with _migration_blocked(monkeypatch):
...
def test_boot_reconcile_fails_closed_without_required_lease_columns(clock, monkeypatch):
    """Startup must retry later, never reconcile through a partial authority schema."""
    _seed()
    with _migration_blocked(monkeypatch):
        with pytest.raises(sqlite3.OperationalError, match="database is locked"):
...
def test_touch_progress_fails_closed_during_a_blocked_required_migration(clock, monkeypatch):
    _seed()
    with _migration_blocked(monkeypatch):
        with pytest.raises(sqlite3.OperationalError, match="database is locked"):
            runs_db.touch_progress("run-1")
...
def test_a_producer_that_ignores_the_cooperative_cancel_is_force_cancelled(clock):
    ...
    async def _drive():
        ...
        await started.wait()
        supervisor = SimpleNamespace(_tasks = {runs_mod._run_key("run-1"): task})
        sweeper = runs_mod.ChatGenerationLeaseSweeper(
