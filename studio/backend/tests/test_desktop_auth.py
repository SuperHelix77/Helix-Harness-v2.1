...
def _route_submodules_main_imports() -> set[str]:
    """Every ``routes.<module>`` imported directly by main.py, read without importing routes."""
    import re
    from pathlib import Path

    backend = Path(__file__).resolve().parent.parent
    main_src = (backend / "main.py").read_text(encoding = "utf-8")
    names = set(re.findall(r"^from routes\.(\w+) import", main_src, re.M))
    assert names, "main.py imports no routes submodule; re-derive this"
    return names
...
def test_health_response_reports_desktop_capability_fields(monkeypatch):
    ...
    for name in _route_submodules_main_imports() - special_submodules:
...
def test_the_router_stub_covers_every_router_main_imports():
    ...
    # Direct routes.<module> imports are derived too, so adding another Helix route cannot
    # accidentally make this health-only test import the real dependency-heavy module.
    assert _route_submodules_main_imports() >= {
        "llama",
        "llama_compat",
        "settings",
        "prompts",
