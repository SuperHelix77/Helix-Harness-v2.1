...
class TestEstimateMemoryRoute:
    ...
    def test_the_route_never_reaps_processes_or_leaks_an_atexit_handler(
        ...
        # Count only callbacks that retain a LlamaCppBackend.  Lazy imports made by
        # the sizing path (notably torch on a cold process) legitimately register
        # their own process-wide handlers, so comparing the global callback count
        # makes this test depend on collection order instead of the route contract.
        registered_backends = []
        register = atexit.register

        def _record_backend_callback(function, *args, **kwargs):
            owner = getattr(function, "__self__", None)
            if isinstance(owner, LlamaCppBackend):
                registered_backends.append(owner)
            return register(function, *args, **kwargs)

        monkeypatch.setattr(atexit, "register", _record_backend_callback)
        for i in range(5):
            ri._estimate_files_cache.clear()
            ri._estimate_config_cache.clear()
        ...
        assert registered_backends == [], (
            f"the estimate leaked {len(registered_backends)} atexit handlers over "
...
class TestParallelSlotResolution:
    """Blank Parallel Slots follows the platform-adjusted server default.

    /load resolves the field through ``_resolve_parallel_slots``. Non-Apple hosts
    inherit ``app.state.llama_parallel_slots``; macOS deliberately uses one slot
    when the field is omitted because unified memory pays the context cost per
    slot. Explicit multi-slot requests remain authoritative.
    """
    ...
    def test_omitted_slots_use_the_platform_adjusted_server_default(self):
        resp = _estimate(
            model_path = "local/model",
            n_ctx = 8192,
            fastapi_request = _request_with_slots(4),
        )
        assert resp.n_parallel == (1 if sys.platform == "darwin" else 4)
    ...
    def test_more_slots_cost_more(self, monkeypatch):
        monkeypatch.setattr(
            ri.LlamaCppBackend,
    ...
    def test_diffusion_gguf_uses_the_runner_slot_and_compute_shape(self, monkeypatch, tmp_path):
        """DiffusionGemma does not launch llama-server, so it owns no --parallel slots or
        llama compute buffers even when the request explicitly asks for several."""
        diffusion_gguf = _write_gguf(tmp_path, "diffusiongemma", _GQA_FIELDS, "diffusion.gguf")
        config = SimpleNamespace(
            identifier = "local/DiffusionGemma",
            gguf_file = diffusion_gguf,
            is_gguf = True,
            gguf_mmproj_file = None,
            gguf_mtp_file = None,
            gguf_dspark_file = None,
        ...
        monkeypatch.setattr(ri, "_cached_estimate_config", lambda *a, **kw: config)
        monkeypatch.setattr(ri, "_local_gguf_main_path", lambda cfg: diffusion_gguf)
        monkeypatch.setattr(ri, "_gguf_resident_file_gb", lambda cfg, **kw: 1.0)
        # If the route incorrectly treats this as llama-server, this keeps four slots
        # alive so the regression cannot be hidden by an unrelated capability clamp.
        monkeypatch.setattr(
            ri.LlamaCppBackend,
            "probe_server_capabilities",
            staticmethod(lambda *a, **kw: {"found": True, "supports_kv_unified": True}),
        ...
        resp = _estimate(
            model_path = diffusion_gguf,
            n_ctx = 8192,
            n_parallel = 4,
        )
        assert resp.available is True
        assert resp.n_parallel == 1
        assert resp.compute_bytes == 0
