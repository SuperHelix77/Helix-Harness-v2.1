...
def test_a_checkpoint_request_override_still_admits_the_memory_tool(monkeypatch):
    ...
    monkeypatch.setattr(routes_mod, "_thread_has_conversation_archive", lambda _tid: True)
    monkeypatch.setattr("core.memory.mem0_store.has_any_memory", lambda: True)
    monkeypatch.setattr("core.inference.checkpoint.enabled", lambda: False)
    ...
    assert _names("checkpoint") == ["search_conversation", "search_memory"]
    assert "search_conversation" not in _names("rolling")
    assert "search_memory" not in _names("rolling")
...
def test_the_reachability_probe_closes_the_connection_it_opens():
    ...
    if not os.path.isdir("/proc/self/fd"):
        pytest.skip("/proc/self/fd is a Linux-only descriptor probe")
...
def test_occupancy_handoff_fires_at_80_percent_even_when_the_prompt_still_fits():
    assert checkpoint.occupancy_triggers_handoff(79, 100) is False
    assert checkpoint.occupancy_triggers_handoff(80, 100) is True
    messages = _thread(pad = 6, chars = 80)
    fitted, truncation = fit_checkpoint_context(
        messages,
        context_length = count(messages) + 40,
        max_tokens = 8,
        count_tokens = count,
        can_reset = True,
    )
    live = count(messages)
    # Live occupancy is ~95% of n_ctx while still under prompt_target (max_tokens is tiny).
    assert live / (live + 40) >= 0.80
    assert truncation is not None
    assert truncation.get("checkpoint_started") is True
    assert truncation.get("handoff") is True
    assert count(fitted) < live


def test_large_output_allowance_does_not_move_the_soft_checkpoint_below_80_percent():
    """The output reserve is a generation budget, not the adaptive epoch trigger.

    With max_tokens omitted, prompt_budget deliberately reserves n_ctx/4. Before the
    pressure thresholds were separated, that made a 78.7%-full prompt reset even though
    HANDOFF_RATIO says 80%.
    """
    messages = _thread(pad = 6, chars = 80)
    live = count(messages)
    context_length = 249

    assert prompt_budget(context_length, None) < live
    assert live / context_length < checkpoint.HANDOFF_RATIO
...
def test_large_output_allowance_still_checkpoints_at_the_explicit_80_percent_edge():
    messages = _thread(pad = 6, chars = 80)
    live = count(messages)
    context_length = live * 5 // 4

    assert live * 5 == context_length * 4, "fixture must land exactly on 80%"
    assert prompt_budget(context_length, None) < live
...
def test_hard_safety_compacts_even_when_the_adaptive_trigger_is_disabled(monkeypatch):
    """Hard safety is an independent backstop, not another spelling of HANDOFF_RATIO."""
    messages = _thread(pad = 6, chars = 80)
    live = count(messages)
    context_length = live + 5
    hard_target = hard_safety_prompt_target(context_length)
    assert live > hard_target

    # Isolate the hard path: an impossible soft threshold must not make an unsafe prompt
    # pass through unchanged.
    monkeypatch.setattr(checkpoint, "HANDOFF_RATIO", 2.0)
    assert checkpoint.occupancy_triggers_handoff(live, context_length) is False
    ...
    assert truncation is not None
    assert truncation.get("checkpoint_started") is True
    assert count(fitted) <= hard_target


def test_hard_safety_refusal_reports_the_actual_minimum_reply_boundary(monkeypatch):
    """An irreducible prompt still refuses when no old turn can buy reply room."""
    messages = [
        {"role": "system", "content": "you are helpful"},
        {"role": "user", "content": "x" * 40_000},
    ]
    ...
    assert truncation["prompt_target"] == hard_target
    assert truncation["irreducible_tokens"] > hard_target


def test_blocked_soft_checkpoint_keeps_the_rolling_fallback(monkeypatch):
    """Archive degradation must not turn adaptive pressure into a no-op."""
    from core.inference import llama_cpp
    ...
    assert truncation is not None
    assert truncation.get("checkpoint") is not True
    assert truncation.get("dropped_messages", 0) > 0
    assert count(fitted) < live


def test_occupancy_handoff_does_not_repeat_inside_the_same_epoch():
    messages = _thread(pad = 6, chars = 80)
    n_ctx = count(messages) + 40
    first, truncation = fit_checkpoint_context(
        messages,
    ...
    assert truncation and truncation.get("checkpoint_started") is True
    dropped = truncation["dropped_messages"]
    again, second = fit_checkpoint_context(
        messages,
        context_length = n_ctx,
        max_tokens = 8,
        count_tokens = count,
    ...
    assert second.get("checkpoint_started") is not True
    assert count(again) == count(first)
    assert count(first) / n_ctx < checkpoint.HANDOFF_RATIO
