...
def test_mlx_vlm_preserves_detokenizer_flush_attached_to_eos(monkeypatch):
    """mlx-vlm's BPE detokenizer can hold a Qwen tool-call tail until finalization.

    The final GenerationResult is labelled with the EOS token even though its ``text`` belongs to
    the preceding generated ids. Dropping that text turns a valid Qwen3.8 call into a reasoning-only
    stop before the tool parser can observe it.
    """
    from core.inference import mlx_inference
    from core.inference.tool_call_parser import parse_tool_calls_from_text
    ...
    def _vlm_stream(*_a, **_k):
        yield SimpleNamespace(
            text = "The user wants me to execute ",
            token = None,
            prompt_tokens = 3,
            generation_tokens = 1,
        )
        # Mirrors mlx-vlm 0.6.17: stopping_criteria fires before EOS enters the detokenizer,
        # then finalize() places all buffered prior text on the EOS-labelled result.
        yield SimpleNamespace(
            text = tail,
            token = 9,
            prompt_tokens = 3,
    ...
    assert snapshots[-1].endswith(tail)
    calls = parse_tool_calls_from_text(
        snapshots[-1], allow_incomplete = False, enabled_tool_names = {"terminal"}
    )
    assert calls[0]["function"] == {"name": "terminal", "arguments": '{"command": "pwd"}'}
...
def test_mlx_text_adaptive_cap_limits_standard_decode(monkeypatch):
    budget = _run_text_budget(
        monkeypatch,
        99,
        served = 100,
...
def test_mlx_text_user_cap_below_adaptive_room_is_unchanged(monkeypatch):
    budget = _run_text_budget(
        monkeypatch,
        20,
        served = 100,
