...
def test_adaptive_cap_executes_completed_tool_call_before_checkpoint(monkeypatch):
    """A tool call completed on the 80% token must finish its round before handoff.

    The first decode hits the adaptive cap with a complete structured call.  The
    call must execute; only the following text segment, whose rendered prompt is
    already at the boundary, may emit the adaptive checkpoint metadata.
    """
    first_stream = [
        _tool_call_sse("web_search", {"query": "cats"}, "call_search"),
        _finish("length"),
        _done(),
    ]
    backend, payloads = _backend_and_payloads(monkeypatch, [first_stream])

    def count_tokens(messages, *_args, **_kwargs):
        return 3277 if any(message.get("role") == "tool" for message in messages) else 3200

    monkeypatch.setattr(backend, "count_chat_tokens", count_tokens)
    calls = _record_tool_calls(monkeypatch, "cat results")
    ...
    events = list(
        backend.generate_chat_completion_with_tools(
            ...
            permission_mode = "off",
            adaptive_context_ratio = 0.80,
        )
    )

    assert calls == [("web_search", {"query": "cats"})]
    tool_end_index = next(i for i, event in enumerate(events) if event.get("type") == "tool_end")
    checkpoint_indexes = [
        i
        for i, event in enumerate(events)
        if event.get("type") == "metadata" and event.get("adaptive_checkpoint")
    ]
    assert len(checkpoint_indexes) == 1
    assert checkpoint_indexes[0] > tool_end_index
    assert len(payloads) == 1, "the post-tool turn should checkpoint before another decode"
    assert payloads[0]["max_tokens"] == 77


def test_plain_adaptive_cap_emits_checkpoint_only_when_it_limits_decode(monkeypatch):
    capped_stream = [
        _sse({"content": "capped"}),
        "data: "
        + json.dumps(
    ...
    backend, payloads = _backend_and_payloads(monkeypatch, [capped_stream])
    monkeypatch.setattr(backend, "count_chat_tokens", lambda *_a, **_k: 3200)
    ...
    assert payloads[0]["max_tokens"] == 77
    metadata = next(event for event in events if isinstance(event, dict))
    assert metadata["finish_reason"] == "length"
    assert metadata["adaptive_checkpoint"]["segment_max_tokens"] == 77
    assert metadata["adaptive_checkpoint"]["occupancy_tokens"] == 3277


def test_plain_user_max_tokens_below_adaptive_room_stays_ordinary(monkeypatch):
    user_capped_stream = [
        "data: "
        + json.dumps(
            {
    ...
    backend, payloads = _backend_and_payloads(monkeypatch, [user_capped_stream])
    monkeypatch.setattr(backend, "count_chat_tokens", lambda *_a, **_k: 3200)
    ...
    assert payloads[0]["max_tokens"] == 50
    metadata = next(event for event in events if isinstance(event, dict))
    assert metadata["finish_reason"] == "length"
    assert "adaptive_checkpoint" not in metadata


def test_plain_adaptive_zero_room_checkpoints_before_llama_server(monkeypatch):
    backend, payloads = _backend_and_payloads(monkeypatch, [])
    monkeypatch.setattr(backend, "count_chat_tokens", lambda *_a, **_k: 3277)

    events = list(
        backend.generate_chat_completion(
            messages = [{"role": "user", "content": "already full"}],
            max_tokens = 100,
            adaptive_context_ratio = 0.80,
        )
    )
    ...
    assert events[0]["adaptive_checkpoint"]["segment_max_tokens"] == 0
