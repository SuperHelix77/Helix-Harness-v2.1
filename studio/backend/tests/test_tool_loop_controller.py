...
def test_window_result_no_progress_ignores_changed_arguments():
    controller = ToolLoopController(tools = [_tool("web_search")])
    notice = "(output omitted: 999 chars, no context room left)"
    completions = []

    for index in range(MAX_IDENTICAL_TOOL_RESULTS):
        decision = controller.prepare_call(
            _call("web_search", {"query": f"attempt-{index}"}, f"call-{index}")
        )
        completions.append(controller.record_result(decision, notice))

    assert completions[-1].result.startswith(notice)
    assert "it will not change" in completions[-1].result


def test_same_result_from_distinct_mutations_keeps_arguments_in_progress_key():
    controller = ToolLoopController(tools = [_tool("terminal")])
    completions = []

    for index in range(MAX_IDENTICAL_TOOL_RESULTS + 1):
        decision = controller.prepare_call(
            _call("terminal", {"command": f"write-record-{index}"}, f"call-{index}")
        )
        completions.append(controller.record_result(decision, "OK"))

    assert all(completion.result == "OK" for completion in completions)
