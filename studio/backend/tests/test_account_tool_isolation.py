...
def test_timed_out_tool_stall_is_scoped_to_the_captured_account(monkeypatch):
    from core.inference import tool_stream_exec

    release = threading.Event()
    bob_ran = []
    monkeypatch.setattr(tool_stream_exec, "TOOL_HARD_TIMEOUT_S", 0.02)
    monkeypatch.setattr(tool_stream_exec, "_CANCEL_SETTLE_TIMEOUT_S", 0.001)
    ...
    def alice_tool(_output):
        release.wait(1.0)
        return "alice done"

    def bob_tool(_output):
        bob_ran.append(current_account_id())
        return "bob done"

    # Capturing an account and invoking the returned wrapper after run_as exits is
    # an explicit account_tool_stream contract (see the worker-context test above).
    alice_stream = run_as(ALICE, tool_policy.account_tool_stream, tool_stream_exec.stream_tool_execution)
    bob_stream = run_as(BOB, tool_policy.account_tool_stream, tool_stream_exec.stream_tool_execution)
    try:
        alice_result = drain(alice_stream(alice_tool, tool_name = "same-tool"))
        assert "exceeded" in alice_result
        assert drain(bob_stream(bob_tool, tool_name = "same-tool")) == "bob done"
        assert bob_ran == [BOB.account_id]
    finally:
        release.set()
