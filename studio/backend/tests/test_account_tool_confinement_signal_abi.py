...
"""Regression: a Landlock confinement must cover signals and the full network contract.
...
@pytest.mark.skipif(
    sys.platform != "linux" or tool_confinement.landlock_abi() < 10,
    reason = "needs a kernel able to apply the full ABI 10 boundary",
)
@pytest.mark.parametrize("abi", (3, 4, 5, 6, 7, 8, 9))
def test_pre_abi_10_never_confines_a_managed_account(abi, monkeypatch):
    """An incomplete signal/TCP/UDP sandbox must not count as confinement."""
...
def test_a_managed_child_is_refused_on_a_pre_scope_abi(monkeypatch):
