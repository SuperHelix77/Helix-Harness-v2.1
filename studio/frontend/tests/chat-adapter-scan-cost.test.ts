...
const FLAG_NEXT_TO_APPEND =
  /streamedChars \+= reasoning\.length \+ delta\.length;\s*streamedReplyChars \+= reasoning\.length \+ delta\.length;\s*producedReplyText = true;/;
...
test("the arrival loop touches the reply only in ways that cannot flatten it", () => {
  ...
  assert.equal(
    loop.includes("streamedReplyChars += reasoning.length + delta.length"),
    true,
    "the live context estimate no longer advances from deltas",
  );
  assert.doesNotMatch(
    loop,
    /cumulativeText\.slice\(/,
    "the live context meter is slicing the full accumulated reply on every publish",
  );
});
