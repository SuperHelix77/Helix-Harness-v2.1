...
test("authoritative pending approval restores a card after the saved cursor passed tool_start", () => {
  const carried: Carried[] = [
    {
      at: 0,
      part: {
  ...
  const recovery = createGenerationToolRecovery(carried, "run", 8);
  const snapshot = recovery.syncPendingApprovals(
    [
      {
        runId: "run",
  ...
  assert.equal(recovery.replayFrom, 8);
  assert.deepEqual(snapshot?.confirmations, [
    {
      toolCallId: "session:thread:approval-1",
      approvalId: "approval-1",
      sessionId: "session",
...
test("approval snapshot materializes a missing display card without an event cursor", () => {
  const carried: Carried[] = [];
  const recovery = createGenerationToolRecovery(carried, "run", 12);
  const snapshot = recovery.syncPendingApprovals(
    [
      {
        runId: "run",
  ...
  assert.equal(recovery.replayFrom, 12);
  ...
  assert.deepEqual(carried, [
    {
      ...
      part: {
        type: "tool-call",
        toolCallId: "session:run:approval-2",
        backendToolCallId: "call_2",
        toolApprovalId: "approval-2",
        toolName: "python",
        args: { code: "print(1)" },
        argsText: '{"code":"print(1)"}',
      },
    },
  ]);
});

test("a stale replayed tool_start cannot resurrect an approval absent from the snapshot", () => {
  const carried: Carried[] = [];
  const recovery = createGenerationToolRecovery(carried, "run", 0);
  recovery.apply(
    {
      ...start(),
  ...
  assert.deepEqual(recovery.syncPendingApprovals([], 0)?.confirmations, []);
  assert.equal(
    (carried[0].part as Record<string, unknown>).toolApprovalId,
    "already-decided",
    "event replay may rebuild history but cannot recreate the decision control",
  );
});
      ...
      const context = vm.createContext({
        ...
        useChatRuntimeStore: {
          getState: () => ({
            clearToolLiveOutput() {},
            clearToolFullOutput() {},
            setToolConfirmation: (...args: unknown[]) => confirmed.push(args),
          }),
        },
      });
...
test("durable approval decision submits the exact run_id request body", async () => {
  const api = readFileSync(
    new URL("../src/features/chat/api/chat-api.ts", import.meta.url),
    "utf8",
  );
  const startAt = api.indexOf("export async function resolveToolConfirmation(");
  const endAt = api.indexOf("\nexport interface CachedGgufRepo", startAt);
  assert.ok(startAt >= 0 && endAt > startAt);
  const source = `${api
    .slice(startAt, endAt)
    .replace("export async function", "async function")}\nglobalThis.resolveToolConfirmation = resolveToolConfirmation;`;
  ...
  const context = vm.createContext({
    ...
    parseJsonOrThrow: async () => ({ resolved: true }),
  });
  vm.runInContext(executable, context);
  assert.equal(
    await context.resolveToolConfirmation(
      "session",
      "approval",
      "deny",
      "run-123",
  ...
  assert.equal(requests[0].url, "/api/inference/tool-confirm");
  ...
  await context.resolveToolConfirmation("legacy-session", "legacy-approval", "allow");
...
const executable = ts.transpileModule(scheduler, {
  ...
  /\.catch\(\(\) => \{\s*\}\)(\s*\.finally\(\(\) => generationRecoveries\.delete\(runId\)\))/,
  ".catch((error) => { globalThis.__generationRecoveryError = error; })$1",
);
...
async function recoverRun(
  ...
  options: {
    ...
    initialConfirmations?: Record<string, Record<string, unknown>>;
  } = {},
) {
  let shown = {
    messages: [
  ...
  const confirmationSyncs: unknown[][] = [];
  const runtime = {
    models: [],
    toolConfirmations: structuredClone(options.initialConfirmations ?? {}),
    registerThreadServerCancel() {},
    setThreadRunning() {},
    clearThreadServerCancel() {},
    clearToolConfirmation(toolCallId: string) {
      delete this.toolConfirmations[toolCallId];
    },
    syncToolConfirmationsForRun(
      _runId: string,
      confirmations: Record<string, unknown>[],
    ) {
      confirmationSyncs.push(structuredClone(confirmations));
      this.toolConfirmations = Object.fromEntries(
        confirmations.map((confirmation) => [
          (confirmation as { toolCallId: string }).toolCallId,
          confirmation,
        ]),
      );
    },
  ...
  const context = vm.createContext({
    ...
    // These scheduler tests exercise durable tool replay, not post-answer learning.
    // The production helper is defined outside the extracted source slice; keep
    // old unkeyed fixture runs side-effect free while allowing settlement to publish.
    recoverPostAnswerFinalization: async ({
      current,
    }: {
    ...
    isTerminalChatGenerationRun: (candidate: { status: string }) =>
  ...
  if (!final && context.__generationRecoveryError) {
    throw context.__generationRecoveryError;
  }
  assert.ok(final);
  if ((options.status ?? "completed") !== "running") {
  ...
  return {
    ...
    snapshots,
    confirmationSyncs,
    toolConfirmations: runtime.toolConfirmations,
  };
}
...
test("reload hydrates approval controls when the saved cursor is past tool_start", async () => {
  const saved = {
    type: "tool-call",
    toolCallId: "session:thread:approval-1",
    backendToolCallId: "call_0",
  ...
  const result = await recoverRun([saved], [start()], {
    cursor: 1,
    status: "running",
    pendingApprovals: [
      {
        ...
        toolCallId: "provider-call-0",
        cardCallId: "call_0",
        arguments: saved.args,
        status: "pending",
        decisionEventSeq: 20,
      },
    ],
  });
  assert.equal(result.replayFrom, 1);
  assert.equal(result.snapshots[0].metadata.generationSeq, 1);
  assert.ok(
    result.confirmationSyncs.some(
      (confirmations) =>
        (confirmations[0] as { approvalId?: string } | undefined)?.approvalId ===
        "approval-1",
    ),
  );
});

test("decided snapshot beats a stale replayed approval start", async () => {
  const saved = {
    type: "tool-call",
    toolCallId: "session:thread:approval-old",
    toolName: "edit_file",
    args: {},
  };
  const result = await recoverRun(
    ...
    { cursor: 1, status: "running", pendingApprovals: [] },
  );
  assert.equal(result.replayFrom, 0);
  assert.ok(result.confirmationSyncs.every((confirmations) => confirmations.length === 0));
});

test("a terminal run clears every run-scoped recovered confirmation", async () => {
  const result = await recoverRun([], [{ choices: [{ delta: { content: "done" } }] }], {
    pendingApprovals: [
      {
        runId: "run",
        approvalId: "stale",
    ...
    initialConfirmations: {
      card: {
        approvalId: "stale",
        sessionId: "session",
        autoAllowKey: "session:thread",
        runId: "run",
      },
    },
  });
  assert.deepEqual(result.toolConfirmations, {});
  assert.ok(result.confirmationSyncs.length > 0);
  assert.ok(result.confirmationSyncs.every((confirmations) => confirmations.length === 0));
});
