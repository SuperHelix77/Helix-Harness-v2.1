...
const {
  ...
  normalizeChatGenerationChunkPayload,
...
test("durable replay normalizes tool control frames exactly like the live stream", () => {
  const start = {
    type: "tool_start",
    tool_name: "terminal",
    tool_call_id: "call-1",
  ...
  assert.deepEqual(normalizeChatGenerationChunkPayload(start), {
    _toolEvent: start,
  });
  assert.deepEqual(
    normalizeChatGenerationChunkPayload({
      type: "tool_status",
      content: "Waiting for approval: command",
    }),
    { _toolStatus: "Waiting for approval: command" },
  );
  const end = {
    type: "tool_end",
    tool_name: "terminal",
  ...
  assert.deepEqual(normalizeChatGenerationChunkPayload(end), {
    _toolEvent: end,
  });
});
...
test("Stop during durable admission backoff prevents another create attempt", async () => {
  const controller = new AbortController();
  let attempts = 0;
  globalThis.fetch = (async () => {
    attempts += 1;
  ...
  // The first reconnect delay is 500 ms. Before this fix the detached create loop woke up and
  // issued a second POST after Stop; give that old path enough time to reproduce deterministically.
...
test("Stop before admission resolves still reaches the server", () => {
  ...
  // Durable with no returned run still knows the stable cancelId. The create response can be
  // lost after the server accepted it, so Stop must fence that id immediately instead of waiting
  // for a response that may never arrive. If the in-flight response does arrive, the direct
  // durable-row cancel remains an additional precise cleanup.
