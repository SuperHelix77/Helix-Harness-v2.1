...
test("monitor stays eager while the closed settings dialog stays out of startup", () => {
  assert.match(SETTINGS_MOUNT_SOURCE, /lazy\(\(\) =>\s*import\("\.\/settings-dialog"\)/);
  assert.match(SETTINGS_MOUNT_SOURCE, /import \{ FloatingMonitor \} from "@\/components\/floating-monitor"/);
  assert.match(SETTINGS_MOUNT_SOURCE, /useSettingsDialogStore\(\(state\) => state\.open\)/);
  assert.match(SETTINGS_MOUNT_SOURCE, /open \? \(/);
  assert.match(SETTINGS_MOUNT_SOURCE, /<Suspense fallback=\{null\}>[\s\S]*?<SettingsDialog \/>/);
  assert.match(SETTINGS_MOUNT_SOURCE, /<FloatingMonitor \/>/);
  assert.doesNotMatch(SETTINGS_MOUNT_SOURCE, /settings-dialog-loading/);
});
