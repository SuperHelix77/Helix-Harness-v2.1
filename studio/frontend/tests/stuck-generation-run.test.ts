...
test("a legacy fallback releases the durable claim at once", () => {
  ...
  const compatibility = adapter.indexOf(
    "Backward compatibility with older Studio backends",
  );
  assert.ok(compatibility > 0, "the legacy-fallback compatibility branch moved");
  const fallback = adapter.indexOf('generationDecision = "legacy";', compatibility);
  assert.ok(fallback > compatibility, "the compatibility branch no longer selects legacy");
