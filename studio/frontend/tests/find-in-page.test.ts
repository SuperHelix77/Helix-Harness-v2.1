...
test("light mode is the chatbox's background, under a slightly heavier shadow", async () => {
  // Glass adds an earlier compound selector for the composer's backdrop blur.
  // Pick the component rule that owns its paint/shadow rather than whichever
  // selector occurrence happens to come first in the stylesheet.
  const composer = [...INDEX.matchAll(/\.unsloth-composer-surface\s*\{([^}]*)\}/g)]
    .map((match) => match[1])
    .find((rule) => /background-color:\s*#[0-9a-f]{6};/i.test(rule));
  assert.ok(composer, "the composer paint rule is gone");
