...
function stubDocument() {
  ...
  (globalThis as { getComputedStyle?: unknown }).getComputedStyle = (
    ...
    getPropertyValue: (name: string) =>
      vars.get(name) ?? (name === "--background" ? "#ffffff" : ""),
    backgroundColor: target.style?.backgroundColor || "#ffffff",
  });
  return vars;
}
...
// The app is compiled in TypeScript "bundler" mode. Bare Node does not resolve
// the store's extensionless relative imports unless the test installs the same
// resolution contract first.
