...
// Opaque palettes use one surface colour. Glass keeps the same RGB base but
// gives popovers more opacity than cards so floating menus stay legible over
// the frosted canvas.
test("popover and card keep the same base colour in every theme", () => {
  ...
  assert.equal(popover.length, card.length);

  const translucent = /^rgb\(([^/]+)\/\s*([\d.]+)\)$/;
  for (let i = 0; i < popover.length; i += 1) {
    const pop = popover[i];
    const base = card[i];
    if (pop === base) continue;
    const popParts = translucent.exec(pop);
    const cardParts = translucent.exec(base);
    assert.ok(popParts && cardParts, `surface mismatch: ${base} vs ${pop}`);
    assert.equal(popParts[1].trim(), cardParts[1].trim());
    assert.ok(
      Number(popParts[2]) >= Number(cardParts[2]),
      `popover ${pop} is less opaque than card ${base}`,
    );
  }
});
