...
function fixture(t: { after(fn: () => void): void }, tauri = true) {
  ...
  let response: unknown = { access_token: "owner-access", refresh_managed: true };
  let invokeCalls = 0;
  let refreshCalls = 0;
  ...
  const session = {
    ...
    storeDesktopAccessToken: (a: string, managed = true) => {
      access = a;
      refresh = managed ? "native-managed" : null;
    },
...
test("single-account desktop keeps refresh authority native, password bypass and coalescing", async (t) => {
  const f = fixture(t);
  ...
  assert.equal(f.refresh, "native-managed");
...
test("returning to one account restores desktop auto-login and guard shortcuts", async (t) => {
  ...
  f.setResponse({ access_token: "restored-owner", refresh_managed: true });
  assert.equal(await f.auth.tauriAutoAuth({ force: true }), true);
  assert.equal(f.auth.isTauriLoginRequired(), false);
...
test("owner tokens after the last managed account is deleted clear that account's browser data", async (t) => {
  ...
  f.setResponse(multi);
  await f.auth.tauriAutoAuth({ force: true });
  f.setResponse({ access_token: "owner-access", refresh_managed: true });
  assert.equal(await f.auth.tauriAutoAuth({ force: true }), true);
