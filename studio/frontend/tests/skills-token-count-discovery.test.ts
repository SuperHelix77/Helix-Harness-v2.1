...
test("token counting prices first-skill learning for a real thread", () => {
  ...
  assert.match(countExtras, /\.\.\.\(hasEnabledSkills \? \["create_skill"\] : \[\]\),/);
  assert.match(countExtras, /!hasEnabledSkills &&\s*!threadId/);
  assert.doesNotMatch(countExtras, /^\s*"learn_skill",$/m);
  assert.doesNotMatch(countExtras, /^\s*"read_skill",$/m);
  assert.doesNotMatch(countExtras, /^\s*"create_skill",$/m);
});

test("request building keeps first-skill learning independent of optional tool pills", () => {
  ...
  assert.match(
    payloadBuilder,
    /supportsTools &&[\s\S]*Boolean\(resolvedThreadId\)[\s\S]*\? \{\s*enable_tools: true/,
  );
  ...
  assert.match(
    localToolCatalog,
    /\.\.\.\(resolvedThreadId\s*\? \["learn_skill", "read_skill"\]\s*: \[\]\),/,
  );
  assert.match(
    localToolCatalog,
    /\.\.\.\(hasEnabledSkills \? \["create_skill"\] : \[\]\),/,
  );
  assert.doesNotMatch(localToolCatalog, /^\s*"learn_skill",$/m);
  assert.doesNotMatch(localToolCatalog, /^\s*"read_skill",$/m);
  assert.doesNotMatch(localToolCatalog, /^\s*"create_skill",$/m);
});

test("zero-skill preflight tells the model how to bootstrap a temporary skill", () => {
  const preflight = CHAT_ADAPTER_SOURCE.slice(
    CHAT_ADAPTER_SOURCE.indexOf("async function skillPreflightBlock"),
    CHAT_ADAPTER_SOURCE.indexOf("function scheduleSelfReflect"),
  );
  assert.match(preflight, /if \(!enabled\.length\)/);
  assert.match(preflight, /call learn_skill to create a thread-scoped temporary skill/);
  assert.match(preflight, /temporary skills are promoted only after evidence of usefulness/);
});
