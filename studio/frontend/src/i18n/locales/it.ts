...
export const it = {
	...
	shell: {
		...
		navigation: {
			...
			audio: "Audio",
			trainChecking:
				"Verifica del supporto all'addestramento su questa macchina...",
		...
		selection: {
			...
			deleteDescription:
				"Eliminare {count} chat? L'operazione non è reversibile.",
	...
	settings: {
		groups: {
			...
			identityData: "Identità e dati",
			runtimeAccess: "Runtime e accesso",
			support: "Supporto",
		},
		accounts: {
			title: "Account",
			description:
				"Crea account Studio privati. I nuovi utenti accedono con un codice di configurazione monouso e scelgono una password.",
			username: "Nome utente",
			create: "Crea account",
			setupCode: "Codice di configurazione",
			setupFor: "Codice di configurazione per {username}",
			shownOnce:
				"Copia subito questo codice e condividilo con il titolare dell'account. Viene mostrato solo qui e può essere usato una sola volta entro 60 minuti.",
			expires: "Scade {expiry}",
			copy: "Copia codice di configurazione",
			copied: "Copiato",
			copyFailed:
				"Impossibile copiare. Seleziona e copia il codice di configurazione qui sopra.",
			dismiss: "Fatto",
		...
		keyboardShortcuts: {
			...
			needsModifier:
				"Aggiungi ⌘, Ctrl o Alt. Un tasto singolo intercetterebbe la digitazione.",
			conflict: "Usata anche da un'altra scorciatoia",
			conflictShadowed:
				"Un'altra scorciatoia usa questa combinazione e viene eseguita al suo posto",
			...
			actions: {
				...
				markChatUnread: {
					...
					description:
						"Segna come non lette le chat selezionate, o quella corrente",
				},
				togglePinChat: {
					label: "Fissa/sblocca",
				...
				nextChatNeedingAttention: {
					...
					description:
						"Passa alla prossima chat in generazione, in coda o non letta",
				},
				clearAllUnreads: {
		...
		debugging: {
			...
			source: "File di log",
			sourceHint:
				"I runner dei modelli scrivono i propri log, quindi un caricamento o una generazione non riusciti sono spesso spiegati lì anziché nel log del server.",
			path: "Posizione",
			...
			privacyNote:
				"In questa vista le credenziali sono mascherate. Nel file su disco non lo sono.",
			copyVisible: "Copia il log visibile",
			empty: "Non è stato ancora registrato nulla.",
			disabled:
				"La registrazione su file è disattivata (UNSLOTH_STUDIO_NO_FILE_LOG=1).",
			missing: "Nessun file di log trovato.",
			unreadable: "Non è stato possibile leggere il file di log.",
			timeout:
				"La richiesta del log e scaduta. Il server potrebbe non essere raggiungibile.",
			droppedNotice:
				"Alcune righe sono state saltate: il log è stato scritto più velocemente di quanto potesse essere letto.",
			morePending:
				"Altre righe sono ancora in lettura; arriveranno al prossimo aggiornamento.",
			staleSession:
				"La registrazione su file e disattivata, quindi questa e una sessione precedente e non verra aggiornata.",
			downloadAllLogs: "Scarica tutti i log (.zip)",
			downloadingAllLogs: "Creazione dell'archivio...",
			exportMaskedNote:
				"Nei file esportati le credenziali sono mascherate. I log molto grandi conservano solo le righe più recenti e alcuni possono essere esclusi del tutto; vedi EXPORT_WARNINGS.txt nell'archivio.",
			...
			exportTooOld:
				"Il backend Unsloth in esecuzione è troppo vecchio per esportare i log. Aggiornalo e riavvialo.",
			exportForbidden:
				"Per scaricare tutti i log serve una sessione di Studio con accesso eseguito. Una chiave API non basta.",
			keywords:
				"debug log logs registro registri errore errori crash traccia diagnostica risoluzione dei problemi",
		},
		voice: {
			title: "Voce",
			description:
			...
			dictation: {
				...
				sttDeviceAutoDescription: "Usa la GPU quando c'è, altrimenti la CPU.",
		...
		general: {
			...
			modelAutoSwitch: {
				...
				mediaEnable:
					"Cambia modello di immagini e video in base alla richiesta",
		...
		appearance: {
			...
			palette: {
				...
				minimal: "Minimale",
				glass: "Vetro neutro",
				materialPresets: "Preset materiale",
				opaque: "Opaco",
				balanced: "Bilanciato",
				airy: "Arioso",
				advancedMaterials: "Controlli materiale avanzati",
				sidebarSurface: "Barra laterale",
				chatSurface: "Superficie chat",
				composerSurface: "Compositore",
				rightRailSurface: "Pannello destro",
				mainTransparency: "Trasparenza dello sfondo principale",
				opacity: "Opacità del vetro",
				opacityDescription:
					"Quanto sono coprenti le superfici satinate. Un valore più basso le rende più trasparenti.",
				blur: "Sfocatura del vetro",
				blurDescription:
					"Intensità dell’effetto satinato su barra laterale, schede e area di composizione.",
			},
			custom: {
				reset: "Ripristina",
		...
		resources: {
			...
			gpu: {
				...
				noUsableGpu:
					"Nessuna GPU di questa macchina è utilizzabile da PyTorch.",
				mismatchCpuBuild:
			...
			llamaBackend: {
				...
				description:
					"Il backend che llama.cpp usa per eseguire i modelli GGUF.",
				...
				applyHint:
					"Scarica la nuova build e riavvia llama.cpp. Il modello caricato verrà scaricato.",
				applyHintWithSize:
					"Scarica {size} e riavvia llama.cpp. Il modello caricato verrà scaricato.",
				switchedTo: "Ora llama.cpp è in esecuzione su {backend}.",
				switchFailed: "Impossibile cambiare il backend di llama.cpp.",
				switchInterrupted:
					"Il cambio è stato interrotto prima del completamento.",
				envLocked:
					"Impostato su {backend} dalla variabile d'ambiente UNSLOTH_LLAMA_CPP_BACKEND, che ha la precedenza su questa opzione.",
				customPath: {
					...
					active:
						"Il tuo llama-server personalizzato verrà usato al prossimo caricamento del modello.",
					environmentManaged: "Gestito dalla variabile d'ambiente {variable}.",
					missingBinary:
						"llama-server non è più disponibile in questa cartella. Scegli un'altra cartella o usa il runtime incluso.",
					reloadRequired:
						"Ricarica il modello per usare il llama-server selezionato.",
					change: "Cambia",
					saving: "Salvataggio...",
				...
				unsupported: {
					notInstalled:
						"Nessuna installazione di llama.cpp gestita da Unsloth, quindi non c'è un backend da cambiare.",
					localLink:
						"llama.cpp è una cartella locale collegata da te, quindi Unsloth non la sostituirà.",
					sourceBuild:
						"Questo llama.cpp è stato compilato dai sorgenti, quindi il backend non è modificabile da qui.",
					customPath:
						"È selezionata una cartella llama.cpp personalizzata. La sua build determina il backend di calcolo.",
					unresolved:
						"Non è stato possibile controllare i backend disponibili. Verifica la connessione e riprova.",
				},
			...
			modelMemory: {
				...
				noRamReserveHint:
					"Salta il caricamento mappato in memoria sulle build Windows supportate quando il modello è interamente scaricato sulla GPU, così le sue pagine non restano residenti. Altrimenti mantiene il caricamento mappato in memoria. I buffer CPU necessari possono comunque occupare RAM. Rimuove --no-mmap e --mlock.",
				mlockVetoed:
					"--mlock resta disattivato: bloccare il modello riserverebbe RAM per l'intero modello. Lo scaricamento automatico in inattività resta disattivato.",
				memlockCapped:
					"Questo sistema limita la memoria bloccata a {limit}. Un modello più grande non verrà bloccato del tutto; aumenta il limite con ulimit -l.",
				reloadRequired:
					"Ricarica il modello per applicare le nuove opzioni di memoria.",
				loadError:
					"Impossibile caricare le impostazioni di memoria del modello",
			...
			storage: {
				...
				modelsFolderDescription: "Dove vengono salvati i modelli scaricati.",
				modelsFolderHint:
					"Dove vengono salvati i modelli scaricati. Cambialo per tenere i modelli fuori dall'unità di sistema. Vale solo per i nuovi download: i modelli che hai già restano dove sono.",
		...
		chat: {
			...
			pastedTextThresholdDescription:
				"Il testo incollato più lungo di questo valore diventa un allegato .txt invece di riempire il campo del messaggio. Premi {shortcut} per incollare comunque nel campo del messaggio.",
			...
			thinking: {
				collapseByDefault:
					"Comprimi il ragionamento per impostazione predefinita",
			...
			tools: {
				collapseByDefault:
					"Comprimi l’attività degli strumenti per impostazione predefinita",
				collapseByDefaultDescription:
			...
			importedChatCountPartial:
				"Importate {count} conversazioni in Recenti; {failed} non sono state salvate.",
		...
		data: {
			...
			archivedImagesDescription:
				"Visualizza e gestisci le immagini che hai archiviato.",
			archivedVideos: "Video archiviati",
			archivedVideosDescription:
				"Visualizza e gestisci i video che hai archiviato.",
			archivedAudio: "Audio archiviati",
			archivedAudioDescription:
				"Visualizza e gestisci le clip audio che hai archiviato.",
	...
	studio: {
		...
		params: {
			...
			doraNeedsVisionLayersOff:
				"Disattiva l'addestramento dei livelli visivi per usare DoRA",
	...
	modelMemory: {
		...
		tooLarge:
			"Più grande della VRAM, verrà scaricato sulla CPU. Una quantizzazione più piccola è più veloce",
	},
	skills: {
		title: "Competenze degli agenti",
		description:
			"Le competenze vengono rilevate nelle cartelle standard degli agenti. Attivale qui, poi digita @ nella chat per menzionarne una.",
		precedence:
			"~/.agents/skills ha la precedenza rispetto a ~/.claude/skills.",
		refresh: "Aggiorna",
		empty:
			"Nessuna competenza trovata. Aggiungi una cartella SKILL.md in ~/.agents/skills o ~/.claude/skills, poi aggiorna.",
		sourceAgents: "Agents",
		sourceClaude: "Claude",
		...
		shadowedBy:
			"Un'altra competenza ({source}) con questo nome ha la precedenza.",
		enable: "Attiva {name}",
