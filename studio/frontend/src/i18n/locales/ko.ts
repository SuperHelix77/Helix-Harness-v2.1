...
export const ko = {
	picker: {
		...
		hfToken: {
			...
			addHint: "비공개 및 접근 제한 리포지토리를 이용하려면 토큰을 설정하세요.",
		},
	},
	common: {
		cancel: "취소",
	...
	settings: {
		groups: {
			workspace: "작업 공간",
			identityData: "ID 및 데이터",
			runtimeAccess: "런타임 및 액세스",
			support: "지원",
		},
		accounts: {
			title: "계정",
			description:
				"개별 Studio 계정을 만듭니다. 새 사용자는 일회용 설정 코드로 로그인한 뒤 비밀번호를 정합니다.",
			username: "사용자 이름",
			create: "계정 만들기",
			setupCode: "설정 코드",
			setupFor: "{username}의 설정 코드",
			shownOnce:
				"지금 이 코드를 복사해 계정 소유자에게 전달하세요. 여기에서만 표시되며 60분 안에 한 번만 사용할 수 있습니다.",
			expires: "만료: {expiry}",
			copy: "설정 코드 복사",
			copied: "복사됨",
			...
			resetDescription:
				"설정 코드를 다시 생성하면 {username}의 비밀번호가 바뀌고 세션이 종료되며 API 키가 취소됩니다. 새 코드를 전달해 비밀번호를 다시 정하도록 하세요.",
			deactivate: "비활성화",
			reactivate: "다시 활성화",
			delete: "계정 삭제",
			deleteTitle: "{username}을(를) 삭제할까요?",
			deleteDescription:
				"{username}의 세션을 취소하고 진행 중인 작업을 중단합니다. 채팅, 설정, 자격 증명, 업로드, 데이터셋, 학습 실행, 출력, 내보내기, 갤러리, 샌드박스, 프로젝트, 임시 파일이 퇴역 처리됩니다. 디렉터리는 이름을 바꿔 옆으로 옮길 뿐 삭제되지 않습니다. 같은 사용자 이름을 다시 만들면 이 데이터가 없는 새 계정이 시작됩니다.",
			cancel: "취소",
			retry: "계정 새로 고침",
		...
		keyboardShortcuts: {
			...
			needsModifier:
				"⌘, Ctrl 또는 Alt를 추가하세요. 단독 키는 입력을 가로챕니다.",
			conflict: "다른 단축키도 이 조합을 사용합니다",
			conflictShadowed:
				"다른 단축키가 이 조합을 차지하고 있어 그쪽이 실행됩니다",
			edit: "단축키 변경",
			clear: "단축키 제거",
			reset: "기본값 복원",
			...
			actions: {
				...
				markChatUnread: {
					label: "읽지 않음으로 표시",
					description:
						"선택한 채팅을, 없으면 현재 채팅을 읽지 않음으로 표시합니다",
				},
				togglePinChat: {
					label: "고정 전환",
					description:
						"선택한 채팅을, 없으면 현재 채팅을 고정하거나 해제합니다",
				},
				selectAllChats: {
					label: "모든 채팅 선택",
				...
				nextChatNeedingAttention: {
					label: "확인이 필요한 다음 채팅",
					description:
						"생성 중이거나 대기 중이거나 읽지 않은 다음 채팅으로 이동합니다",
				},
				clearAllUnreads: {
					label: "모두 읽음으로 표시",
		...
		debugging: {
			...
			source: "로그 파일",
			sourceHint:
				"모델 러너는 각자 자체 로그를 기록하므로, 로드나 생성이 실패한 이유는 서버 로그가 아니라 그쪽에 남아 있는 경우가 많습니다.",
			path: "위치",
			pathCopy: "경로 복사",
			...
			refreshNow: "지금 새로 고침",
			privacyNote:
				"이 화면에서는 자격 증명이 가려집니다. 디스크에 저장된 파일에서는 가려지지 않습니다.",
			...
			timeout:
				"로그 요청 시간이 초과되었습니다. 서버에 연결할 수 없을 수 있습니다.",
			droppedNotice:
				"일부 줄이 누락되었습니다. 로그를 읽는 속도보다 기록되는 속도가 빨랐습니다.",
			morePending:
				"아직 더 많은 줄을 읽고 있습니다. 다음 새로 고침에 표시됩니다.",
			staleSession:
				"파일 로깅이 꺼져 있으므로 이것은 이전 세션이며 업데이트되지 않습니다.",
			downloadAllLogs: "모든 로그 다운로드 (.zip)",
			downloadingAllLogs: "로그를 묶는 중...",
			exportMaskedNote:
				"내보낸 파일에서는 자격 증명이 가려집니다. 매우 큰 로그는 가장 최근 줄만 남으며, 일부 로그는 아예 빠질 수 있습니다. 아카이브의 EXPORT_WARNINGS.txt를 확인하세요.",
			...
			exportTooOld:
				"실행 중인 Unsloth 백엔드가 너무 오래되어 로그를 내보낼 수 없습니다. 해당 백엔드를 업데이트한 뒤 다시 시작하세요.",
			exportForbidden:
				"모든 로그를 다운로드하려면 로그인된 Studio 세션이 필요합니다. API 키만으로는 할 수 없습니다.",
			keywords:
				"디버그 디버깅 로그 오류 에러 충돌 스택 추적 진단 문제 해결 debug log logs error",
		},
		voice: {
			title: "음성",
			...
			dictionary: {
				...
				sectionDescription:
					"받아쓰기에서 특정 단어나 문구를 어떻게 표기할지 설정합니다",
		...
		general: {
			...
			modelAutoSwitch: {
				...
				idleError:
					"모델을 로드 상태로 유지하려면 0을, 그렇지 않으면 60초 이상을 입력하세요.",
			...
			rag: {
				...
				downloadingDescription:
					"진행 상황은 다운로드 패널에 표시됩니다. 완료되면 색인에 사용됩니다.",
		...
		appearance: {
			...
			palette: {
				label: "색상 팔레트",
				description:
					"라이트 모드와 다크 모드에서 Unsloth 전반에 쓰이는 색상입니다.",
				standard: "표준",
				classic: "클래식",
				minimal: "미니멀",
				glass: "중립 유리",
				materialPresets: "재질 프리셋",
				opaque: "불투명",
				balanced: "균형",
				airy: "에어리",
				advancedMaterials: "고급 재질 제어",
				sidebarSurface: "사이드바",
				chatSurface: "채팅 표면",
				composerSurface: "작성 영역",
				rightRailSurface: "오른쪽 패널",
				mainTransparency: "주 배경 투명도",
				opacity: "유리 불투명도",
				opacityDescription:
					"반투명 표면이 얼마나 불투명한지 설정합니다. 값이 낮을수록 더 투명합니다.",
				blur: "유리 흐림",
				blurDescription: "사이드바, 카드, 입력창의 반투명 효과 강도입니다.",
			},
			custom: {
				reset: "초기화",
				resetAll: "사용자 지정 초기화",
				...
				importFont: {
					upload: "업로드",
					scanFolder: "폴더 선택",
					alreadyAvailable: "이 글꼴은 이미 있으므로 기존 사본을 사용합니다.",
		...
		resources: {
			...
			gpu: {
				...
				noUsableGpu:
					"이 컴퓨터의 GPU 중 PyTorch에서 사용할 수 있는 것이 없습니다.",
				mismatchCpuBuild:
			...
			llamaBackend: {
				...
				applying: "설치 중...",
				applyHint:
					"새 빌드를 내려받고 llama.cpp를 다시 시작합니다. 로드된 모델은 언로드됩니다.",
				applyHintWithSize:
					"{size}를 내려받고 llama.cpp를 다시 시작합니다. 로드된 모델은 언로드됩니다.",
				switchedTo: "이제 llama.cpp가 {backend}에서 실행됩니다.",
				switchFailed: "llama.cpp 백엔드를 변경하지 못했습니다.",
				switchInterrupted: "전환이 완료되기 전에 중단되었습니다.",
				envLocked:
					"환경 변수 UNSLOTH_LLAMA_CPP_BACKEND가 {backend}(으)로 고정했으며, 이 설정보다 우선합니다.",
				customPath: {
					...
					missingBinary:
						"이 폴더에서 llama-server를 더 이상 사용할 수 없습니다. 다른 폴더를 선택하거나 번들 런타임을 사용하세요.",
					reloadRequired:
						"선택한 llama-server를 사용하려면 모델을 다시 로드하세요.",
					change: "변경",
					saving: "저장 중...",
					useBundled: "번들 사용",
				...
				unsupported: {
					notInstalled:
						"관리되는 llama.cpp 설치를 찾을 수 없어 전환할 백엔드가 없습니다.",
					localLink:
						"llama.cpp는 직접 연결한 로컬 디렉터리이므로 Unsloth가 교체하지 않습니다.",
					sourceBuild:
						"이 llama.cpp는 소스에서 빌드되어 여기서 백엔드를 전환할 수 없습니다.",
					customPath:
						"사용자 지정 llama.cpp 폴더가 선택되어 있습니다. 해당 빌드가 컴퓨팅 백엔드를 결정합니다.",
					unresolved:
						"사용 가능한 백엔드를 확인하지 못했습니다. 연결을 확인한 뒤 다시 시도하세요.",
				},
			...
			modelMemory: {
				...
				keepResidentHint:
					"모델이 로드되어 있는 동안 가중치를 시스템 RAM으로 되돌리지 않습니다. 유휴 시 자동 언로드를 끄고, 가중치가 실제로 호스트 RAM에 있는 경우(통합 메모리 또는 부분 GPU 오프로드)에는 --mlock도 전달하여 운영체제가 가중치를 페이지 아웃했다가 다음 프롬프트에서 다시 업로드하지 않도록 합니다.",
				noRamReserve: "모델을 위해 시스템 RAM을 예약하지 않음",
				noRamReserveDescription: "모델 가중치가 차지하는 RAM을 줄입니다.",
				noRamReserveHint:
					"모델이 GPU에 완전히 오프로드된 경우 지원되는 Windows 빌드에서 메모리 매핑 로딩을 건너뛰어 파일 페이지가 상주하지 않도록 합니다. 그렇지 않으면 메모리 매핑 로딩을 유지합니다. 필요한 CPU 버퍼는 여전히 RAM을 사용할 수 있습니다. --no-mmap과 --mlock을 제거합니다.",
				mlockVetoed:
					"--mlock은 꺼진 상태로 유지됩니다. 모델을 고정하면 모델 전체 크기만큼 RAM을 예약하게 됩니다. 유휴 시 자동 언로드는 계속 비활성화됩니다.",
				memlockCapped:
					"이 시스템은 잠금 메모리를 {limit}로 제한합니다. 더 큰 모델은 완전히 고정되지 않습니다. ulimit -l로 한도를 늘리세요.",
			...
			storage: {
				...
				modelsFolderDescription: "다운로드한 모델이 저장되는 위치입니다.",
				modelsFolderHint:
					"다운로드한 모델이 저장되는 위치입니다. 시스템 드라이브를 차지하지 않도록 변경하세요. 새 다운로드에만 적용되며, 이미 받은 모델은 원래 위치에 그대로 남습니다.",
				openAction: "열기",
				copyAction: "경로 복사",
				copied: "경로가 복사되었습니다",
				openError: "폴더를 열지 못했습니다",
		...
		agents: {
			...
			options: {
				...
				persist: "Unsloth가 관리하는 에이전트 저장소를 실행 사이에 유지합니다.",
				asSubagent:
		...
		chat: {
			...
			pastedTextThresholdDescription:
				"이 길이를 초과한 붙여넣기 텍스트는 입력창을 채우는 대신 .txt 첨부 파일이 됩니다. {shortcut} 를 누르면 그래도 입력창에 붙여넣습니다.",
			...
			exportHistoryDescription: "모든 채팅과 메시지를 JSON으로 다운로드합니다.",
			exportAction: "내보내기",
			...
			importedChatCountPartial:
				"{count}개의 대화를 최근 항목으로 가져왔습니다. {failed}개는 저장하지 못했습니다.",
			...
			clearOneChatDescription: "이 기기의 유일한 채팅을 영구적으로 삭제합니다.",
		...
		data: {
			...
			description: "이 기기에 저장된 채팅 기록과 업로드한 파일을 관리합니다.",
		...
		apiKeys: {
			...
			codingAgentsSwap:
				"claude를 codex, openclaw, opencode 또는 hermes로 바꾸세요.",
		...
		about: {
			...
			shutDownStudioDescription: "Unsloth 서버를 중지하고 세션을 종료합니다.",
			shutDown: "종료",
			update: {
				title: "Unsloth 업데이트",
				...
				desktopReadyDescription: "새로운 데스크톱 앱 버전이 있는지 확인합니다.",
				...
				desktopCheckFailedDescription: "연결 상태를 확인한 후 다시 시도하세요.",
	...
	studio: {
		...
		wizard: {
			...
			methodTooltip:
				"모델 학습 방식입니다. LoRA와 QLoRA는 전체 가중치 대신 작은 어댑터를 업데이트합니다.",
		...
		datasetPicker: {
			...
			someLocationsUnscanned: "일부 데이터셋 위치를 스캔하지 못했습니다.",
			noLocalDatasets:
		...
		modelPicker: {
			...
			reasonAdapter: "어댑터 출력은 기본 학습 모델로 사용할 수 없습니다.",
			reasonNotTrainable: "기기에 있는 이 모델은 학습할 수 없습니다.",
			reasonUnsupportedFormat: "이 모델 형식은 학습에서 지원되지 않습니다.",
		...
		methods: {
			...
			full: {
				label: "전체 파인튜닝",
				hint: "모든 가중치를 학습합니다. 품질이 가장 높지만 VRAM이 가장 많이 필요합니다.",
				note: "fp16",
			},
			cpt: {
				label: "지속 사전학습",
		...
		params: {
			...
			rank: "랭크",
			rankTooltip: "저랭크 행렬의 차원입니다. 높을수록 용량이 커집니다.",
			alpha: "알파",
			alphaTooltip:
				"LoRA 업데이트의 스케일링 계수입니다. 보통 랭크의 2배입니다.",
			dropout: "드롭아웃",
			dropoutTooltip: "과적합을 줄이기 위한 LoRA 레이어의 드롭아웃 확률입니다.",
			visionLayers: "비전 레이어",
			...
			saveSteps: "저장 스텝",
			saveStepsTooltip:
				"N 스텝마다 체크포인트를 저장합니다. 0이면 비활성화됩니다.",
			evalSteps: "평가 스텝",
			evalStepsTooltip:
		...
		charts: {
			settings: "차트 설정",
			settingsDescription: "학습이 계속 실행되는 동안 차트 표시를 조정합니다.",
	...
	modelMemory: {
		...
		tooLarge:
			"VRAM보다 커서 CPU로 오프로드됩니다. 더 작은 양자화가 더 빠릅니다",
	},
	skills: {
		title: "에이전트 스킬",
		description:
			"스킬은 표준 에이전트 폴더에서 찾습니다. 여기서 활성화한 뒤 채팅에서 @를 입력해 언급하세요.",
		precedence: "~/.agents/skills가 ~/.claude/skills보다 우선합니다.",
		refresh: "새로 고침",
		empty:
			"에이전트 스킬이 없습니다. ~/.agents/skills 또는 ~/.claude/skills에 SKILL.md 폴더를 추가한 뒤 새로 고침하세요.",
		sourceAgents: "Agents",
		sourceClaude: "Claude",
		sourceBundled: "기본 제공",
		shadowed: "가려짐",
