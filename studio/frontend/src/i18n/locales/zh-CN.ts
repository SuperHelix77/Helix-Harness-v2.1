...
export const zhCN = {
	...
	shell: {
		...
		selection: {
			...
			deleteProjectsFilesDescription: "每个项目的工作区文件夹会从磁盘中删除。",
	...
	settings: {
		groups: {
			workspace: "工作区",
			identityData: "身份与数据",
			runtimeAccess: "运行时与访问",
			support: "支持",
		},
		accounts: {
			title: "账户",
			description:
				"创建独立的 Studio 账户。新用户使用一次性设置码登录并设置密码。",
			username: "用户名",
			create: "创建账户",
			setupCode: "设置码",
			setupFor: "{username} 的设置码",
			shownOnce:
				"请立即复制此设置码并交给账户持有人。它只在此处显示一次，60 分钟内可使用一次。",
			expires: "过期时间：{expiry}",
			copy: "复制设置码",
			copied: "已复制",
			...
			resetDescription:
				"重新生成设置码会替换 {username} 的密码、结束其会话并吊销其 API 密钥。请把新的设置码交给对方，让其重新设置密码。",
			deactivate: "停用",
			reactivate: "重新启用",
			delete: "删除账户",
			deleteTitle: "删除 {username}？",
			deleteDescription:
				"这将撤销 {username} 的会话并取消其进行中的工作。其聊天、设置、凭据、上传、数据集、训练运行、输出、导出、图库、沙箱、项目和临时文件都会被退役。目录只会重命名移开，绝不删除。再次创建该用户名将开始一个不包含这些数据的新账户。",
			cancel: "取消",
			retry: "刷新账户",
			failed: "账户请求失败。",
		},
		...
		keyboardShortcuts: {
			...
			primarySlot: "快捷键",
			alternateSlot: "备用快捷键",
			browserReserved: "浏览器可能会占用该组合键。桌面应用中可正常使用。",
			actions: {
				findInPage: {
					label: "在页面中查找",
		...
		debugging: {
			...
			source: "日志文件",
			sourceHint:
				"模型运行器会各自写入日志，因此加载或生成失败的原因通常记录在那里，而不是服务器日志中。",
			path: "位置",
			pathCopy: "复制路径",
			refreshSection: "刷新",
			mode: "模式",
			...
			exportMaskedNote:
				"导出的文件中已遮蔽凭据。超大日志仅保留最近的行，部分日志可能被完全略去；请查看压缩包中的 EXPORT_WARNINGS.txt。",
			...
			exportFailed: "无法下载日志。",
			exportTooOld:
				"正在运行的 Unsloth 后端版本过旧，无法导出日志。请更新该后端并重新启动。",
			exportForbidden:
				"下载全部日志需要已登录的 Studio 会话，仅有 API 密钥不够。",
			keywords:
				"调试 日志 错误 崩溃 堆栈 跟踪 诊断 排查 故障 debug log logs error",
		},
		voice: {
			title: "语音",
			...
			dictation: {
				...
				customModelLabel: "模型",
				customModelDescription: "发送到 /v1/audio/transcriptions 的模型名称。",
		...
		general: {
			...
			modelAutoSwitch: {
				...
				mediaIdlePaused: "当“将模型保留在 GPU 内存中”开启时暂停。",
		...
		profile: {
			...
			stats: {
				...
				emptyChats:
					"暂无聊天或 API 使用记录。请开始一段对话，或发起经过认证的本地 API 请求。",
		...
		appearance: {
			...
			palette: {
				...
				balanced: "均衡",
				airy: "通透",
				advancedMaterials: "高级材质控制",
				sidebarSurface: "侧边栏",
				chatSurface: "聊天表面",
				composerSurface: "输入框",
				rightRailSurface: "右侧面板",
				mainTransparency: "主背景透明度",
				opacity: "玻璃不透明度",
				opacityDescription: "磨砂表面的实体程度。数值越低越通透。",
				blur: "玻璃模糊",
				blurDescription: "侧边栏、卡片和输入框的磨砂强度。",
			},
			custom: {
				reset: "重置",
				resetAll: "重置自定义",
		...
		resources: {
			...
			gpu: {
				...
				mismatchCpuBuild:
					"PyTorch 是仅 CPU 版本（{version}），因此无法使用下方的 GPU。修复安装即可恢复 GPU 支持。",
				mismatchUnavailable:
					"PyTorch（{version}）无法初始化下方的 GPU，因此无法使用。请检查显卡驱动，或修复安装。",
				unusableDevice: "不可用",
			...
			llamaBackend: {
				...
				customPath: {
					...
					missingBinary:
						"此文件夹中的 llama-server 已不可用。请选择其他文件夹或使用内置运行时。",
			...
			modelMemory: {
				...
				keepResidentHint:
					"模型保持加载期间，不把权重交还给系统内存。关闭空闲自动卸载；当权重确实位于主机内存中时（统一内存，或部分卸载到 GPU），还会传入 --mlock，使操作系统无法将其换出、并在你下次提问时重新上传。",
				noRamReserve: "不为模型预留系统内存",
				noRamReserveDescription: "减少模型权重占用的内存。",
				noRamReserveHint:
					"当模型完全卸载到 GPU 时，在受支持的 Windows 版本上跳过内存映射加载，使其页面不再常驻；否则保持内存映射加载。必要的 CPU 缓冲区仍可能占用内存。移除 --no-mmap 和 --mlock。",
				mlockVetoed:
					"--mlock 保持关闭：锁定模型会为其全部内容预留内存。空闲自动卸载仍处于禁用状态。",
				memlockCapped:
					"本系统将锁定内存限制为 {limit}。更大的模型无法被完全锁定；请使用 ulimit -l 提高上限。",
			...
			storage: {
				...
				modelsFolderHint:
					"下载的模型的存放位置。可更改以避免占用系统盘。仅对新的下载生效。已有的模型仍保留在原位置。",
				openAction: "打开",
				copyAction: "复制路径",
				copied: "路径已复制",
				openError: "无法打开文件夹",
		...
		agents: {
			...
			showingModels:
				"已显示 {total} 个匹配项中的 {shown} 个。继续输入可缩小范围。",
			quantization: "量化",
			loadingQuantizations: "正在加载量化版本...",
			noQuantizations: "无单独的量化版本",
			recommended: "推荐",
			downloaded: "已下载",
			quantizationLoadError: "无法加载全部量化版本。命令将使用可用的模型值。",
		...
		chat: {
			...
			pastedTextThresholdDescription:
				"超过此长度的粘贴文本将变为 .txt 附件，而不会填满消息输入框。 按 {shortcut} 仍可粘贴到消息输入框。",
			...
			importedChatCountPartial:
				"已将 {count} 个对话导入到最近对话；{failed} 个未能保存。",
			importFailed: "导入失败。",
			clearHistory: "清除聊天记录",
		...
		remoteLan: {
			title: "远程与局域网",
			description: "通过局域网或临时公开 URL，从其他设备访问此 Unsloth。",
		},
		apiKeys: {
			title: "API",
			...
			codingAgentsSwap:
				"可将 claude 替换为 codex、openclaw、opencode 或 hermes。",
	...
	studio: {
		...
		wizard: {
			...
			methodTooltip:
				"模型的训练方式。LoRA 和 QLoRA 只更新小型适配器，而不是全部权重。",
			...
			cachedModelGoneDescription: "模型文件已不在此设备上，训练时会重新下载。",
		...
		preview: {
			...
			noticeModelDownload: "此模型尚未存储在本设备上。训练时会自动下载。",
			noticeModelPartial: "训练会先继续并完成模型下载，然后再加载模型。",
			noticeDatasetDownload: "此数据集尚未存储在本设备上。训练时会自动下载。",
			noticeDatasetPartial: "训练会先继续并完成数据集下载，然后再读取数据集。",
		...
		dataset: {
			selectors: {
				...
				manualDescription:
					"请输入要使用的准确 Hugging Face 配置名称和拆分名称。",
			...
			streaming: {
				...
				blockers: {
					source: "请使用 Hugging Face 数据集（不能是本地上传或 S3 来源）。",
					...
					appleSilicon: "Apple Silicon（MLX）暂不支持流式传输。",
				},
				options: {
				...
				notifications: {
					...
					adjusted: "已调整流式传输设置。已禁用不兼容选项：{options}。",
					needsMaxSteps:
						"流式传输需要固定的最大步数（流式数据集的长度未知）。请先将最大步数设为大于 0。",
					enabledAdjusted: "已启用流式传输。已禁用不兼容选项：{options}。",
				},
			},
			auto: "自动",
			rawText: "原始文本",
		...
		training: {
			...
			resumeUnavailable: "只有已停止或出错且已保存检查点的训练运行才能恢复。",
			chooseModel: "选择模型",
			chooseDataset: "选择数据集",
	...
	modelMemory: {
		readout: "权重 {model} + 上下文 {context} = {total}，可用显存 {budget}",
		readoutWithSpec:
	...
	skills: {
		title: "代理技能",
		description:
			"技能会从标准的代理文件夹中发现。在此启用后，在聊天中输入 @ 即可提及。",
		precedence: "~/.agents/skills 优先于 ~/.claude/skills。",
		refresh: "刷新",
		empty:
			"未找到代理技能。请在 ~/.agents/skills 或 ~/.claude/skills 下添加包含 SKILL.md 的文件夹，然后刷新。",
		sourceAgents: "Agents",
		sourceClaude: "Claude",
		sourceBundled: "内置",
		shadowed: "被覆盖",
