...
export const ru = {
	...
	settings: {
		groups: {
			...
			identityData: "Идентификация и данные",
			runtimeAccess: "Среда выполнения и доступ",
			support: "Поддержка",
		},
		accounts: {
			title: "Учётные записи",
			description:
				"Создавайте отдельные учётные записи Studio. Новые пользователи входят по одноразовому коду настройки и задают пароль.",
			username: "Имя пользователя",
			create: "Создать учётную запись",
			setupCode: "Код настройки",
			setupFor: "Код настройки для {username}",
			shownOnce:
				"Скопируйте этот код сейчас и передайте владельцу учётной записи. Он показывается только здесь и действует один раз в течение 60 минут.",
			expires: "Истекает {expiry}",
			copy: "Скопировать код настройки",
			copied: "Скопировано",
			copyFailed:
				"Не удалось скопировать. Выделите и скопируйте код настройки выше.",
			dismiss: "Готово",
			...
			resetDescription:
				"Создание нового кода настройки заменяет пароль {username}, завершает его сеансы и отзывает его ключи API. Передайте ему новый код, чтобы он снова выбрал пароль.",
			deactivate: "Деактивировать",
			reactivate: "Активировать снова",
			delete: "Удалить учётную запись",
			deleteTitle: "Удалить {username}?",
			deleteDescription:
				"Это отзывает сеансы {username} и отменяет текущую работу. Чаты, настройки, учётные данные, загрузки, наборы данных, обучения, результаты, экспорты, галереи, песочницы, проекты и временные файлы выводятся из использования. Каталоги переименовываются и откладываются, но не удаляются. Повторное создание этого имени пользователя начнёт новую учётную запись без этих данных.",
			cancel: "Отмена",
		...
		keyboardShortcuts: {
			...
			needsModifier:
				"Добавьте ⌘, Ctrl или Alt. Одиночная клавиша перехватит ввод текста.",
			conflict: "Используется также другим сочетанием",
			conflictShadowed:
				"Эта комбинация занята другим сочетанием, и выполняется оно",
			...
			actions: {
				...
				markChatUnread: {
					...
					description:
						"Отметить выбранные чаты или текущий чат как непрочитанные",
				},
				togglePinChat: {
				...
				clearChatSelection: {
					...
					description:
						"Снять выделение с выбранных чатов. Escape тоже снимает выделение",
				},
				deleteSelectedChats: {
				...
				nextChatNeedingAttention: {
					...
					description:
						"Перейти к следующему генерирующему, ожидающему или непрочитанному чату",
				},
				clearAllUnreads: {
		...
		debugging: {
			...
			source: "Файл журнала",
			sourceHint:
				"Средства запуска моделей ведут собственные журналы, поэтому причина неудачной загрузки или генерации чаще всего указана именно в них, а не в журнале сервера.",
			path: "Расположение",
			...
			privacyNote:
				"В этом окне учётные данные скрыты. В файле на диске они не скрыты.",
			copyVisible: "Копировать видимую часть журнала",
			empty: "Пока ничего не записано.",
			disabled:
				"Запись журнала в файл отключена (UNSLOTH_STUDIO_NO_FILE_LOG=1).",
			missing: "Файл журнала не найден.",
			unreadable: "Не удалось прочитать файл журнала.",
			timeout:
				"Истекло время ожидания запроса журнала. Возможно, сервер недоступен.",
			droppedNotice:
				"Часть строк пропущена: журнал записывался быстрее, чем его удавалось читать.",
			morePending:
				"Ещё строки продолжают читаться; они появятся при следующем обновлении.",
			staleSession:
				"Запись журнала в файл отключена, поэтому это предыдущий сеанс, который не будет обновляться.",
			downloadAllLogs: "Скачать все журналы (.zip)",
			downloadingAllLogs: "Упаковка журналов...",
			exportMaskedNote:
				"В экспортированных файлах учётные данные скрыты. Очень большие журналы сохраняют только самые свежие строки, а некоторые могут быть исключены целиком; смотрите EXPORT_WARNINGS.txt в архиве.",
			...
			exportTooOld:
				"Запущенный сервер Unsloth слишком старый для экспорта журналов. Обновите его и перезапустите.",
			exportForbidden:
				"Для скачивания всех журналов нужен вход в сеанс Studio. Ключа API недостаточно.",
			keywords:
				"отладка журнал журналы лог логи ошибка ошибки сбой трассировка диагностика поиск неисправностей debug log",
		},
		voice: {
			title: "Голос",
			...
			dictation: {
				...
				sttDeviceAutoDescription: "Использовать GPU, когда он есть, иначе CPU.",
		...
		general: {
			...
			passwordDialog: {
				...
				newTooShort:
					"Новый пароль должен содержать не менее {minLength} символов.",
			...
			modelAutoSwitch: {
				...
				idleError:
					"Введите 0, чтобы модель оставалась загруженной, или не менее 60 секунд.",
		...
		appearance: {
			...
			palette: {
				...
				glass: "Нейтральное стекло",
				materialPresets: "Предустановки материала",
				opaque: "Непрозрачный",
				balanced: "Сбалансированный",
				airy: "Воздушный",
				advancedMaterials: "Расширенные настройки материала",
				sidebarSurface: "Боковая панель",
				chatSurface: "Поверхность чата",
				composerSurface: "Поле ввода",
				rightRailSurface: "Правая панель",
				mainTransparency: "Прозрачность основного фона",
				opacity: "Непрозрачность стекла",
				opacityDescription:
					"Степень плотности матовых поверхностей. Чем меньше значение, тем они прозрачнее.",
				blur: "Размытие стекла",
				blurDescription:
					"Сила матового эффекта на боковой панели, карточках и поле ввода.",
			},
			custom: {
				reset: "Сбросить",
				...
				reduceMotion: {
					...
					description:
						"Уменьшить анимацию или использовать системную настройку.",
					system: "Системная",
					on: "Вкл.",
					off: "Выкл.",
				},
		...
		resources: {
			...
			llamaBackend: {
				...
				applying: "Установка...",
				applyHint:
					"Скачает новую сборку и перезапустит llama.cpp. Загруженная модель будет выгружена.",
				applyHintWithSize:
					"Скачает {size} и перезапустит llama.cpp. Загруженная модель будет выгружена.",
				...
				envLocked:
					"Значение {backend} задано переменной окружения UNSLOTH_LLAMA_CPP_BACKEND, она важнее этой настройки.",
				customPath: {
					...
					active:
						"Ваш llama-server будет использован при следующей загрузке модели.",
					environmentManaged: "Управляется переменной окружения {variable}.",
					missingBinary:
						"llama-server больше недоступен в этой папке. Выберите другую папку или используйте комплектную среду.",
					reloadRequired:
						"Перезагрузите модель, чтобы использовать выбранный llama-server.",
					change: "Изменить",
					saving: "Сохранение...",
				...
				unsupported: {
					notInstalled:
						"Управляемая установка llama.cpp не найдена, поэтому менять нечего.",
					localLink:
						"llama.cpp, это локальный каталог, который вы подключили сами, и Unsloth его не заменяет.",
					sourceBuild:
						"Этот llama.cpp собран из исходников, его бэкенд нельзя переключить отсюда.",
					customPath:
						"Выбрана пользовательская папка llama.cpp. Её сборка определяет вычислительный бэкенд.",
					unresolved:
						"Не удалось проверить доступные бэкенды. Проверьте подключение и повторите попытку.",
				},
			...
			modelMemory: {
				...
				keepResidentHint:
					"Веса не возвращаются в системную ОЗУ, пока модель загружена. Отключает автовыгрузку по простою, а если веса действительно находятся в ОЗУ хоста (единая память или частичная выгрузка на GPU), дополнительно передаёт --mlock, чтобы ОС не вытесняла их и не загружала заново при следующем запросе.",
				noRamReserve: "Не резервировать системную ОЗУ под модель",
				noRamReserveDescription: "Уменьшает объём ОЗУ, занятый весами модели.",
				noRamReserveHint:
					"Пропускает загрузку через отображение файла в память в поддерживаемых сборках Windows, когда модель полностью выгружена на видеокарту, чтобы её страницы не оставались в памяти. В остальных случаях сохраняет загрузку через отображение файла в память. Необходимые буферы ЦП всё равно могут занимать ОЗУ. Убирает --no-mmap и --mlock.",
				mlockVetoed:
					"--mlock остаётся выключенным: закрепление модели зарезервировало бы ОЗУ под неё целиком. Автовыгрузка по простою по-прежнему отключена.",
				memlockCapped:
					"Система ограничивает блокируемую память значением {limit}. Модель большего размера не будет закреплена полностью; увеличьте лимит через ulimit -l.",
				reloadRequired:
					"Перезагрузите модель, чтобы применить новые параметры памяти.",
			...
			storage: {
				...
				modelsFolderDescription: "Где хранятся загруженные модели.",
				modelsFolderHint:
					"Где хранятся загруженные модели. Измените путь, чтобы модели не занимали системный диск. Применяется только к новым загрузкам. Уже имеющиеся модели останутся на прежнем месте.",
				openAction: "Открыть",
				copyAction: "Копировать путь",
				copied: "Путь скопирован",
				openError: "Не удалось открыть папку",
		...
		agents: {
			...
			options: {
				...
				yolo: "Пропускает запросы подтверждения. Используйте только в доверенных средах.",
			},
			remote: {
		...
		chat: {
			...
			pastedTextThresholdDescription:
				"Вставленный текст длиннее этого значения станет вложением .txt вместо заполнения поля сообщения. Нажмите {shortcut}, чтобы всё равно вставить в поле сообщения.",
			...
			artifacts: {
				...
				blockedBannerPlural:
					"Заблокировано внешних ресурсов: {count} с {hosts}.",
			...
			importedChatCountPartial:
				"Импортировано {count} бесед в Недавние; {failed} не удалось сохранить.",
			...
			clearAllChatsDescription:
				"Безвозвратно удалить все чаты на этом устройстве.",
			...
			oneChatClearedRemainOne:
				"Очищен 1 чат; остался 1 чат. Повторите попытку.",
		...
		data: {
			...
			archivedImagesDescription:
				"Просмотр и управление изображениями, которые вы архивировали.",
			archivedVideos: "Архивные видео",
			archivedVideosDescription:
				"Просмотр и управление видео, которые вы архивировали.",
			archivedAudio: "Архивированные аудио",
			archivedAudioDescription:
				"Просмотр и управление аудиоклипами, которые вы архивировали.",
			...
			fineTuneTrainFailed: "Не удалось загрузить датасет во вкладку «Обучение»",
		},
		connections: {
			title: "Подключения",
		...
		apiKeys: {
			...
			codingAgentsSwap:
				"Замените claude на codex, openclaw, opencode или hermes.",
		...
		about: {
			...
			update: {
				...
				desktopAvailable: "Доступна версия {version} настольного приложения",
				...
				desktopCurrent: "Установлена последняя версия настольного приложения",
	...
	studio: {
		...
		wizard: {
			...
			methodTooltip:
				"Как обучается модель. LoRA и QLoRA обновляют небольшие адаптеры вместо всех весов.",
		...
		methods: {
			...
			cpt: {
				...
				hint: "Продолженное предобучение для новых предметных областей или языков.",
				note: "продолженное",
			},
		},
		subtitles: {
		...
		dataset: {
			...
			streaming: {
				...
				blockers: {
					...
					trainOnCompletions: "Отключите «Только ответы ассистента».",
					evalSplit:
						"Выберите отдельную выборку для оценки — оценка включена, но отдельная выборка не задана.",
					visionModel: "Модели зрения не поддерживают потоковую передачу.",
					audioModel: "Аудиомодели не поддерживают потоковую передачу.",
					embeddingModel:
		...
		params: {
			...
			alpha: "Alpha",
			alphaTooltip:
				"Коэффициент масштабирования обновлений LoRA. Обычно 2× ранга.",
			dropout: "Dropout",
			dropoutTooltip:
			...
			doraNeedsVisionLayersOff:
				"Отключите обучение слоёв зрения, чтобы использовать DoRA",
			...
			gradAccumTooltip:
				"Имитирует больший размер батча без дополнительного VRAM.",
		...
		history: {
			...
			deleteArtifactsFailed: "Запуск удалён, но удалить его файлы не удалось.",
		...
		charts: {
			...
			viewWindowDescription:
				"Показывать только последние шаги или всю историю.",
			window: "Окно",
			all: "Все",
			...
			smoothingDescription:
				"Двигайте вправо для большего сглаживания. `0` = без обработки.",
	...
	modelMemory: {
		...
		tooLarge:
			"Больше объёма VRAM, часть уйдёт на CPU. Меньшая квантизация работает быстрее",
	},
	skills: {
		title: "Навыки агентов",
		description:
			"Навыки находятся в стандартных папках агентов. Включите их здесь, затем введите @ в чате, чтобы упомянуть навык.",
		precedence: "~/.agents/skills имеет приоритет над ~/.claude/skills.",
		refresh: "Обновить",
		empty:
			"Навыки не найдены. Добавьте папку с SKILL.md в ~/.agents/skills или ~/.claude/skills и обновите.",
		sourceAgents: "Agents",
		sourceClaude: "Claude",
