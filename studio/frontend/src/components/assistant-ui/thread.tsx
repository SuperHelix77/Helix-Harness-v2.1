...
} from "@/features/chat/prompt-storage/prompt-storage";
import {
  listPromptEntries,
  type PromptEntry,
...
import {
  ...
  forgetAutoContinue,
  recordAutoContinue,
  shouldAdaptiveAutoContinueMessage,
...
const DeepResearchWebsiteAccessDialog = reactLazy(() =>
  import("@/features/chat/components/deep-research-website-access-dialog").then(
    (module) => ({ default: module.DeepResearchWebsiteAccessDialog }),
  ),
);
    ...
    <div className="aui-thread-welcome-root mx-auto my-auto flex w-full max-w-(--thread-max-width) grow flex-col">
      <div className="aui-thread-welcome-center flex w-full grow flex-col items-center justify-start pt-[27.5dvh]">
        ...
        <div className="aui-thread-welcome-message flex w-full flex-col justify-center gap-9 px-4">
          ...
          <div className="unsloth-welcome-greeting flex flex-col items-center justify-center gap-2">
            <h1 className="aui-thread-welcome-message-inner unsloth-welcome-title fade-in slide-in-from-bottom-1 animate-in text-center font-heading text-3xl font-semibold tracking-[-0.03em] duration-200">
...
const Composer: FC<{
  ...
  const composerContent = (
    <>
      ...
      {researchWebsiteAccessOpen && effectiveDeepResearchEnabled ? (
...
const ContinueMessageBarForLastMessage: FC = () => {
  ...
  const adaptiveCheckpoint = useMemo(
    () => readAdaptiveCheckpointMetadata(metadata),
    [metadata],
  );
  const cancelled =
        ...
        custom: {
          [CONTINUATION_RUN_CONFIG_KEY]: {
            ...
            ...(adaptiveCheckpoint
              ? {
                  adaptiveResumeRound:
                    nextAdaptiveResumeRound(adaptiveCheckpoint),
                }
              : {}),
          },
        },
  ...
  }, [adaptiveCheckpoint, aui, messageId, partial, thoughtSignature]);
