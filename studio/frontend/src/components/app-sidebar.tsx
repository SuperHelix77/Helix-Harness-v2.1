...
import {
	deleteTrainingRun,
	renameTrainingRun,
} from "@/features/training/api/history-api";
import {
	emitTrainingRunDeleted,
	emitTrainingRunUpdated,
} from "@/features/training/events";
import { useTrainingCompletionWatch } from "@/features/training/hooks/use-training-completion-watch";
...
import { getTrainingRunDisplayTitle } from "@/features/training/lib/run-display";
import {
	isTrainingStartPending,
	useTrainingRuntimeStore,
} from "@/features/training/stores/training-runtime-store";
			...
			<div className="rounded-[14px] border border-border/60 bg-transparent px-2 py-2 group-data-[collapsible=icon]:mx-auto group-data-[collapsible=icon]:size-[34px] group-data-[collapsible=icon]:overflow-hidden group-data-[collapsible=icon]:p-0">
				<div className="flex items-center gap-2 group-data-[collapsible=icon]:h-full group-data-[collapsible=icon]:justify-center">
					<HugeiconsIcon
						icon={ZapIcon}
						strokeWidth={1.75}
						className="size-[19px] shrink-0 text-nav-fg"
					/>
					<div className="min-w-0 flex-1 group-data-[collapsible=icon]:hidden">
						<span className="block truncate text-ui-12 font-semibold text-nav-fg">
							Learning
						</span>
...
export function AppSidebar() {
	...
	const sidebarNav = useAppearanceCustomStore(
		...
		(item) =>
			item.id !== "train" && item.id !== "images" && item.id !== "video",
	);
	...
	const rowIdByThreadId = useMemo(() => {
		...
		for (const item of allChatItems) {
			for (const threadId of getSidebarItemThreadIds(item))
				map[threadId] = item.id;
		}
		return map;
	}, [allChatItems]);
	...
	const setSelectionActive = useChatNavigationStore(
		(s) => s.setSelectionActive,
	);
	useEffect(() => {
	...
	const walkRecentlyViewed = (delta: number) =>
		openChatItemById(
			useChatNavigationStore.getState().stepRecentlyViewed(delta),
		);
										...
										onSelect={() => {
											void (async () => {
												try {
													...
													if (distinct.length > 1) {
														toast.error(
															"This chat wrote to more than one folder.",
															{
																description:
							...
							>
								...
								>
									<img
										...
										alt=""
										aria-hidden="true"
										className="helix-logo size-[22px] shrink-0"
									/>
									<span className="helix-wordmark relative -top-px truncate font-heading text-[calc(13px+0.5rem*var(--ui-font-scale,1))] font-semibold tracking-[0.06em] leading-tight">
										HELIX HARNESS
									</span>
								</Link>
            ...
            travel in this band and a full-width gradient washed it out. */}
              ...
              sits above the avatar instead. */}
						...
						<SidebarMenuItem className="relative">
							<SidebarMenuButton
								size="lg"
								isActive={pathname === "/engine"}
								aria-label="Helix Engine"
								onClick={() => {
									void navigate({ to: "/engine" });
									closeMobileIfOpen();
								}}
							...
							>
								<div className="hidden size-[32px] shrink-0 items-center justify-center group-data-[collapsible=icon]:flex group-data-[collapsible=icon]:size-full">
									<img
										src="/helix-mark.svg"
										alt=""
										aria-hidden="true"
										className="helix-logo size-[21px]"
									/>
								</div>
								<div className="flex min-w-0 flex-1 flex-col gap-px leading-tight group-data-[collapsible=icon]:hidden">
									<span className="helix-ink truncate font-heading text-ui-13p5 tracking-[0.08em] font-semibold">
										HELIX ENGINE
									</span>
									<span className="truncate text-ui-11p5 tracking-nav text-muted-foreground">
										in-app control plane
									</span>
								</div>
							</SidebarMenuButton>
							<NonModalDropdownMenu
