...
export function parseUnslothDeepLink(
  ...
  const acceptedTargets = new Set([
    "helixharness://open_from_hf",
    "helixharness://open_from_hf/",
    // Keep parsing legacy intents that can still arrive from an older registered build.
    "unsloth://open_from_hf",
    "unsloth://open_from_hf/",
  ]);
  if (
    !acceptedTargets.has(target)
  ) {
    return null;
  }
  ...
  if (
    (url.protocol !== "helixharness:" && url.protocol !== "unsloth:") ||
