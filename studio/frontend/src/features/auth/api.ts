...
import {
  clearAuthTokens,
  clearDesktopRefreshCredential,
  getAuthToken,
  getRefreshToken,
  mustChangePassword,
  setMustChangePassword,
  storeDesktopAccessToken,
  storeAuthTokens,
} from "./session";
...
export async function refreshSession(): Promise<boolean> {
  if (isTauri) {
    if (refreshInflight && refreshInflightToken === "__tauri_native__") {
    ...
    const promise = (async () => {
      try {
        ...
        const payload = await invoke<{
          access_token: string;
          must_change_password: boolean;
        }>("desktop_refresh_auth");
        if (startGeneration !== logoutGeneration) return false;
        storeDesktopAccessToken(payload.access_token, true);
    ...
    refreshInflightToken = "__tauri_native__";
    try {
...
export async function logout(): Promise<void> {
  ...
  try {
    ...
    if (response && response.status === 401 && (isTauri || getRefreshToken())) {
      const refreshed = await refreshSession();
      if (refreshed) response = await postLogout(getAuthToken());
    }
  } finally {
    logoutGeneration += 1;
    await clearDesktopRefreshCredential();
    clearAuthTokens();
  }
}
