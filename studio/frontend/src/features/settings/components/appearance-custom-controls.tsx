...
> = {
	...
	glass: {
		light: { accent: "#42677b", background: "#f4f6f7", foreground: "#172027" },
		dark: { accent: "#9fc7d8", background: "#182027", foreground: "#f4f7f8" },
	},
};
...
function TransparencySliderRow({
	label,
	value,
	range,
	onChange,
	...
	return (
		<div className="flex flex-col gap-3">
			<TransparencySliderRow
				label={t("settings.appearance.palette.mainTransparency")}
				value={glass.mainBackgroundTransparency}
				range={MAIN_BACKGROUND_TRANSPARENCY_RANGE}
				onChange={glass.setMainBackgroundTransparency}
			/>
			{palette === "glass" ? (
				<>
					<div
						className="flex flex-wrap gap-2 pt-1"
						aria-label={t("settings.appearance.palette.materialPresets")}
					>
						<Button
							type="button"
							variant="outline"
							...
							onClick={() => {
								glass.setMainBackgroundTransparency(0);
								glass.setSidebarTransparency(0);
								glass.setChatSurfaceTransparency(0);
								glass.setComposerTransparency(0);
								glass.setRightRailTransparency(0);
								glass.setBlur(0);
							}}
						>
							{t("settings.appearance.palette.opaque")}
						</Button>
						<Button
							type="button"
							variant="outline"
							...
							className="h-7 rounded-full px-3 text-xs"
							onClick={() => {
								glass.setMainBackgroundTransparency(34);
								glass.setSidebarTransparency(46);
								glass.setChatSurfaceTransparency(72);
								glass.setComposerTransparency(18);
								glass.setRightRailTransparency(38);
								glass.setBlur(28);
							}}
						>
							{t("settings.appearance.palette.balanced")}
						</Button>
						<Button
							type="button"
							variant="outline"
							...
							className="h-7 rounded-full px-3 text-xs"
							onClick={() => {
								glass.setMainBackgroundTransparency(62);
								glass.setSidebarTransparency(58);
								glass.setChatSurfaceTransparency(84);
								glass.setComposerTransparency(32);
								glass.setRightRailTransparency(52);
								glass.setBlur(34);
							}}
						>
							{t("settings.appearance.palette.airy")}
						</Button>
					</div>
					<div className="flex w-full items-center gap-3">
						<span className="w-28 shrink-0 text-xs text-muted-foreground">
							{t("settings.appearance.palette.blur")}
						</span>
						<Slider
							value={[glass.blur]}
							min={GLASS_BLUR_RANGE.min}
							max={GLASS_BLUR_RANGE.max}
							step={1}
							onValueChange={(values: number[]) => glass.setBlur(values[0])}
							aria-label={t("settings.appearance.palette.blur")}
						/>
						<span className="w-8 text-right text-xs tabular-nums text-muted-foreground">
							{glass.blur}
						</span>
					...
					<details className="group rounded-xl border border-border/60 px-3 py-2">
						...
						<div className="mt-3 flex flex-col gap-3">
							<TransparencySliderRow
								label={t("settings.appearance.palette.sidebarSurface")}
								value={glass.sidebarTransparency}
								range={SIDEBAR_TRANSPARENCY_RANGE}
								onChange={glass.setSidebarTransparency}
							/>
							<TransparencySliderRow
								label={t("settings.appearance.palette.chatSurface")}
								value={glass.chatSurfaceTransparency}
								range={CHAT_SURFACE_TRANSPARENCY_RANGE}
								onChange={glass.setChatSurfaceTransparency}
							/>
							<TransparencySliderRow
								label={t("settings.appearance.palette.composerSurface")}
								value={glass.composerTransparency}
								range={COMPOSER_TRANSPARENCY_RANGE}
								onChange={glass.setComposerTransparency}
							/>
							<TransparencySliderRow
								label={t("settings.appearance.palette.rightRailSurface")}
								value={glass.rightRailTransparency}
								range={RIGHT_RAIL_TRANSPARENCY_RANGE}
								onChange={glass.setRightRailTransparency}
							/>
							<div className="flex w-full items-center gap-3">
								<span className="w-28 shrink-0 text-xs text-muted-foreground">
								...
								<Slider
									value={[glass.opacity]}
									min={GLASS_OPACITY_RANGE.min}
									max={GLASS_OPACITY_RANGE.max}
									step={2}
									onValueChange={(values: number[]) =>
										glass.setOpacity(values[0])
									}
									aria-label={t("settings.appearance.palette.opacity")}
								/>
								<span className="w-8 text-right text-xs tabular-nums text-muted-foreground">
									{glass.opacity}
								</span>
