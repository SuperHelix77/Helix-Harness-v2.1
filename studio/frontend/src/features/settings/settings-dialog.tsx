 ...
 * the whole application rather than one panel.
 *
...
const SETTINGS_GROUP_LABELS: Record<SettingsGroup, TranslationKey> = {
	Workspace: "settings.groups.workspace",
	"Identity & data": "settings.groups.identityData",
	"Runtime & access": "settings.groups.runtimeAccess",
	Support: "settings.groups.support",
};
...
export function SettingsDialog() {
	...
	const visibleTabs = useMemo(
		() => TABS.filter((tab) => settingsTabVisible(tab.id, isOwner)),
		[isOwner],
	);
			...
			.map((tab) => {
				...
				const entries = SETTINGS_SEARCH_INDEX[tab.id]
					.filter((key) => {
						...
						return keywordsKey
							? t(keywordsKey).toLowerCase().includes(q)
							: false;
					})
					.map((key) => t(key));
			...
			<Dialog open={open} onOpenChange={(o) => !o && closeDialog()}>
                ...
                page fill, so both track the active palette. */}
						<aside className="font-heading flex w-[248px] shrink-0 flex-col border-r border-sidebar-border bg-sidebar text-sidebar-foreground p-2 dark:border-r-0 max-sm:w-full max-sm:border-r-0 max-sm:border-b max-sm:border-sidebar-border">
							<div className="relative mx-1 mt-3 mb-2 shrink-0">
								<HugeiconsIcon
									icon={Search01Icon}
									strokeWidth={2}
							...
							{results ? (
								<div className="hover-scrollbar flex min-h-0 flex-1 flex-col gap-1 overflow-y-auto px-1 py-1 max-sm:max-h-[40dvh]">
									{results.length === 0 ? (
							...
							>
								{visibleTabs.map((tab, index) => {
									...
									return (
										<Fragment key={tab.id}>
											{showGroup ? (
												<p className="mt-2 mb-1 px-3 text-[10px] font-semibold uppercase tracking-[0.12em] text-muted-foreground/70 first:mt-0 max-sm:hidden">
													{t(SETTINGS_GROUP_LABELS[tab.group])}
												</p>
											) : null}
											<button
