...
function useLiveUsageEstimated(): boolean {
  return useChatRuntimeStore((store) => {
    const key = store.activeThreadId ?? "__default";
    return store.liveContextUsageByThreadId[key] != null;
  });
}
...
> = ({
  ...
  const estimated = useLiveUsageEstimated();
        ...
        <div className="grid min-w-44 gap-1.5 text-xs">
          {state.estimated ? (
            <div className="flex items-center justify-between gap-4">
              <span className="text-muted-foreground">Usage source</span>
              <span className="font-medium">Live estimate</span>
            </div>
          ) : null}
          {percent !== null ? (
            <div className="flex items-center justify-between gap-4">
