...
export type ContinuationRequest = {
  ...
  /** Present only when this sibling resumes a backend-declared adaptive checkpoint. Kept
   *  separate from the ordinary Max Tokens counter so AUTO_CONTINUE_LIMIT remains exact. */
  adaptiveResumeRound?: number;
};
...
export type AdaptiveCheckpointMetadata = AdaptiveCheckpointControl & {
  /** Number of adaptive sibling resumes already started for this logical answer. */
  resumeRound: number;
  /** `pending` alone blocks the sibling: completed and fail-open learning failures may resume. */
  checkpointStatus: "pending" | "completed" | "failed";
  checkpointAvailable?: boolean;
  trainingDeferred?: boolean;
  checkpointError?: string;
};

/** Read the backend-authoritative adaptive marker off an assistant message. A plain
 * `finish_reason: "length"` is intentionally insufficient. */
export function readAdaptiveCheckpointMetadata(
  metadata: unknown,
): AdaptiveCheckpointMetadata | null {
  const custom = (metadata as { custom?: Record<string, unknown> } | undefined)
    ?.custom;
  const value = custom?.adaptiveCheckpoint;
  if (!value || typeof value !== "object" || Array.isArray(value)) {
    return null;
  }
  const candidate = value as Partial<AdaptiveCheckpointMetadata>;
  if (
    candidate.reason !== "context_ratio" ||
    typeof candidate.ratio !== "number" ||
    typeof candidate.context_length !== "number" ||
    typeof candidate.trigger_tokens !== "number" ||
    typeof candidate.prompt_tokens !== "number" ||
    typeof candidate.completion_tokens !== "number" ||
    typeof candidate.occupancy_tokens !== "number" ||
    typeof candidate.segment_max_tokens !== "number" ||
    typeof candidate.resumeRound !== "number" ||
    !Number.isInteger(candidate.resumeRound) ||
    candidate.resumeRound < 0 ||
    (candidate.checkpointStatus !== "pending" &&
      candidate.checkpointStatus !== "completed" &&
      candidate.checkpointStatus !== "failed")
  ) {
    return null;
  }
  return candidate as AdaptiveCheckpointMetadata;
}

/** Adaptive segment stops are deliberately allowed more rounds than ordinary Max Tokens cuts,
 *  but remain bounded against a pathological backend/model loop. This counter is carried in
 *  message/run metadata, so reloads and sibling continuation messages do not reset it. */
export const ADAPTIVE_AUTO_CONTINUE_LIMIT = 12;

export function shouldAdaptiveAutoContinue(
  checkpoint: AdaptiveCheckpointMetadata | null | undefined,
  reason: IncompleteReason | null | undefined,
  key: string | null | undefined,
  limit = ADAPTIVE_AUTO_CONTINUE_LIMIT,
): boolean {
  if (
    reason !== "length" ||
    !checkpoint ||
    ...
    checkpoint.checkpointError === "checkpoint_cancelled"
  ) {
    return false;
  }
  return checkpoint.resumeRound < limit;
}

export function nextAdaptiveResumeRound(
  checkpoint: AdaptiveCheckpointMetadata,
): number {
  return checkpoint.resumeRound + 1;
}
...
/** Adaptive twin of `shouldAutoContinueMessage`: the cross-tab claim is still per message,
 *  while the bounded adaptive round lives in durable message metadata. */
export function shouldAdaptiveAutoContinueMessage(
  messageId: string | null | undefined,
  checkpoint: AdaptiveCheckpointMetadata | null | undefined,
  reason: IncompleteReason | null | undefined,
  key: string | null | undefined,
  ...
  return shouldAdaptiveAutoContinue(checkpoint, reason, key, limit);
}
