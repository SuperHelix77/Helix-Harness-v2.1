...
export function mergeBackendRecommendedInference({
  ...
  const defaultMaxTokens = recommendedDefaultMaxTokens(
    loadedContextLength,
    current.maxTokens,
  );
  return {
...
/** Raise the 8192 default when a large GGUF window loaded. Never lower an explicit pin. */
export function recommendedDefaultMaxTokens(
  loadedContextLength: number | null,
  currentMaxTokens: number,
): number {
  const ceiling = localMaxTokensCeiling(
    loadedContextLength,
    currentMaxTokens,
  );
  const clamped = Math.min(currentMaxTokens, ceiling);
  if (currentMaxTokens !== defaultInferenceParams.maxTokens) {
    return Math.max(MAX_TOKENS_MIN, clamped);
  }
  if (loadedContextLength == null || loadedContextLength <= 0) {
    return Math.max(MAX_TOKENS_MIN, clamped);
  }
  const bumped = Math.min(16_384, Math.floor(loadedContextLength / 4));
  if (bumped <= currentMaxTokens) {
    return Math.max(MAX_TOKENS_MIN, clamped);
  }
  return Math.max(MAX_TOKENS_MIN, Math.min(ceiling, bumped));
}
