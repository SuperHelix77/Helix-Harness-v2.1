...
export interface OpenAIChatCompletionsRequest {
  ...
  /** Studio continuation bookkeeping used only to keep the adaptive auto-resume budget durable. */
  adaptive_resume_round?: number;
  /** Stable logical-turn key persisted inside durable chat-run request payloads. It lets reload
   * finalization distinguish new keyed runs from older durable runs whose foreground learning was
   * unkeyed and therefore must not be replayed under a new key. */
  finalization_idempotency_key?: string;
...
/** Studio-only control frame emitted immediately before a public `finish_reason: "length"`
 *  when the backend intentionally stops a decode segment at the adaptive context boundary. */
export interface AdaptiveCheckpointControl {
...
export interface OpenAIChatChunk {
  ...
  /** Internal Studio control marker. Public OpenAI-compatible consumers never receive this shape. */
  _adaptiveCheckpoint?: AdaptiveCheckpointControl;
  /** Added by the durable Studio follower from the persisted event row. */
  _adaptiveCheckpointEventSeq?: number;
  _adaptiveCheckpointRunId?: string;
  usage?: {
    prompt_tokens: number;
    completion_tokens: number;
    total_tokens: number;
