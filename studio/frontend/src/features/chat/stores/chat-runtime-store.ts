...
type ChatRuntimeStore = {
  ...
  /** Stream-time estimates are deliberately separate from exact terminal usage. A background
   *  run may advance its own entry, but only the visible thread may project one into
   *  `contextUsage`. Terminal server usage clears the matching transient entry. */
  liveContextUsageByThreadId: Record<string, ContextUsageSnapshot>;
  ...
  /** Best-effort usage while a reply is streaming. Never persisted as exact history metadata. */
  setThreadLiveContextUsage: (
    threadId: string,
...
export const useChatRuntimeStore = create<ChatRuntimeStore>((set, get) => ({
  ...
  setThreadRunning: (threadId, running, options) =>
    set((state) => {
      ...
      let nextLiveUsage = state.liveContextUsageByThreadId;
      let visibleUsage = state.contextUsage;
      const visible =
        threadId === "__default"
          ? state.activeThreadId == null
          : state.activeThreadId === threadId;
      const clearLiveUsage = () => {
        if (!(threadId in nextLiveUsage)) return;
        nextLiveUsage = { ...nextLiveUsage };
        delete nextLiveUsage[threadId];
        if (visible) {
          visibleUsage =
            threadId === "__default"
              ? null
              : (state.contextUsageByThreadId[threadId] ?? null);
        }
      };
      if (running) {
        // A fresh lifecycle may not inherit an interrupted run's estimate. Exact per-thread usage
        // remains available as the baseline until this run publishes its first live estimate.
        if (owners.length === 0) clearLiveUsage();
        next[threadId] = true;
        if (options?.owner) {
      ...
      return {
        ...
        ...(nextLiveUsage !== state.liveContextUsageByThreadId
          ? { liveContextUsageByThreadId: nextLiveUsage }
          : {}),
        ...(visibleUsage !== state.contextUsage
          ? { contextUsage: visibleUsage }
          : {}),
      };
    }),
  adoptDefaultThreadRun: (threadId) =>
    set((state) => {
      ...
      move(state.liveContextUsageByThreadId, "liveContextUsageByThreadId");
      move(
  ...
  setActiveThreadId: (activeThreadId) =>
    set((state) => ({
      ...
      contextUsage: activeThreadId
        ? (state.liveContextUsageByThreadId[activeThreadId] ??
          state.contextUsageByThreadId[activeThreadId] ??
          null)
        : null,
    })),
  ...
  syncToolConfirmationsForRun: (runId, confirmations) =>
    set((state) => {
      const desired = new Map(
        confirmations.map((confirmation) => [
          confirmation.toolCallId,
          {
            ...
            autoAllowKey: confirmation.autoAllowKey,
            runId,
          },
        ]),
      );
      const current = Object.entries(state.toolConfirmations).filter(
        ([, confirmation]) => confirmation.runId === runId,
      );
      if (
        current.length === desired.size &&
        current.every(([toolCallId, confirmation]) => {
          const expected = desired.get(toolCallId);
          return (
            expected?.approvalId === confirmation.approvalId &&
            expected.sessionId === confirmation.sessionId &&
            expected.autoAllowKey === confirmation.autoAllowKey
          );
        })
      ) {
        return state;
      ...
      const next = { ...state.toolConfirmations };
      for (const [toolCallId, confirmation] of Object.entries(next)) {
        if (confirmation.runId === runId) delete next[toolCallId];
      }
      for (const confirmation of confirmations) {
        next[confirmation.toolCallId] = {
          approvalId: confirmation.approvalId,
          sessionId: confirmation.sessionId,
          autoAllowKey: confirmation.autoAllowKey,
          runId,
  ...
  setContextUsage: (contextUsage) =>
    set((state) => {
      if (!state.activeThreadId) {
        const live = { ...state.liveContextUsageByThreadId };
        delete live.__default;
        return {
          contextUsage,
          liveContextUsageByThreadId: live,
        };
      }
      ...
      delete live[state.activeThreadId];
      return {
        contextUsage,
        contextUsageByThreadId: next,
        liveContextUsageByThreadId: live,
      };
    }),
  setThreadContextUsage: (threadId, usage) =>
    set((state) => {
      ...
      return {
        ...
        liveContextUsageByThreadId: live,
        ...(state.activeThreadId === threadId ? { contextUsage: usage } : {}),
      };
    }),
  setThreadLiveContextUsage: (threadId, usage, options) =>
    set((state) => {
      const live = { ...state.liveContextUsageByThreadId };
      if (usage) {
        live[threadId] = usage;
