...
export type ContextUsageBarInput = {
  ...
  /** True while the current total is a stream-time estimate rather than terminal server usage. */
  estimated?: boolean;
...
export function deriveContextUsageBar({
  ...
  return {
    ...
    label: `${estimated ? "Estimated " : ""}Context usage: ${formatTokenCount(usedTokens)} of ${formatTokenCount(limit)} tokens`,
    totalRowName: "Total",
