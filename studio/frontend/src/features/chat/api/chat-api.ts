...
/** Remove an empty assistant placeholder that never started generation. */
export async function deleteOrphanAssistantMessage(
  threadId: string,
  messageId: string,
): Promise<void> {
  const response = await authFetch(
  ...
  if (!response.ok) {
    throw new Error(`Failed to delete orphan assistant (${response.status})`);
  }
}
  ...
  try {
    while (true) {
      ...
      while (separatorIndex >= 0) {
        ...
        // Studio-only adaptive boundary. This is deliberately NOT a terminal signal: the
        // following public OpenAI chunk still carries finish_reason="length", and keeping
        // that authority on the server lets external clients remain standards-compatible.
        if ("type" in parsed && parsed.type === "adaptive_checkpoint") {
          const { type: _type, ...checkpointFields } = parsed as unknown as
            AdaptiveCheckpointControl & { type: "adaptive_checkpoint" };
          const checkpoint = checkpointFields as AdaptiveCheckpointControl;
          if (typeof checkpoint.prompt_tokens === "number") {
            promptTokens = checkpoint.prompt_tokens;
          }
          yield {
            _adaptiveCheckpoint: checkpoint,
          } as OpenAIChatChunk;
          separatorIndex = buffer.search(/\r?\n\r?\n/);
          continue;
        }
