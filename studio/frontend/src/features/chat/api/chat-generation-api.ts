...
export type ChatTurnFinalizationStatus =
  | "none"
  | "pending"
  | "running"
  | "completed"
...
export interface ChatGenerationPendingApproval {
  runId: string;
  approvalId: string;
  sessionId: string;
  toolName: string;
...
export interface ChatGenerationRun {
  ...
  /** Present on Reliability Architecture v3 backends. Older servers omit it. */
  finalizationStatus?: ChatTurnFinalizationStatus;
  finalizationError?: string | null;
  finalizationStartedAt?: number | null;
  finalizationCompletedAt?: number | null;
  /** Authoritative unresolved approval snapshot. Omitted by older backends. */
  pendingApprovals?: ChatGenerationPendingApproval[];
  created?: boolean;
}
...
export function normalizeChatGenerationChunkPayload(
  ...
  if (
    ...
    (payload.type === "tool_start" ||
      ...
      payload.type === "tool_args" ||
      payload.type === "document_citations")
  ) {
  ...
  if (
    ...
    typeof payload === "object" &&
    "type" in payload &&
    payload.type === "adaptive_checkpoint"
  ) {
    const { type: _type, ...checkpointFields } = payload as Record<string, unknown> & {
      type: "adaptive_checkpoint";
    };
    return {
      _adaptiveCheckpoint: checkpointFields as unknown as AdaptiveCheckpointControl,
    } as OpenAIChatChunk;
  }
  return payload;
}
...
export function isLegacyFallbackChatGenerationAdmissionError(
  ...
  return (
    // Compatibility with an older backend that predates durable server-side
    // tool replay. Current Helix backends admit tool-capable local runs.
...
export function chatGenerationStopPlan(
  ...
  // Even after durable admission has been selected, the POST response can be lost after the
  // server accepted the stable run/cancel id. Until a run object comes back, fence that known
  // cancel_id through /inference/cancel: the backend can either cancel the registered generation
  // or stash the signal for one that is about to register. The pending create still cancels the
  // durable row directly if its in-flight response eventually arrives.
  if (decision === "durable") return { cancelRunId: null, postLegacyCancel: true };
...
export async function createChatGenerationRun(
  ...
  while (true) {
    if (retrySignal?.aborted) {
      throw retrySignal.reason ?? new DOMException("Aborted", "AbortError");
    }
    try {
      // Do not attach retrySignal to the POST itself. A Stop can race an ambiguous admission whose
      // request already reached the server; allowing that in-flight response to settle gives
      // createChatGenerationRunUntilAbort the run id it needs to cancel precisely. The signal only
      // prevents another POST after a failed attempt/backoff.
      ...
      failures += 1;
      await waitForReconnect(reconnectDelay(failures), retrySignal);
      if (retrySignal?.aborted) {
        throw retrySignal.reason ?? new DOMException("Aborted", "AbortError");
      }
    }
...
export async function createChatGenerationRunUntilAbort(
  ...
  const createPromise = createChatGenerationRun(input, signal);
