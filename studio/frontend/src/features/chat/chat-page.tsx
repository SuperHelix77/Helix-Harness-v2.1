...
const HelixWorkflowPanel = lazy(() =>
  import("@/features/helix-engine/engine-panels").then((module) => ({
    default: module.HelixWorkflowPanel,
  })),
);

const HelixExecutionGraphPanel = lazy(() =>
  import("@/features/helix-engine/engine-panels").then((module) => ({
    default: module.HelixExecutionGraphPanel,
  })),
);

const ArtifactSurface = lazy(() =>
  import("./artifacts/artifact-surface").then((module) => ({
    default: module.ArtifactSurface,
  })),
);

const ChatWorkspacePanel = lazy(() =>
  import("./components/chat-workspace-panel").then((module) => ({
    default: module.ChatWorkspacePanel,
  })),
);

const ResearchActivityPanel = lazy(() =>
  import("./components/research-activity-panel").then((module) => ({
    default: module.ResearchActivityPanel,
  })),
);

const ResearchActivitySheet = lazy(() =>
  import("./components/research-activity-panel").then((module) => ({
    default: module.ResearchActivitySheet,
  })),
);

const ChatSettingsPanel = lazy(() =>
  import("./chat-settings-sheet").then((module) => ({
    default: module.ChatSettingsPanel,
  })),
);
...
type HelixRailPanel = "workflow" | "execution" | null;
...
const SingleContent = memo(function SingleContent({
  ...
  helixRailPanel: HelixRailPanel;
  onHelixRailPanelChange: (panel: HelixRailPanel) => void;
}): ReactElement {
  ...
  const showWorkspacePanel =
    ...
    !showArtifactPanel &&
    !showPlanPanel;
  const showHelixWorkflowPanel =
    helixRailPanel === "workflow" &&
    !workspaceOpen &&
    !isMobile &&
    !showResearchPanel &&
    !showArtifactPanel &&
    !showPlanPanel;
  const showHelixExecutionPanel =
    helixRailPanel === "execution" &&
    !workspaceOpen &&
    !isMobile &&
    !showResearchPanel &&
    !showArtifactPanel &&
  ...
  const showContextPanel =
    ...
    showPlanPanel ||
    showWorkspacePanel ||
    showHelixWorkflowPanel ||
    showHelixExecutionPanel;

  useEffect(() => {
    if (showResearchPanel || showArtifactPanel || showPlanPanel) {
      if (workspaceOpen) onWorkspaceOpenChange(false);
      if (helixRailPanel) onHelixRailPanelChange(null);
    }
  }, [
    helixRailPanel,
    onHelixRailPanelChange,
        ...
        >
          ...
          >
             {showResearchPanel && openResearchRunId ? (
               ...
               <Suspense
                 fallback={
                   <div className="p-4 text-xs text-muted-foreground">Loading canvas…</div>
                 }
               >
                 <ArtifactSurface
                  artifact={artifact}
               ...
               <Suspense fallback={<div className="p-4 text-xs text-muted-foreground">Loading Helix Workflow…</div>}>
                 <HelixWorkflowPanel
                   threadId={goalThreadId}
                   onClose={() => onHelixRailPanelChange(null)}
                 />
               </Suspense>
             ) : showHelixExecutionPanel ? (
               <Suspense fallback={<div className="p-4 text-xs text-muted-foreground">Loading Execution Graph…</div>}>
                 <HelixExecutionGraphPanel
                   threadId={goalThreadId}
                   onClose={() => onHelixRailPanelChange(null)}
                 />
               </Suspense>
               ...
               <Suspense
                 fallback={
                   <div className="p-4 text-xs text-muted-foreground">Loading workspace…</div>
                 }
               >
                 <ChatWorkspacePanel
                   threadId={goalThreadId}
  ...
  return (
    <CompareHandlesProvider handlesRef={handlesRef}>
      <div className="flex min-h-0 min-w-0 flex-1 basis-0 flex-col">
        ...
        <div className="shrink-0 app-main-chrome-background pl-5 pr-5 md:pr-[30px] pb-2 pt-1">
...
function GeneralCompareHeader({
  ...
  return (
    <div
      className={cn(
        "pointer-events-none relative z-40 flex h-[48px] shrink-0 items-start gap-2 app-main-chrome-background pt-[var(--studio-chat-header-padding-top,11px)]",
        side === "left"
          ? pinned
            ? "pl-12 pr-3 md:pl-2"
...
export function ChatPage({
  ...
  const workspaceOpen =
    active && workspaceState.scope === workspaceScope && workspaceState.open;
  const setWorkspaceOpen = useCallback(
    (open: boolean) => setWorkspaceState({ scope: workspaceScope, open }),
    [workspaceScope],
  );
  const [helixRailState, setHelixRailState] = useState<{
    scope: string;
    panel: HelixRailPanel;
  }>({ scope: workspaceScope, panel: null });
  const helixRailPanel =
    active && helixRailState.scope === workspaceScope ? helixRailState.panel : null;
  const setHelixRailPanel = useCallback(
    (panel: HelixRailPanel) => setHelixRailState({ scope: workspaceScope, panel }),
    [workspaceScope],
  );

  // Chat stays mounted while the user visits other product surfaces. If these
  // states are only hidden on a scope mismatch, returning to the same chat can
  // make a previously open manual rail reappear. Treat route/search scope
  // transitions as a UI boundary and carry the new scope forward closed.
  useEffect(() => {
    if (!active) {
      setWorkspaceState({ scope: workspaceScope, open: false });
      setHelixRailState({ scope: workspaceScope, panel: null });
      return;
    }
    setWorkspaceState((state) =>
      state.scope === workspaceScope
        ? state
        : { scope: workspaceScope, open: false },
    );
    setHelixRailState((state) =>
      state.scope === workspaceScope
        ? state
        : { scope: workspaceScope, panel: null },
    );
  ...
  // Run Settings is a large optional surface. Do not resolve its lazy module on
  // first paint merely to render a closed sheet; after the first open, keep it
  // mounted so local drafts and reconciliation effects retain their historical
  // lifetime across close/reopen cycles.
  const [settingsModuleMounted, setSettingsModuleMounted] = useState(settingsOpen);
  useEffect(() => {
  ...
  const transientUiScopeRef = useRef(workspaceScope);
  useEffect(() => {
    const scopeChanged = transientUiScopeRef.current !== workspaceScope;
    transientUiScopeRef.current = workspaceScope;
    if (active && !scopeChanged) return;

    // Persistent Chat keeps runtimes alive across navigation; route-owned
    // surfaces must not survive with them. Closing UI state here also prevents
    // a panel/popover from being portaled above another product page.
    ...
    <div className="flex min-h-0 min-w-0 flex-1 basis-0 overflow-hidden app-main-background">
      ...
      <div className="relative flex min-h-0 min-w-0 flex-1 basis-0 flex-col overflow-hidden has-[>[data-chat-model-notice]]:[--studio-chat-notice-height:2.25rem]">
        ...
        {view.mode !== "compare" && (
          <div
            aria-hidden
            className="chat-header-fade pointer-events-none absolute left-0 right-[10px] top-[calc(var(--studio-content-top-inset,0px)+var(--studio-chat-header-height,48px)+var(--studio-chat-notice-height,0px))] z-20 h-6 bg-gradient-to-b from-[var(--main-chrome-surface)] to-transparent"
          />
        )}
        <div
          className={cn(
            "pointer-events-none absolute top-[var(--studio-content-top-inset,0px)] left-0 right-[10px] z-40 flex h-[var(--studio-chat-header-height,48px)] shrink-0 items-start app-main-chrome-background pt-[var(--studio-chat-header-padding-top,11px)] pr-[calc(0.5rem+var(--studio-chat-header-right-inset,var(--studio-window-control-inset,0px)))]",
            isMobile
              ? "pl-12"
              : pinned
                ? "pl-2"
        ...
        >
          <div className="pointer-events-auto flex items-center gap-1">
            ...
            {view.mode !== "compare" && (
              <ModelSelector
                ...
                className="max-w-[62vw] border border-border/45 !pr-3 sm:max-w-none !h-[var(--studio-chat-control-height,34px)]"
              />
            )}
          ...
          <div className="pointer-events-auto ml-auto flex items-center gap-1">
            ...
            {view.mode === "single" && !isMobile && (
              <Tooltip>
                <TooltipPrimitive.Trigger asChild={true}>
                  <button
                    type="button"
                    onClick={() => {
                      if (helixRailPanel === "workflow") {
                        setHelixRailPanel(null);
                        return;
                      }
                      closeArtifactSurface();
                    ...
                    aria-label={
                      helixRailPanel === "workflow"
                        ? "Close Helix Workflow"
                        : "Open Helix Workflow"
                    }
                    aria-pressed={helixRailPanel === "workflow"}
                  >
                    <HugeiconsIcon icon={FlowchartIcon} strokeWidth={1.75} className="size-icon" />
                  </button>
                </TooltipPrimitive.Trigger>
                <TooltipContent side="bottom" sideOffset={6} className="tooltip-compact">
                  Helix Workflow
                ...
                <TooltipPrimitive.Trigger asChild={true}>
                  <button
                    ...
                    onClick={() => {
                      ...
                      useChatRuntimeStore.getState().setChatMode("normal");
                      setWorkspaceOpen(false);
                      setHelixRailPanel("execution");
                    }}
                    className="flex size-[30px] cursor-pointer items-center justify-center rounded-[10px] text-nav-fg transition-colors hover:bg-nav-surface-hover hover:text-black dark:hover:text-white focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-ring"
                    aria-label={
                      helixRailPanel === "execution"
                        ? "Close Execution Graph"
                        : "Open Execution Graph"
                    }
                    aria-pressed={helixRailPanel === "execution"}
                  >
                    <HugeiconsIcon icon={HierarchyIcon} strokeWidth={1.75} className="size-icon" />
                  </button>
                </TooltipPrimitive.Trigger>
                <TooltipContent side="bottom" sideOffset={6} className="tooltip-compact">
                  Execution Graph
            ...
            {view.mode === "single" && !isMobile && (
              <Tooltip>
                ...
                <TooltipContent side="bottom" sideOffset={6} className="tooltip-compact">
                  Workspace: outputs, memory, sources, subagents
                </TooltipContent>
              </Tooltip>
            )}
            {!settingsOpen && (
                ...
                {baseView.mode === "project" ? (
                  ...
                  >
                    <SingleContent
                      ...
                      helixRailPanel={helixRailPanel}
                      onHelixRailPanelChange={setHelixRailPanel}
                    />
                  </NativeAttachmentTargetContext.Provider>
                )}
              </ChatRuntimeProvider>
