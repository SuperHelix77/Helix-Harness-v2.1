...
import { addLearningExperience } from "./api/memory-api";
import { recordSelfTrainingExample } from "./api/self-training-api";
...
function toThreadMessage(m: MessageRecord): ThreadMessage {
  ...
  const contentEmpty =
    ...
    m.content.length === 0 ||
    m.content.every((part) => {
      if (!part || typeof part !== "object") return true;
      const record = part as { type?: unknown; text?: unknown };
      return (
        (record.type === "text" || record.type == null) &&
...
function recoveredFinalizationReceiptDone(
  metadata: Record<string, unknown>,
  field: string,
  idempotencyKey: string,
): boolean {
  const receipt = metadata[field] as
    | { idempotencyKey?: unknown; status?: unknown }
    | undefined;
  return (
...
async function recoverPostAnswerFinalization(options: {
  run: ChatGenerationRun;
  threadId: string;
  content: MessageRecord["content"];
  current: Record<string, unknown>;
}): Promise<Record<string, unknown>> {
  const { run, threadId, content } = options;
  const idempotencyKey = run.requestPayload.finalization_idempotency_key;
  // Old durable runs completed before the keyed contract existed. Replaying them under a newly
  // invented key would create new side effects, so only requests that persisted the key opt in.
  if (
  ...
  if (run.finalizationStatus && run.finalizationStatus !== "none") {
    return {
      ...options.current,
      turnFinalizationStatus: run.finalizationStatus,
      turnFinalizationIdempotencyKey: idempotencyKey,
      ...(run.finalizationError
        ? { turnFinalizationError: run.finalizationError }
        : {}),
    };
  }

  const prompt = recoveredGenerationUserText(run.requestPayload).trim();
  const rawAssistant = generationRawContent(content).raw;
  const visibleAssistant = recoveredGenerationAssistantText(content);
  const completion = parseOnTheFlySkillDraft(visibleAssistant)
    ? stripOnTheFlySkillDraft(stripHermesLearningProposal(visibleAssistant))
  ...
  const protocolTurn =
    isHermesLearningReviewRequest(prompt) ||
    isHelixSelfCriticRequest(prompt) ||
    isHelixSelfAuditRequest(prompt);
  if (!prompt || !rawAssistant.trim() || !completion || protocolTurn) {
    const reason = !prompt
      ? "missing_prompt"
      : !rawAssistant.trim() || !completion
        ? "missing_completion"
        : "learning_protocol_turn";
    for (const field of [
  ...
  if (
    !recoveredFinalizationReceiptDone(
      next,
      "recoveredMemoryFinalization",
      idempotencyKey,
    )
    ...
    const memoryInput = {
      text: `User: ${prompt.slice(0, 1500)}\nAssistant: ${rawAssistant.slice(0, 1500)}`,
      threadId,
      kind: "experience",
      title: prompt.slice(0, 80) || "Completed turn",
      idempotencyKey,
    };
    let memoryResult: Awaited<ReturnType<typeof addLearningExperience>> | null = null;
    for (let attempt = 0; attempt < 2; attempt += 1) {
      memoryResult = await addLearningExperience(memoryInput);
      if (!memoryResult.transportError) break;
    }
    next = {
      ...next,
      recoveredMemoryFinalization: {
        idempotencyKey,
        status: memoryResult?.transportError ? "failed" : "completed",
        receipt: memoryResult,
      },
    };
  }
  ...
  if (
    ...
    next = await recoverCompletedTurnSelfTraining({
      current: next,
      idempotencyKey,
      execute: () =>
        recordSelfTrainingExample({
          modelId: run.requestPayload.model,
          prompt,
          completion,
  ...
  if (
    ...
    const storedEnvelope = next.helixFinalIngestRequest;
    const adapterState =
      typeof run.requestPayload.use_adapter === "boolean"
        ? run.requestPayload.use_adapter
        : undefined;
    const effectiveModelId = `${run.requestPayload.model}::adapter=${
      adapterState === true
        ? "enabled"
        : adapterState === false
          ? "disabled"
    ...
    const ingestRequest: HelixFinalIngestRequest =
      ...
      (storedEnvelope as { idempotency_key?: unknown }).idempotency_key ===
        idempotencyKey
        ? (storedEnvelope as HelixFinalIngestRequest)
        : {
            idempotency_key: idempotencyKey,
            session_id: run.requestPayload.session_id || threadId,
            thread_id: threadId,
            turn_id: run.id,
            prompt: prompt.slice(0, 4_000),
            final_result: rawAssistant.slice(0, 4_000),
            model_id: run.requestPayload.model,
            effective_model_id: effectiveModelId,
            ...(adapterState === undefined ? {} : { adapter_state: adapterState }),
            telemetry: {
              recovered_finalization: true,
              finalization_idempotency_key: idempotencyKey,
            },
          };
    next = { ...next, helixFinalIngestRequest: ingestRequest };
    let ingestReceipt: Record<string, unknown> | null = null;
    let ingestError: unknown;
    for (let attempt = 0; attempt < 2 && ingestReceipt === null; attempt += 1) {
      try {
        ingestReceipt = await ingestHelixTurn(ingestRequest);
      } catch (error) {
        ingestError = error;
      }
    }
    next = {
      ...next,
      recoveredHelixFinalization: ingestReceipt
        ? {
            idempotencyKey,
            status: "completed",
        ...
        : {
            ...
            error:
              ingestError instanceof Error
                ...
                : String(ingestError ?? "final ingest replay failed").slice(0, 500),
          },
    };
  }
...
function scheduleGenerationRecovery(
  ...
  const needsGenerationRecovery = generationNeedsRecovery(metadata);
  const needsFinalizationRecovery = generationNeedsRecoveredFinalization(metadata);
  if (
    typeof runId !== "string" ||
    (!needsGenerationRecovery && !needsFinalizationRecovery)
  ) {
    return;
  }
  ...
  if (!needsGenerationRecovery && needsFinalizationRecovery) {
    const finalizationOnly = (async () => {
      const run = await getChatGenerationRun(runId);
      if (
        run.threadId !== threadId ||
        run.assistantMessageId !== storedMessage.id ||
        run.status !== "completed" ||
        run.finishReason === "length"
      ) {
        return;
      }
      const nextMetadata = await recoverPostAnswerFinalization({
        run,
        threadId,
      ...
      for (const view of views) {
        ...
        try {
            ...
            messages: exported.messages.map((item) =>
              item.message.id === storedMessage.id
                ? {
                    ...
                    message: {
                      ...item.message,
                      status: restoredAssistantStatus({ custom: nextMetadata }),
                      metadata: {
          ...
          // Storage already has the receipt if the view changed during finalization.
        }
      }
    })()
      .catch(() => undefined)
      .finally(() => generationRecoveries.delete(runId));
    generationRecoveries.set(runId, { promise: finalizationOnly, views });
    return;
  }
  ...
  const recovery = (async () => {
    ...
    let recoveryStopRequested = metadata.recoveryStopRequested === true;
    const checkpointAbort = new AbortController();
    const serverCancel = () => {
      recoveryStopRequested = true;
      if (!checkpointAbort.signal.aborted) {
        checkpointAbort.abort(new DOMException("Aborted", "AbortError"));
      }
      // Persist the user's intent independently of the server terminal state. The segment may
      // already be `completed/length` while checkpoint work is still in flight; after reload that
      // must remain a Stop, not become an automatic adaptive continuation.
      void commit(
        {
          ...currentMetadata,
          recoveryStopRequested: true,
    ...
    const syncPendingApprovals = (run: ChatGenerationRun): boolean => {
      const recovered = toolRecovery.syncPendingApprovals(
        isTerminalChatGenerationRun(run) ? [] : run.pendingApprovals,
        raw.length,
      );
      if (!recovered) return false;
      runtime.syncToolConfirmationsForRun(
        runId,
        recovered.confirmations.map((confirmation) => ({
          ...confirmation,
          autoAllowKey: `${confirmation.sessionId || "_default"}:${threadId}`,
        })),
      );
    ...
    const publish = async (run: ChatGenerationRun) => {
      ...
      if (
        ...
        nextMetadata.recoveryStopRequested === true
      ) {
        nextMetadata = {
          ...nextMetadata,
          recoveryStopRequested: true,
      ...
      if (nextMetadata.generationSettled === true) {
        ...
        nextMetadata = recoveredGenerationFinalMetadata({
          ...
          firstChunkAt,
          totalChunks,
          toolCalls: toolNames(content),
        });
        if (run.status === "completed" && !lengthLimited) {
          nextMetadata = await recoverPostAnswerFinalization({
            run,
            threadId,
            content,
            current: nextMetadata,
    ...
    try {
      ...
      try {
        for await (const update of followChatGenerationRun(runId, {
          ...
          for (const toolCallId of toolRecovery.takeEndedConfirmationCardIds()) {
            runtime.clearToolConfirmation(toolCallId);
          }
          const approvalsChanged = syncPendingApprovals(update.run);
          if (update.event && update.event.seq > cursor) {
            ...
            if (update.event.type === "chunk") {
              const chunk = update.event.payload as {
                ...
                _adaptiveCheckpoint?: OpenAIChatChunk["_adaptiveCheckpoint"];
                usage?: {
                  prompt_tokens?: unknown;
                  completion_tokens?: unknown;
                  total_tokens?: unknown;
              ...
              if (chunk._adaptiveCheckpoint) {
                const adaptiveCheckpoint = chunk._adaptiveCheckpoint;
                const durableRound = recoveredAdaptiveResumeRound(
                  update.run.requestPayload,
                  adaptiveCheckpoint,
                );
                if (durableRound === null) {
                  currentMetadata = {
                    ...currentMetadata,
                    adaptiveCheckpointRecoveryError: {
                      runId,
                      eventSeq: update.event.seq,
                      reason: "missing_durable_resume_round",
                    },
                  };
                  // Do not advance the persisted event cursor. A future build/backend with the
                  // missing durable round can replay this same event safely.
                  await commit(currentMetadata, true);
                  lastPublishedStatus = update.run.status;
                  continue;
                }
                const adapterState =
                  typeof update.run.requestPayload.use_adapter === "boolean"
                    ? update.run.requestPayload.use_adapter
                    : undefined;
                const effectiveModelId = `${update.run.requestPayload.model}::adapter=${
                  adapterState === true
                    ? "enabled"
                    : adapterState === false
                      ? "disabled"
                ...
                const checkpoint = await recoverAdaptiveCheckpointEvent({
                  current: currentMetadata,
                  control: adaptiveCheckpoint,
                  runId,
                  eventSeq: update.event.seq,
                  resumeRound: durableRound,
                  signal: checkpointAbort.signal,
                  // The pending receipt deliberately carries the OLD generationSeq. A crash in
                  // the request/response gap therefore replays this control event and asks the
                  // backend for its durable receipt instead of skipping it.
                  persistPending: (pending) => commit(pending, true),
                  execute: () =>
                    runAdaptiveCheckpoint({
                      checkpointEventSeq: update.event!.seq,
                      runId,
                      resumeRound: durableRound,
                      sessionId:
                        update.run.requestPayload.session_id || threadId,
                      threadId,
                      turnId: runId,
                      prompt: recoveredGenerationUserText(
                        update.run.requestPayload,
                      ),
                      partialResult: recoveredGenerationAssistantText(rebuild()),
                      modelId: update.run.requestPayload.model,
                      effectiveModelId,
                      adapterState,
                      signal: checkpointAbort.signal,
                      telemetry: {
                        adaptive_checkpoint: adaptiveCheckpoint,
                        adaptive_resume_round: durableRound,
                        checkpoint_event_seq: update.event!.seq,
                        generation_run_id: runId,
                        prompt_tokens: adaptiveCheckpoint.prompt_tokens,
                        completion_tokens:
                          adaptiveCheckpoint.completion_tokens,
                        occupancy_tokens:
                          adaptiveCheckpoint.occupancy_tokens,
                        context_length: adaptiveCheckpoint.context_length,
                        trigger_tokens: adaptiveCheckpoint.trigger_tokens,
                        segment_max_tokens:
                          adaptiveCheckpoint.segment_max_tokens,
                        context_ratio: adaptiveCheckpoint.ratio,
                        context_truncation:
                          currentMetadata.contextTruncation ?? null,
                      },
                    }),
                });
                currentMetadata = checkpoint.metadata;
                if (checkpoint.status === "aborted") {
                  // Save the Stop marker without consuming the control event. On reload the same
                  // identity is recognized as user-cancelled and checkpoint effects stay off.
                  currentMetadata = {
                    ...currentMetadata,
                    recoveryStopRequested: true,
                    incomplete: { reason: "cancelled" },
                ...
                // Completed and fail-open failed checkpoints have settled this control event. The
                // ordinary publish now advances generationSeq and makes the adaptive metadata the
                // exact custom.adaptiveCheckpoint shape used by the continuation/lease path.
                lastPublishedStatus = update.run.status;
      ...
      store.syncToolConfirmationsForRun(runId, []);
