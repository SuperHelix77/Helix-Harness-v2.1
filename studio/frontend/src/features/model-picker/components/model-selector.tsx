...
interface ModelSelectorProps {
  ...
  /** Show a compact, localized "Model" affordance before the selected name. */
  ...
  return (
    <PopoverTrigger asChild={true}>
      <button
        ...
        aria-label={
          controlLabel
            ? `${controlLabel}: ${currentModel?.name ?? placeholder}`
            : undefined
        }
        className={cn(
          "helix-model-selector-trigger unsloth-model-selector-trigger group/trigger flex min-w-0 items-center gap-2 transition-colors",
      ...
      >
        {controlLabel ? (
          <>
            <span className="shrink-0 text-ui-10 font-semibold uppercase tracking-[0.12em] text-muted-foreground">
              {controlLabel}
            </span>
  ...
  return (
    <Popover open={open} onOpenChange={setOpen}>
      <ModelSelectorTrigger
        ...
        controlLabel={
          showControlLabel ? t("studio.wizard.modelLabel") : undefined
        }
